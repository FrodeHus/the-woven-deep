...
- **Codex discovery derives from session data.** Discovered = what this session's Hall records plus the active run have seen: monsters from perceived encounters, items identified or used, spells cast, classes played. One pure `deriveCodexState(records, activeRun?, pack)` in the session layer. A first-class cross-session discovery ledger is milestone 6 profile work.
