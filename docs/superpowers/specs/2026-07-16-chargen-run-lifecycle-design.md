...
- `class`: `description`, `playable: boolean`, `silhouetteGlyph` (uncolored — a per-class accent color was dropped during implementation; class rows inherit the standard glyph palette), `unlockHint` (required on non-playable entries), `classTags` (non-empty; these flow into Hall records), and `kits` — two to three named kits, each a list of equipped entries (`contentId`, `slot`, optional `enabled`) and backpack entries (`contentId`, optional `quantity`). Cross-file validation: every kit item exists, is equippable in the named slot (or is a valid backpack item), and playable classes have at least two kits.
