...
The Tactical Triptych per the master design: the map is the primary center region and never yields primacy; the layout is responsive with the map taking the most real estate at every size (amended during implementation, pulled forward from 5D):

- The viewport is dynamic: cell size stays tied to the monospace font (one `ch` × one line-height), and the visible cell window grows with the map pane — a larger browser window shows more map, bounded by the floor size, with a minimum window floor so tiny panes stay playable. A `ResizeObserver` on the map pane feeds the camera's viewport size; font scaling itself is 5D settings work.
- Space is surrendered in this order as the window shrinks, map last: first the threat panel collapses (its per-actor details become a hover popover on actor cells plus a keyboard-openable drawer holding the same information — hover is a mouse convenience, never the only path, per the accessibility rules); then the hero panel collapses to a slim always-visible vitals strip (health, hunger, light) with the rest in its drawer; the event log shrinks to a three-line minimum but never disappears, because it is the textual-equivalents channel.
