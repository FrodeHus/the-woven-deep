...
**Milestone 5 COMPLETE** (5A guest play core, 5B chargen and run lifecycle, 5C town slice, 5D-1 guest interface, 5D-2 experience polish — all e2e gates green).
...
**6A identity and persistence foundation — this plan's scope, e2e gate green:** `docs/superpowers/plans/2026-07-18-identity-persistence.md` (design: `docs/superpowers/specs/2026-07-18-identity-persistence-design.md`) — email magic-link sign-in with server-side profiles and roaming settings is proven end-to-end by `apps/web/e2e/auth.spec.ts` (`npm run guest:e2e` green, all six specs, run green twice consecutively): a guest boots the title, chooses "Sign in with email", submits a unique address, and sees the uniform confirmation; the magic link is retrieved from the dev-echo endpoint (`GET /api/dev/last-login-link`, active because the e2e server runs with no Mailgun config), followed to establish the session, and the app boots signed in with the email on the title; the theme is switched to high contrast and roams to the server; a fresh browser context carrying only the session cookie (empty localStorage) confirms the high-contrast theme arrived from the server, and sign-out returns the title to the guest menu. The five prior specs (`guest-play.spec.ts`, `run-lifecycle.spec.ts`, `town-loop.spec.ts`, `interface.spec.ts`, `polish.spec.ts`) remain green and untouched. Layered Fastify/`better-sqlite3` server (ordered `user_version` migrations, per-table repositories, injected clock/randomness/mail-transport auth services, cookie/CSRF/origin route wiring) with a framework-free client account module; security properties enforced and documented in `docs/server-admin/authentication.md` (hashed 256-bit tokens, 15-minute single-use links, `HttpOnly`/`Secure`/`SameSite=Lax` revocable sessions, origin + CSRF on authenticated mutations, per-email/per-source rate limits, no secrets in the browser bundle). `NODE_ENV=production` now requires an explicit non-localhost `PUBLIC_URL` (`config.ts` fails loud), and the Dockerfile/compose env surface carries `PUBLIC_URL`, `COOKIE_SECRET`, `MAILGUN_*`, and the two `LOGIN_RATE_LIMIT_*` names. Full verification block green: root `npm test` (1225), typecheck, build, `content:validate`, all five demos plus the run-records demo (zero engine/content drift — `git diff <base>..HEAD --stat -- packages/engine packages/content content` empty), and smoke (against a running server). `content:startup-gate` is a Docker image build+run gate and needs a Docker-capable host; where the daemon is unavailable the documented fallback holds (the new `PUBLIC_URL`/`COOKIE_SECRET`/`MAILGUN_*`/`LOGIN_RATE_LIMIT_*` keys are present in `Dockerfile` and `compose.yaml`, and `config.ts` accepts a production-shaped env by test).

**6B server-authoritative runs — complete:** `docs/superpowers/plans/2026-07-22-server-authoritative-runs.md` (design: `docs/superpowers/specs/2026-07-21-server-authoritative-runs-design.md`) — signed-in profiles play on the server with the same `packages/engine`: run orchestration extracted into the shared framework-free `@woven-deep/session-core` used by both the guest client and the new server play path, authoritative run state in SQLite (`active_runs`) over a WebSocket play endpoint, a client `ProfileSession` behind the `RunSession` interface, and client/server engine agreement proven by a cross-process determinism harness. Guests unchanged.

**6C verified Hall & metaprogression — complete:** `docs/superpowers/plans/2026-07-23-verified-hall.md` (design: `docs/superpowers/specs/2026-07-22-verified-hall-design.md`) — the server finalizes a profile's concluded run into a persisted Hall by calling the pure `finalizeRun`/scoring/heirloom/achievement/lifetime engine logic server-side, evaluates class unlocks, and exposes self-service data export (`GET /api/profile/export`) and account deletion (`DELETE /api/profile`). Guest Hall stays `sessionStorage`-scoped and byte-identical; no guest→profile promotion.

**Milestone 6 COMPLETE** (6A identity and persistence, 6B server-authoritative runs, 6C verified Hall & metaprogression). **Milestone 7 (town progression and full campaign content) is in progress.**
...
**Delivered so far (as of 2026-07-27):**

- **Endgame Final Chamber & endings — complete:** `docs/superpowers/plans/2026-07-21-endgame-final-chamber.md` (design: `docs/superpowers/specs/2026-07-21-endgame-final-chamber-design.md`) — all four completion types are producible: the authored depth-20 Final Chamber, the three ending choices via the `final-chamber-choice` command (Become the Heart → `became-heart`; Turn away → the weakened Heart boss fight → win `refused` / lose forced `became-heart`; Assemble the tablet → `broke-cycle`), Ancient Tablet fragment items with a rare deep spawn, and the Heart lineage store write with predecessor display.
- **Magic system (scope addition to this milestone) — complete:** engine/content/server plan `docs/superpowers/plans/2026-07-23-magic-system-engine.md` and client casting UI plan `docs/superpowers/plans/2026-07-24-casting-ui.md` — AoE targeting with deterministic multi-actor sweep, learn-from-tome, recall/town-portal, a condition damage-over-time tick, a spell vendor, a ~14-spell spellbook, and the client's live footprint preview + spellbook overlay, proven headless by the deterministic `magic:demo` replay and the parity harness.
- **7A deep dungeon (depths 13–19) — complete:** `docs/superpowers/plans/2026-07-24-deep-dungeon.md` — pure content: two Heart-corrupted monster families (`the-bound`, `echo-wrought`) with encounters, a deep-antechamber vault, a second deadlier trap, deep reward relics, and deeper merchant tiers restoring a monotonic ramp from depth 12 to the depth-20 boss.
- **7B milestone bosses — complete:** `docs/superpowers/plans/2026-07-24-milestone-bosses.md` — three guaranteed milestone bosses (the Ashfather at depth 5, the Tide-Sovereign at 10, the Heart-Herald at 15), each in a forced boss-arena vault with a unique reward drop, guaranteed via `milestone-boss`-tagged vault forcing plus a boss pre-pass in `placeFloorPopulations` that fails loudly if placement is impossible.
- **7C metaprogression completion — complete:** `docs/superpowers/plans/2026-07-25-metaprogression-completion.md` — achievements as a parameterized, referentially-validated `criteria` union with a real 10-entry roster (defeat-boss/fallen-hero/reach-depth/complete-ending), the per-run `defeatedBossMonsterIds` signal, and data-driven class-unlock conditions replacing the hardcoded `UNLOCK_RULES` map (with corrected hints).
- **Gameplay slices — complete:** findable items (G5, `docs/superpowers/plans/2026-07-19-findable-items-g5.md` — monster loot drops and vault item-slot filling), locks & lockpicking (G7, `docs/superpowers/plans/2026-07-20-locks-lockpicking-g7.md` — locked doors/chests, `d20 + disarm` picking, keys, the production feature spawner), light-out feats (`docs/superpowers/plans/2026-07-20-light-out-feats.md`), and NPC dialogue (#79, `docs/superpowers/plans/2026-07-26-npc-dialogue.md` — topic-based conversation with reputation/lore/trade consequences, the travelling lampwright first).

**Remaining in this milestone:** profile hero-customization — reachable class unlocks + portrait selection (issue #95) — and the balance-complete YAML content pass.
