...
- `src/compiler/content-validation.ts`: kit item/slot cross-checks incl. intra-kit reservedSlots conflicts (a two-handed item's reserved slots must stay empty within the kit), playable-classes-need-2-kits, unlockHint-required-when-locked, modifier keys ⊆ `DERIVED_STAT_NAMES`, point-buy table covers bounds with non-decreasing costs.
