...
- Floor variants collapse with adjacency weighting: a cell becomes `floor-dirty` iff `cellSeed % 4 === 0` **or** at least two already-collapsed orthogonal neighbors (scan order: y-major) are dirty and `cellSeed % 2 === 0` — this clusters dirt coherently without unseeded randomness.
