...
|---|---|---|---|
| `schemaVersion` | integer | Required | Must be exactly `18`. |
...
| `artifactOfferPercent` | integer 0-100 | Yes | Percent chance used by the legendary-artifact offer roll. The bundled value is `12`. |

```yaml
generation:
  doorTilePercent: 35
  artifactOfferPercent: 12
```
...
A `permanent` merchant (`permanent: true`) is a fixed town shopkeeper: it never departs, so it must omit `minimumLifetime`, `maximumLifetime`, and `departureWarningThresholds` entirely. The three bundled town merchants — the Provisioner, the Armorer, and the Curios Dealer — are each `permanent: true`, each carries a distinct NPC faction and stock loot table, and each declares `requiredVaultTags: [town]` so placement resolves only against the `town` vault. The `merchant-service.strongbox` service, offered by the Town Provisioner at base price `120` across every faction tier, lets a hero rent house storage; because a strongbox purchase is one-time per merchant relationship, its offer requires `minimumUses` and `maximumUses` of exactly `1`. Restocking a permanent merchant's stock loot table is driven by the balance entry's `restockMilestones`. A restock also re-arms every service offer: each offer's remaining uses are re-rolled from its authored `minimumUses..maximumUses` band, while the offer's ID, `basePrice`, and tier gating stay exactly as authored. That is what makes a service a repeatable gold sink rather than a one-off — note the strongbox is unaffected in practice, since its own `house.upgradesPurchased` gate caps it at one purchase per run regardless of remaining uses. The Town Curios Dealer also offers `merchant-service.remove-curse` alongside `merchant-service.identify`: it targets a hero-owned item whose curse has been revealed, deletes the curse and leaves everything else about the item (enchantment, identification, condition) untouched, and consumes no randomness. The Town Armorer offers `merchant-service.enchant` at base price `80`: it targets a hero-owned, ordinary (non-artifact, non-revealed-cursed) piece of equipment and draws a fresh enchantment on the `enchanting` stream. A first enchant costs the authored base price; a re-enchant of an already-enchanted item costs double and replaces the old draw outright -- re-forging an enchanted piece costs double, and the Weave may give you less than it gave before.
...
- A service quotes `basePrice * tier purchasePriceBps / 10000`, rounded up, with the same nonzero minimum of `1`. The `basePrice` fed into that quote is first multiplied by the run's service step: `1`, plus `1` for every `restockMilestones` entry the run has already fired. So the same service costs `1x` before the first milestone and `5x` once all four bundled milestones have passed. The step composes with the enchant service's re-enchant doubling, making a late-run re-forge the catalog's most expensive act. Retune it by editing `restockMilestones` — there is no separate knob.
...
Content schema version `8` added the `currency` item category (see `item.gold-coins` in the bundled pack) and the required balance `floorLoot` block described above. Migration from 7: bump every file's `schemaVersion` to 8, and add a `floorLoot` block to the balance entry; the new `currency` category is available to items but optional. (The required balance `fragmentSpawnRollDenominator` field predates this and was introduced during version `7` without its own bump; a pack authored against an early v7 needs it too.)

Content schema version `9` adds the required balance `generation` block with `doorTilePercent`, the knob the junction-door pass rolls. Migration from 8: bump every file's `schemaVersion` to 9 and add `generation: { doorTilePercent: 35 }` to the balance entry; no other field changes. Every bundled source file declares `schemaVersion: 9`, and the compiled pack hash covers the bumped envelope, so a pinned pack hash changes with this version even when no entry content changed.
...
Content schema version `17` adds the balance `starvationDamageIncrement` and `starvationDamageMaximum` knobs, which turn starvation from a flat tick into an escalating one. Migration from 16: bump every file's `schemaVersion` to 17 and add both fields to the balance entry. `starvationDamageIncrement` is a non-negative safe integer added to the previous tick's damage on each successive starvation tick; `starvationDamageMaximum` is a positive safe integer ceiling that must be at least `starvationDamage` (the compiler rejects a maximum below the floor). Damage on the *n*-th consecutive tick is `min(starvationDamage + (n - 1) * starvationDamageIncrement, starvationDamageMaximum)`. Setting `starvationDamageIncrement: 0` and `starvationDamageMaximum: starvationDamage` reproduces the v16 flat tick exactly, so a v16 balance entry migrates without any behavioural change. The tick counter resets whenever the hero stops starving, so eating always returns the next tick to `starvationDamage`. No other field changes.

Content schema version `18` adds `surrendered` to `CompletionType`, the completion a run records when the player gives it up deliberately rather than dying or reaching an ending at the Final Chamber. Migration from 17: bump every file's `schemaVersion` to 18 and add a `surrendered` key to the balance entry's `score.completionBonus` map, which is a closed record over `CompletionType` and rejects a pack that omits it. A value of `0` reproduces the shipped balance — the same bonus an ordinary death pays, because the cost of surrendering is charged against banked Ancient Tablet fragments rather than against score: a surrendered run banks none of the fragments its hero was carrying. No other field changes, and no existing entry is affected. For Hall ordering, `surrendered` shares the bottom tier with `died`, so the two sort against each other on score alone.
...
- `appeasement` describes how a haunt of this template can be appeased instead of fought. `classFavors` maps a free-form class tag to the item categories a hero of that calling is remembered wanting (an unlisted tag contributes nothing); `causelessCategories` is accepted additionally when the haunt's death record has no cause, or a cause with no killer entity; `defaultCategories` is accepted when the class-tag favors contribute nothing, and must be non-empty so every haunt stays appeasable. Every category list is drawn from the closed `ItemCategory` registry, excludes `currency` (coins are a transaction, not an offering), and rejects duplicates within one list.
...
| `modifiers` | map of `DerivedStatName` to integer | Defaults to `{}` | Derived-stat bonuses granted while the item is equipped. Every key must be a registered derived-stat name and every value strictly positive. Use this for an identity the `combat` block cannot express (`weaveRegen`, `search`, `spellPower`) without promoting the item to a legendary artifact. A non-empty block requires a non-null `equipment` block. |
...
| `artifact` | object or null | Yes | Legendary-artifact definition, described below. Null for every ordinary item. |
...
`artifact` marks an item as a legendary artifact. It is `null` on every non-artifact item.

| Field | Type | Required | Rules and meaning |
|---|---|---|---|
| `canon` | `true` | Yes | Literal marker; the block is never authored with any other value. |
| `signature` | object or null | Yes | A signature spell the artifact grants: `spellId` (must resolve to a `spell`), a positive `charges`, and a non-negative `rechargePerFloor` that cannot exceed `charges`. Null when the artifact grants no signature spell. |
| `drawbackModifiers` | map of `DerivedStatName` to integer | Yes | Negative derived-stat penalties the artifact imposes while carried or equipped. Every key must be a registered derived-stat name and every value must be negative. May be empty only when `light.inextinguishable` is `true`. |
| `light` | object or null | Yes | `fuelless` and `inextinguishable` booleans describing an artifact's light behavior. A non-null `artifact.light` requires the item's own `light` block to also be non-null. |

An artifact item must additionally satisfy: `rarity: legendary`, `stackLimit: 1`, `identification: { mode: known }`, and at least one of `signature` or a non-null `combat` block (or both). A content ID carrying a non-null `artifact` block can never appear as a direct choice in a loot table -- the same exclusion already applied to guaranteed boss-unique items -- since artifacts are offered through the dedicated artifact-offer roll (`generation.artifactOfferPercent`) rather than ordinary loot placement.

```yaml
artifact:
  canon: true
  signature: { spellId: spell.cinder-breath, charges: 3, rechargePerFloor: 1 }
...
Each content-kind section above contains a complete copyable `schemaVersion: 17` document. The bundled `content/` directory is also an executable reference and is validated in every repository test run. Copy the complete directory before customizing it; do not mount a partial overlay.
