...
const HIDDEN_MERCHANT_FIELDS = [
  ...
  'aggressionPenaltyApplied',
  'deathPenaltyApplied',
  'stockLossResolved',
  'commerceBonusApplied',
];
...
function proveMilestone(result, split, content) {
  ...
  assert(
    placed.length === 2 && placed.every((population) => population.encounterId === encounter.id),
    'demo did not force two eligible Lampwright merchant placements',
  );
  assert(
    new Set(placed.map((population) => population.floorId)).size === 2,
    'the two merchant placements must occupy separate floors',
  );

  const bought = record(result, 'before-buy').authoritativeEvents.find(
    (event) => event.type === 'trade.bought',
  );
  const sold = record(result, 'before-sell').authoritativeEvents.find(
    (event) => event.type === 'trade.sold',
  );
  const identified = record(result, 'before-identify').authoritativeEvents.find(
    (event) => event.type === 'trade.service-purchased',
  );
  ...
  assert(
    closed.some((event) => event.type === 'trade.closed' && event.completedCommerce) &&
  ...
  assert(
    openProjection.trade !== undefined,
    'the open trade must project observable commerce data',
  );
  ...
  for (const field of HIDDEN_MERCHANT_FIELDS) {
    assert(
      !projectionsJson.includes(`"${field}"`),
      `projection leaked hidden merchant field ${field}`,
    );
  }

  const warnings = result.records
    .flatMap((candidate) => candidate.authoritativeEvents)
    .filter((event) => event.type === 'merchant.departure-warning');
  assert(warnings.length > 0, 'departure warnings never crossed a threshold');
  assert(
    new Set(warnings.map((event) => event.populationId)).size === 2,
    'both merchants must cross departure warning thresholds',
  );

  const provokeRecord = record(result, 'before-provoke');
  const provoked = provokeRecord.authoritativeEvents.find(
    (event) => event.type === 'merchant.provoked',
  );
  const dropped = provokeRecord.authoritativeEvents.find(
    (event) => event.type === 'merchant.stock-dropped',
  );
  assert(
    provoked?.response === 'flee' && dropped,
    'the production attack must provoke a fleeing merchant',
  );
  const stockBefore =
    result.initial.items
      .filter(
        (item) =>
          ...
          item.location.populationId === provoked.populationId,
      )
      .reduce((total, item) => total + item.quantity, 0) -
    bought.quantity +
    sold.quantity;
  const expectedDrop = Math.min(
    stockBefore,
    Math.ceil(stockBefore * encounter.definition.stockDropFraction),
  );
  assert(
    dropped.units === expectedDrop,
    `stock loss must equal ceil(${stockBefore} * ${encounter.definition.stockDropFraction}) = ${expectedDrop}, got ${dropped.units}`,
  );
  assert(
    dropped.itemIds.every((itemId) =>
      result.state.items.some((item) => item.itemId === itemId && item.location.type === 'floor'),
    ),
    'dropped stock must persist on the floor',
  );

  const death = record(result, 'before-death').authoritativeEvents;
  assert(
    death.some((event) => event.type === 'merchant.died') &&
  ...
  assert(
    refusal.commandResult.status === 'invalid' &&
      refusal.commandResult.reason === 'merchant.refuses',
  ...
  const survivor = merchants(result.state).find(
    (population) => population.populationId === departed.populationId,
  );
  assert(
    survivor.lifecycle === 'departed' && survivor.floorId !== result.state.activeFloorId,
    'the departure must resolve on an inactive floor',
  );
  assert(
    !departure.authoritativeEvents.some(
      (event) => event.type === 'actor.turn.completed' && event.actorId === departed.actorId,
    ),
    'the off-floor departure must not grant the merchant an actor turn',
  );
  assert(
    !result.state.actors.some((actor) => actor.actorId === departed.actorId),
    'the departed merchant actor must be removed',
  );

  const final = merchants(result.state)
    .map((population) => population.lifecycle)
    .sort();
  assert(
    stableJson(final) === stableJson(['dead', 'departed']),
...
function printTranscript(result, content) {
  ...
  console.log(
    `two forced eligible Lampwright merchant placements; hero starts with the authored starting currency ${balance.startingCurrency}`,
  );
  ...
  console.log(
    'production attack provokes, drops exact ceil-fraction stock, and the merchant flees',
  );
...
async function main() {
  ...
  if (options.verify) {
    ...
    const candidateHashesPath = join(
      await mkdtemp(join(tmpdir(), 'merchant-demo-')),
      'merchant-demo-hashes.json',
    );
