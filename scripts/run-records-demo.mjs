...
async function verifyReviewedHashes(hashes) {
  ...
  assert(
    ...
    'reviewed run records demo hashes do not match',
  );
}
...
function proveMilestone(result, split) {
  ...
  const conclusion = record(result, 'before-death').authoritativeEvents.find(
    (event) => event.type === 'run.concluded',
  );
  assert(
    conclusion && conclusion.completionType === 'died',
    'the hero did not conclude with a died completion',
  );
  ...
  assert(
    finalization.record.heirloom.sourceItemId === 'item.run-records-demo.sword',
    'the heirloom did not roll the equipped weapon',
  );
  assert(
    finalization.record.score.total ===
  ...
  assert(
    standings.length === 1 && standings[0].hallRecordId === finalization.record.recordId,
  ...
  const transcript =
    ...
    stableJson(finalization.record) +
    stableJson(finalization.events) +
    stableJson(finalization.deltas.achievementGrants) +
    stableJson(standings) +
    stableJson(heart);
...
async function main() {
  ...
  if (options.verify) {
    ...
    const candidateHashesPath = join(
      await mkdtemp(join(tmpdir(), 'run-records-demo-')),
      'run-records-demo-hashes.json',
    );
