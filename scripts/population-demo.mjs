...
async function verifyReviewedHashes(hashes) {
  ...
  assert(
    stableJson(hashes) === stableJson(reviewed),
    'reviewed population demo hashes do not match',
  );
}
...
function proveMilestone(result, split, content) {
  ...
  for (const record of result.records.filter((candidate) =>
    causalAttackBoundaries.has(candidate.boundary),
  )) {
    assert(
      record.command.type === 'attack',
      `${record.boundary} was not caused by a production attack command`,
    );
    assert(
      record.authoritativeEvents.some(
        (event) => event.type === 'actor.died' && event.actorId === record.command.targetActorId,
      ),
      `${record.boundary} attack did not defeat its target`,
    );
  }
  ...
  const relays = result.records
    .flatMap((record) => record.authoritativeEvents)
    .filter((event) => event.type === 'group.awareness-shared');
  assert(
    relays.length > 0 &&
      new Set(relays.map((event) => event.actorId)).size < group.roleMembership.length,
  ...
  assert(
    swarmDefinition?.kind === 'encounter' && swarmDefinition.model === 'swarm',
    'swarm content missing',
  );
  assert(
    swarm.livingMemberIds.length <= swarmDefinition.definition.maximumLivingMembers,
    'swarm exceeded member cap',
  );
  assert(
    swarm.shutdownState === swarmDefinition.definition.sourceDestructionResponse,
    'swarm did not persist its configured containment response',
  );
  assert(
    result.records[0].projection.actors.some((actor) => actor.actorId === swarm.sourceActorId),
    'swarm source was not visibly projected',
  );
  assert(
    boss?.crossedPhaseIds.length > 1 && boss.rewardCreated,
    'boss phase or reward demonstration missing',
  );
  ...
  const championSlot = result.initial.floors
    .flatMap((floor) => floor.placementSlots)
    .find((slot) => slot.slotId === 'slot.champion');
  assert(
    championActor?.populationPresentation?.name.endsWith(", the Deep's Champion") === true &&
      championSlot?.required === false,
    'named Champion is not in an optional bypassable arena slot',
  );
  assert(
    echoActor?.populationPresentation?.name === 'Echo of Bryn' &&
      echoActor.maxHealth < championActor.maxHealth,
    'Echo was not named and weaker than the Champion',
  );
  const arenaFloor = result.initial.floors.find((floor) => floor.floorId === championActor.floorId);
  const livingPopulationActors = result.initial.actors.filter(
    (actor) =>
      actor.floorId === arenaFloor.floorId && actor.health > 0 && actor.populationId !== null,
  );
  assert(
    preservesRequiredRoutes({
      width: arenaFloor.width,
      height: arenaFloor.height,
      tiles: arenaFloor.tiles,
      requiredPoints: [arenaFloor.stairUp, arenaFloor.stairDown],
      blockedPoints: livingPopulationActors,
    }),
    'living population placements blocked the mandatory stair route',
  );
  const blockedTiles = [...arenaFloor.tiles];
  for (const actor of livingPopulationActors)
    blockedTiles[actor.y * arenaFloor.width + actor.x] = 0;
  const mandatoryRoute = analyzeConnectivity({
    width: arenaFloor.width,
    height: arenaFloor.height,
    tiles: blockedTiles,
    start: arenaFloor.stairUp,
    target: arenaFloor.stairDown,
  }).route;
  assert(
    mandatoryRoute.length > 0 && mandatoryRoute.every((point) => point.x < 12),
  ...
  for (let seed = 1; seed < 10_000 && !rejectedByNormalGate; seed += 1) {
    const decisions = createEncounterRunDecisions({
      encounters,
      state: [
        seed >>> 0,
        (seed ^ 0x9e3779b9) >>> 0,
        Math.imul(seed, 0x85ebca6b) >>> 0,
        Math.imul(seed ^ 0xc2b2ae35, 0x27d4eb2f) >>> 0,
      ],
    }).decisions;
    rejectedByNormalGate =
      decisions.find((decision) => decision.encounterId === bossEncounter.id)?.eligible === false;
  }
  assert(
    rejectedByNormalGate &&
  ...
  assert(
    result.state.items.some(
      (item) =>
        item.itemId.includes(`echo-loot.${echo.populationId}`) && item.heirloom === undefined,
    ),
    'Echo ordinary loot missing',
  );
...
async function main() {
  ...
  const echoActor = continuous.initial.actors.find(
    (actor) =>
      actor.populationId ===
      continuous.initial.populations.find((population) => population.model === 'echo')
        ?.populationId,
  );
  console.log('relay-limited leader group');
  console.log(
    stableJson(authoritativeEvents.filter((event) => event.type === 'group.awareness-shared')),
  );
  ...
  console.log(
    stableJson(authoritativeEvents.filter((event) => event.type.startsWith('champion.'))),
  );
  console.log(
    'normal production gate rejected; forced optional arena placed; stair endpoints remain connected while Champion and Echo are blocked',
  );
  console.log(
    `${echoActor?.populationPresentation?.displayName ?? 'Echo of Bryn'} and ordinary loot`,
  );
