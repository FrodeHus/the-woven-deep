...
describe('bundled content', () => {
  ...
  it('ships the exact Lampwright merchant contract', async () => {
    ...
    expect(entries.get('encounter.travelling-lampwright')).toMatchObject({
      ...
      definition: {
        ...
        maximumLifetime: 5000,
        departureWarningThresholds: [1000, 500, 100],
        aggressionResponse: 'flee',
        commerceReputationDelta: 25,
        aggressionReputationDelta: -300,
        deathReputationDelta: -200,
  ...
  it('ships run-record content: achievements, score coefficients, and monster threat', async () => {
    ...
    expect(pack.schemaVersion).toBe(14);
    const entries = new Map(pack.entries.map((entry) => [entry.id, entry]));
  ...
  it('reports every foundational gameplay category', async () => {
    ...
    expect(pack.generationReport.foundationalCategories).toEqual([
      'defense',
      'food',
      'healing',
      'identification',
  ...
  it('ships coherent early loot, identification pools, and distinct light fuel models', async () => {
    ...
    expect(
      [...accessibleTags]
        .filter((tag) => ['food', 'healing', 'light', 'offense', 'defense'].includes(tag))
        .sort(),
    ...
    expect(
      potionPool?.kind === 'identification-pool'
        ? potionPool.verbs.length * potionPool.nouns.length
        : 0,
    ).toBeGreaterThan(2);
  ...
  it('revalidates condition references in a stored compiled pack', async () => {
    ...
    expect(() => validateCompiledContentPack(corrupted)).toThrow(
