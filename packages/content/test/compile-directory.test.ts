...
const compactBalance =
  '{kind: balance, startingCurrency: 40, id: balance.core, name: Core, tags: [core], readinessThreshold: 100, normalActionCost: 100, speedMinimum: 25, speedMaximum: 400, energyMinimum: -10000, energyMaximum: 10000, attributeMinimum: 0, attributeMaximum: 30, hungerMaximum: 5000, hungerThresholds: {hungry: 3000, weak: 1000, starving: 0}, starvationInterval: 500, starvationDamage: 1, recoveryInterval: 500, recoveryAmount: 1, weaveRegenAmount: 2, restMaximumDuration: 5000, recoveryByHungerStage: {sated: 100, hungry: 50, weak: 0, starving: 0}, hungerStageModifiers: {sated: {}, hungry: {}, weak: {}, starving: {}}, formulas: {health: {base: 8, vitality: 2}}, actionCosts: {action.move: 100}, score: {depthCoefficient: 100, bossDefeatCoefficient: 250, threatCoefficient: 5, discoveryCoefficient: 25, completionBonus: {died: 0, refused: 400, became-heart: 800, broke-cycle: 1500}, turnEfficiencyBudget: 500, turnEfficiencyDecayInterval: 200}, pointBuy: {budget: 1, costs: [{value: 0, cost: 0}, {value: 1, cost: 0}, {value: 2, cost: 0}, {value: 3, cost: 0}, {value: 4, cost: 0}, {value: 5, cost: 0}, {value: 6, cost: 0}, {value: 7, cost: 0}, {value: 8, cost: 0}, {value: 9, cost: 0}, {value: 10, cost: 0}, {value: 11, cost: 0}, {value: 12, cost: 0}, {value: 13, cost: 0}, {value: 14, cost: 0}, {value: 15, cost: 0}, {value: 16, cost: 0}, {value: 17, cost: 0}, {value: 18, cost: 0}, {value: 19, cost: 0}, {value: 20, cost: 0}, {value: 21, cost: 0}, {value: 22, cost: 0}, {value: 23, cost: 0}, {value: 24, cost: 0}, {value: 25, cost: 0}, {value: 26, cost: 0}, {value: 27, cost: 0}, {value: 28, cost: 0}, {value: 29, cost: 0}, {value: 30, cost: 0}]}, restockMilestones: [5, 10, 15, 20], house: {baseCapacity: 6, strongboxIncrement: 4}, encounterDensity: {openCellsPerEncounter: 2000}, fragmentSpawnRollDenominator: 40, floorLoot: {scatterCount: {minimum: 2, maximum: 4}, chestCount: {minimum: 0, maximum: 2}, lockedChestPercent: 50, lockedDoorPercent: 15, minimumAnchorDistance: 8, minimumSpreadDistance: 6, depthBands: {shallowMaxDepth: 6, midMaxDepth: 13}, chestLockDifficulty: {shallow: 10, mid: 13, deep: 16}}}';
const compactTimedCondition =
  '{kind: condition, id: condition.stunned, name: Stunned, description: Cannot act, tags: [control], color: "#d8c46a", duration: {mode: timed, default: 100, maximum: 500}, stacking: {mode: refresh, maximumStacks: 1}, modifiersPerStack: {defense: -2}, traits: [condition-trait.incapacitated]}';
const compactPermanentCondition =
  '{kind: condition, id: condition.warded, name: Warded, description: Remains until removed, tags: [beneficial], color: "#80b8ff", duration: {mode: permanent, default: null, maximum: null}, stacking: {mode: refresh, maximumStacks: 1}, modifiersPerStack: {}, traits: []}';
...
const compactNpc =
  '{kind: npc, id: npc.lampwright, name: Lampwright, tags: [], glyph: L, color: "#ffd166", factionId: npc-faction.lampwrights, attributes: {might: 8, agility: 9, vitality: 10, wits: 12, resolve: 11}, health: 20, speed: 100, perception: 12, accuracy: 8, defense: 10, damage: {count: 1, sides: 4, bonus: 0}, armor: 1, resistances: {physical: 0, fire: 0, cold: 0, lightning: 0, poison: 0, arcane: 0}, disposition: neutral, behaviorId: npc-behavior.travelling-merchant, behaviorParameters: {}, selfPreservationThresholdBps: 3500}';
const compactStock =
  '{kind: loot-table, id: loot-table.stock, name: Stock, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
...
function backgroundEntry(extraItemContentId: string | null): string {
  const extraItems =
    extraItemContentId === null ? '[]' : `[{contentId: ${extraItemContentId}, quantity: 1}]`;
...
describe('compileContentDirectory', () => {
  ...
  it('resolves a monster lootTableId against a known loot table', async () => {
    const lootyMonster = compactMonster.replace(
      ', threat: 2, rarity: common}',
      ', threat: 2, rarity: common, lootTableId: loot-table.stock, dropChance: 0.25}',
    );
    const root = await fixture({
      'content.yaml': contentFile(lootyMonster, compactItem, compactStock, compactVault),
    });
    const pack = await compileContentDirectory({ rootDir: root });
    expect(pack.entries.find((entry) => entry.id === 'monster.rat')).toMatchObject({
      lootTableId: 'loot-table.stock',
      dropChance: 0.25,
    });
  });
  ...
  it('rejects a monster lootTableId that does not resolve to a loot table', async () => {
    const lootyMonster = compactMonster.replace(
      ', threat: 2, rarity: common}',
      ', threat: 2, rarity: common, lootTableId: loot-table.missing, dropChance: 1}',
    );
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /unknown loot-table reference loot-table\.missing/,
    );
  });
  ...
  it('rejects a monster lootTableId that resolves to a non-loot-table entry', async () => {
    const lootyMonster = compactMonster.replace(
      ', threat: 2, rarity: common}',
      ', threat: 2, rarity: common, lootTableId: item.lantern, dropChance: 1}',
    );
    const root = await fixture({ 'content.yaml': contentFile(lootyMonster, compactItem) });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /loot-table reference item\.lantern resolves to item/,
    );
  });
  ...
  it('requires nonmerchant discovery protection fields while defaulting them for merchants', async () => {
    const root = await fixture({
      'content.yaml': contentFile(
        ...
        compactItem,
        compactVault,
        compactIndividualEncounter.replace(
          ', discoveryProtectionIncrement: 0, discoveryProtectionCap: 1',
          '',
        ),
      ),
    });
  ...
  it('requires a merchant service to be enabled by every targeted faction tier', async () => {
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /service merchant-service\.identify is not enabled for faction tier wary/,
    );
  });
  ...
  it('preserves authored tier indices in overlap diagnostics', async () => {
    ...
    await expectCompileIssues(compileContentDirectory({ rootDir: root }), [
      {
        file: 'content.yaml',
        path: '$.entries.npc-faction.lampwrights.tiers.0',
  ...
  it.each([
    [
      'duplicate tier ID',
      compactFaction.replace('tierId: neutral', 'tierId: wary'),
      /duplicate reputation tier wary/,
    ],
    [
    ...
    [
      'zero faction multiplier',
      compactFaction.replace('purchasePriceBps: 12000', 'purchasePriceBps: 0'),
      /purchasePriceBps/,
    ],
    [
      'negative merchant multiplier',
      compactMerchant.replace('merchantSaleBps: 12000', 'merchantSaleBps: -1'),
      /merchantSaleBps/,
    ],
    [
      'duplicate warning',
      compactMerchant.replace('[1000, 500, 100]', '[1000, 1000, 100]'),
      /warning thresholds must be unique/,
    ],
    [
      'out-of-range warning',
      compactMerchant.replace('[1000, 500, 100]', '[3000, 500, 100]'),
      /below minimum lifetime/,
    ],
    [
      'stock inversion',
      compactMerchant.replace(
        'minimumStockRolls: 1, maximumStockRolls: 2',
        'minimumStockRolls: 3, maximumStockRolls: 2',
      ),
      /maximum stock rolls/,
    ],
    [
      'lifetime inversion',
      compactMerchant.replace(
        'minimumLifetime: 3000, maximumLifetime: 5000',
        'minimumLifetime: 6000, maximumLifetime: 5000',
      ),
      /maximum lifetime/,
    ],
  ])(
    'rejects complete merchant semantic matrix case: %s',
    async (_label, replacement, diagnostic) => {
      const root = await fixture({
        'content.yaml': contentFile(
          compactMonster,
          compactItem,
  ...
  it('rejects duplicate merchant service offer IDs', async () => {
    const duplicateService = compactMerchant.replace(
      'services: [',
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /duplicate merchant service merchant-service\.identify/,
    );
  });

  it('walks nested merchant stock tables before accepting the pack', async () => {
    const nested =
      '{kind: loot-table, id: loot-table.nested-stock, name: Nested, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const parent = compactStock.replace(
      'contentId: item.lantern, lootTableId: null',
      'contentId: null, lootTableId: loot-table.nested-stock',
    );
    const root = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactItem.replace('price: 4', 'price: 0'),
        compactVault,
        compactFaction,
        compactNpc,
        parent,
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /merchant stock item item\.lantern requires positive price/,
    );
  });

  it('rejects a boss-unique item reachable from merchant stock', async () => {
    const ordinary = compactItem
      .replace('item.lantern', 'item.ordinary')
      .replace('name: Lantern', 'name: Ordinary');
    const bossLoot =
      '{kind: loot-table, id: loot-table.boss, name: Boss, tags: [], rolls: 1, choices: [{contentId: item.ordinary, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const boss =
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /merchant stock item item\.lantern is guaranteed unique/,
    );
  });

  it.each(['heirloom', 'quest', 'objective', 'nontransferable'])(
    'rejects merchant stock tagged %s',
    async (tag) => {
      const item = compactItem.replace(
        'tags: [defense, food, healing, identification, light, offense]',
        `tags: [${tag}]`,
      );
      const root = await fixture({
        'content.yaml': contentFile(
  ...
  it.each([
    [
      'missing faction',
      compactNpc.replace('npc-faction.lampwrights', 'npc-faction.missing'),
      /unknown npc-faction reference/,
    ],
    [
      'faction gap',
      compactFaction.replace('minimum: 0, maximum: 100', 'minimum: 1, maximum: 100'),
      /reputation tiers must cover every value/,
    ],
    [
      'starting reputation outside bounds',
      compactFaction.replace('startingReputation: 0', 'startingReputation: 101'),
      /starting reputation must be within faction bounds/,
    ],
    [
      'missing NPC',
      compactMerchant.replace('npc.lampwright', 'npc.missing'),
      /unknown npc reference/,
    ],
    [
      'missing stock',
      compactMerchant.replace('loot-table.stock', 'loot-table.missing'),
      /unknown loot-table reference/,
    ],
    [
      'warning order',
      compactMerchant.replace('[1000, 500, 100]', '[500, 1000, 100]'),
      /warning thresholds must be unique/,
    ],
    [
      'service uses',
      compactMerchant.replace('minimumUses: 1, maximumUses: 2', 'minimumUses: 2, maximumUses: 1'),
      /maximum service uses/,
    ],
    [
      'unknown service tier',
      compactMerchant.replace('tierIds: [neutral]', 'tierIds: [trusted]'),
      /absent from NPC faction/,
    ],
  ...
  it('rejects merchant stock with a zero price or reserved tag', async () => {
    for (const item of [
      ...
      await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
        /positive price|reserved quest tag/,
      );
    }
  });
  ...
  it('rejects a playable class with only one kit', async () => {
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, playableClass(1)),
    });
  ...
  it('rejects a class kit that sets enabled on a non-light equipped item', async () => {
    const sword = compactItem
      ...
      .replace('name: Lantern', 'name: Sword')
      ...
      .replace(
        'light: {color: [255, 200, 100], radius: 6, strength: 180, fuelCapacity: 1000, fuelPerTime: 1, warningThresholds: [100], fuelTags: [lamp-oil]}',
        'light: null',
      );
    const kit1 =
  ...
  it('rejects a class kit that equips a missing item', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, missingItemClass),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /unknown item reference item\.missing/,
    );
  });
  ...
  it('rejects a class kit that equips an item in a slot it does not allow', async () => {
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /cannot be equipped in slot main-hand/,
    );
  });
  ...
  it('rejects a class kit that equips a two-handed item alongside something in its reserved slot', async () => {
    const twoHandedBow = compactItem
      .replace('item.lantern', 'item.bow')
      .replace('name: Lantern', 'name: Bow')
      .replace(
        'equipment: {slots: [off-hand], handedness: one-handed, reservedSlots: []}',
        'equipment: {slots: [main-hand], handedness: two-handed, reservedSlots: [off-hand]}',
      );
    const conflictingKit =
      '{kitId: first, name: First, equipped: ' +
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /kit first.*reserved slot off-hand/i,
    );
  });
  ...
  it('rejects duplicate IDs in code-unit path order', async () => {
    const root = await fixture({
      'z.yaml': contentFile(compactMonster, compactItem, compactVault),
      'ä.yaml': contentFile(
        compactMonster.replace('name: Rat', 'name: Rat Two').replace('health: 4', 'health: 5'),
      ),
    });
  ...
  it('accepts registered patrol waypoints and rejects an empty patrol', async () => {
    const patrol = compactMonster
      ...
      .replace(
        'behaviorParameters: {}',
        'behaviorParameters: {waypoints: [{x: 1, y: 2}, {x: 3, y: 4}]}',
      );
    const validRoot = await fixture({
      'content.yaml': contentFile(patrol, compactItem, compactVault),
    });
    await expect(compileContentDirectory({ rootDir: validRoot })).resolves.toBeDefined();

    const invalidRoot = await fixture({
      'content.yaml': contentFile(
        patrol.replace('waypoints: [{x: 1, y: 2}, {x: 3, y: 4}]', 'waypoints: []'),
        compactItem,
        compactVault,
      ),
    });
    await expect(compileContentDirectory({ rootDir: invalidRoot })).rejects.toThrow(
      /waypoints|too small/i,
    );
  });
  ...
  it('rejects inconsistent equipment handedness', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, invalidItem, compactVault),
    });

    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /two-handed equipment must use the main-hand slot/i,
    );
  });
  ...
  it('rejects unregistered effects and invalid registered effect parameters', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, unknownEffect, compactVault),
    });

    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /unregistered effect effect\.unknown|expected number to be >0/i,
    );
  });

  it.each([
    [
      "a typo'd effect parameter key",
      'effects: [{effectId: effect.heal, parameters: {dyce: {count: 1, sides: 4, bonus: 0}}, requiresLivingTarget: false}]',
    ],
    [
      'a missing required effect parameter',
      'effects: [{effectId: effect.heal, parameters: {}, requiresLivingTarget: false}]',
    ],
    [
      'an ill-typed effect parameter value',
      'effects: [{effectId: effect.reveal, parameters: {radius: far}, requiresLivingTarget: false}]',
    ],
    ...
    expect(
      issues.every((issue) => issue.path.startsWith('$.entries.item.lantern.effects.0.parameters')),
    ).toBe(true);
  ...
  it("rejects a typo'd behavior parameter key at compile with a parameter-scoped error", async () => {
    ...
    expect(caught).toBeInstanceOf(ContentCompileError);
    expect(
      (caught as ContentCompileError).issues.some((issue) =>
        issue.path.startsWith('$.entries.monster.rat.behavior'),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    [
      ...
      'condition.missing',
      '',
      /unknown condition reference condition\.missing/,
    ],
    [
      'wrong content kind',
      'item.lantern',
      '',
      /condition reference item\.lantern resolves to item/,
    ],
    [
      'duration above maximum',
      'condition.stunned',
      ', duration: 501',
      /duration 501 exceeds maximum 500/,
    ],
  ...
  it('accepts omitted timed and permanent durations but rejects a permanent override with an exact path', async () => {
    const omitted = compactItem.replace(
      'effects: []',
      'effects: [{effectId: effect.condition.apply, parameters: {conditionId: condition.warded}, requiresLivingTarget: true}]',
    );
    const validRoot = await fixture({
  ...
  it('rejects nested loot-table cycles', async () => {
    const first =
      '{kind: loot-table, id: loot-table.first, name: First, tags: [], rolls: 1, choices: [{contentId: null, lootTableId: loot-table.second, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const second =
      '{kind: loot-table, id: loot-table.second, name: Second, tags: [], rolls: 1, choices: [{contentId: null, lootTableId: loot-table.first, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, first, second),
    });

    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /loot-table cycle detected/i,
    );
  });
  ...
  it('bounds checked loot weights, rolls, quantities, and recursive worst-case units', async () => {
    ...
    for (const invalid of [
      ...
      const root = await fixture({
        'content.yaml': contentFile(compactMonster, boundedItem, compactVault, invalid),
      });
      await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
        /loot.*(weight.*2\^32|rolls.*256|quantity.*256)/i,
      );
    }
    ...
    const nested = await fixture({
      'content.yaml': contentFile(compactMonster, boundedItem, compactVault, child, nestedBoundary),
    });
    ...
    const excessive = await fixture({
      'content.yaml': contentFile(
        ...
        compactVault,
        child,
        nestedBoundary.replace(
          'minimumQuantity: 1, maximumQuantity: 1',
          'minimumQuantity: 2, maximumQuantity: 2',
        ),
      ),
    });
    await expect(compileContentDirectory({ rootDir: excessive })).rejects.toThrow(
      /loot.*worst-case.*4096/i,
    );
  });
  ...
  it('requires exactly one balance entry', async () => {
    const root = await fixture({
      'content.yaml': contentFile(
        ...
        compactItem,
        compactVault,
        compactBalance.replace('balance.core', 'balance.alternate'),
      ),
    });

    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /expected exactly one balance entry; found 2/i,
    );
  });
  ...
  it('requires a positive rest maximum duration', async () => {
    const root = await fixture({
      'balance.yaml': contentFile(
        compactBalance.replace('restMaximumDuration: 5000', 'restMaximumDuration: 0'),
      ),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /restMaximumDuration/i,
    );
  });
  ...
  it('rejects a pack missing any foundational gameplay category', async () => {
    const root = await fixture({
      'content.yaml': contentFile(
        compactMonster.replace('identification, ', ''),
        compactItem.replace('identification, ', ''),
        compactVault,
      ),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /missing foundational category identification/i,
    );
  });

  it('loads unidentified presentation from a separate pool instead of item definitions', async () => {
    const potion = (id: string) =>
      compactItem
        .replace('item.lantern', id)
    ...
    const pool =
      '{kind: identification-pool, id: identification-pool.potions, name: Potion appearances, tags: [], category: potion, verbs: [Clouded, Smoking, Whispering], nouns: [vial, flask, phial], visuals: [{id: visual.teal-glass, glyph: "!", color: "#4faaa2"}, {id: visual.amber-glass, glyph: "¡", color: "#c58745"}]}';
    const root = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactVault,
        pool,
        potion('item.potion-one'),
        potion('item.potion-two'),
      ),
    });
  ...
  it('requires enough unique verb-noun names for every item using a pool', async () => {
    ...
    const pool =
      '{kind: identification-pool, id: identification-pool.potions, name: Potion names, tags: [], category: potion, verbs: [Bubbling], nouns: [vial], visuals: [{id: visual.blue-glass, glyph: "!", color: "#4466aa"}]}';
    const root = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactVault,
  ...
  it('accepts an unbanded loot choice and a valid depth band, rejecting an inverted band', async () => {
    const banded =
      '{kind: loot-table, id: loot-table.banded, name: Banded, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1, minDepth: 5, maxDepth: 10}]}';
    const validRoot = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, banded),
    });
    await expect(compileContentDirectory({ rootDir: validRoot })).resolves.toBeDefined();

    const inverted = banded.replace('minDepth: 5, maxDepth: 10', 'minDepth: 10, maxDepth: 5');
    const invalidRoot = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, inverted),
    });
    await expect(compileContentDirectory({ rootDir: invalidRoot })).rejects.toThrow(
      /minDepth.*maxDepth|maxDepth.*minDepth/i,
    );
  });

  it('requires direct loot choices to reference items', async () => {
    const table =
      '{kind: loot-table, id: loot-table.invalid, name: Invalid, tags: [], rolls: 1, choices: [{contentId: monster.rat, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, table),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /content reference monster\.rat resolves to monster; expected item/i,
    );
  });

  it('requires ranged weapon ammunition tags to have matching ammunition', async () => {
    const weapon = compactItem
      .replace('item.lantern', 'item.bow')
      .replace('category: light', 'category: weapon')
      .replace(
        'light: {color: [255, 200, 100], radius: 6, strength: 180, fuelCapacity: 1000, fuelPerTime: 1, warningThresholds: [100], fuelTags: [lamp-oil]}',
      ...
      .replace(
        'combat: null',
        'combat: {accuracy: 0, defense: 0, armor: 0, damage: {count: 1, sides: 4, bonus: 0}, range: 6, ammunitionTag: bolt}',
      );
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, weapon, compactVault),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /ammunition tag bolt has no matching ammunition item/i,
    );
  });

  it('rejects unknown balance action-cost identifiers', async () => {
    const root = await fixture({
      'balance.yaml': contentFile(compactBalance.replace('action.move', 'action.teleport')),
      'content.yaml': contentFile(compactMonster, compactItem, compactVault),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /unregistered action cost action\.teleport/i,
    );
  });

  it('enforces balance ordering and monster speed bounds', async () => {
    const invalidBalance = compactBalance
      .replace('speedMinimum: 25', 'speedMinimum: 200')
      .replace('speedMaximum: 400', 'speedMaximum: 100');
    const fastMonster = compactMonster.replace('speed: 110', 'speed: 401');
    const root = await fixture({
      'balance.yaml': contentFile(invalidBalance),
      'content.yaml': contentFile(fastMonster, compactItem, compactVault),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /speedMinimum.*speedMaximum|speed 401.*bounds/i,
    );
  });
  ...
  it.each([
    ...
    [
      'unsafe depth',
      compactVault.replace('maxDepth: 5', `maxDepth: ${Number.MAX_SAFE_INTEGER + 1}`),
    ],
    ...
    [
      'unsafe placement limit',
      compactVault.replace('maxPerFloor: 1', `maxPerFloor: ${Number.MAX_SAFE_INTEGER + 1}`),
    ],
    ...
    [
      ...
      compactVault.replace(
        ...
        'light: {idSuffix: amber, glyph: "*", presentationToken: fixture.lamp, color: [255, 180, 64], radius: 0, strength: 180}',
      ),
    ],
    [
      'light color',
      compactVault.replace(
        'slot: {id: monster-main, kind: monster, required: true, tags: [guard]}',
        'light: {idSuffix: amber, glyph: "*", presentationToken: fixture.lamp, color: [256, 180, 64], radius: 6, strength: 180}',
      ),
    ],
    [
      'light strength',
      compactVault.replace(
        'slot: {id: monster-main, kind: monster, required: true, tags: [guard]}',
        'light: {idSuffix: amber, glyph: "*", presentationToken: fixture.lamp, color: [255, 180, 64], radius: 6, strength: 256}',
      ),
    ],
    ['duplicate rotations', compactVault.replace('[0, 180]', '[0, 0]')],
    ['unsorted rotations', compactVault.replace('[0, 180]', '[180, 0]')],
    [
      'multi-code-point legend key',
      compactVault.replace('legend: {', 'legend: {xy: {terrain: floor}, '),
    ],
    [
      'duplicate fixture suffix',
      compactVault
        .replace('"#+m.#"', '"#+mm#"')
        .replace(
          'slot: {id: monster-main, kind: monster, required: true, tags: [guard]}',
          'light: {idSuffix: amber, glyph: "*", presentationToken: fixture.lamp, color: [255, 180, 64], radius: 6, strength: 180}',
        ),
    ],
    [
      'declared-width overflow',
      compactVault
        .replace('["#####", "#+m.#", "#####"]', `["+${'.'.repeat(160)}"]`)
        .replace('legend: {"#": {terrain: wall}, ', 'legend: {'),
    ],
    [
      'declared-height overflow',
      compactVault
        .replace(
          '["#####", "#+m.#", "#####"]',
          `[${Array.from({ length: 101 }, () => '"+"').join(', ')}]`,
        )
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, invalidVault),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toBeInstanceOf(
      ContentCompileError,
    );
  });
  ...
  it('parses a vault item slot that names a loot table', async () => {
    ...
    expect(vault).toMatchObject({
      legend: { m: { slot: { lootTableId: 'loot-table.stock', contentId: null } } },
    });
  });
  ...
  it('parses a vault item slot that names a fixed item', async () => {
    ...
    const vault = pack.entries.find((entry) => entry.id === 'vault.test-room');
    expect(vault).toMatchObject({
      legend: { m: { slot: { lootTableId: null, contentId: 'item.lantern' } } },
    });
  });
  ...
  it('includes vault IDs and values in global uniqueness and content hashing', async () => {
    ...
    const changedRoot = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactItem,
        compactVault.replace('weight: 10', 'weight: 11'),
      ),
    });
    const originalRoot = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault),
    });
  ...
  it('resolves encounter monster references and quantity bounds', async () => {
    const root = await fixture({
      'content.yaml': contentFile(
        ...
        compactVault,
        compactIndividualEncounter.replace(
          'monster.rat, minimumQuantity: 1',
          'monster.missing, minimumQuantity: 1',
        ),
      ),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /unknown monster reference monster\.missing/i,
    );
  });

  it('accepts encounter selection weight 2^32 exactly and rejects a checked aggregate above it', async () => {
    const weighted = (source: string, id: string, weight: number) =>
      source.replace('encounter.rat', id).replace('weight: 1, rarity', `weight: ${weight}, rarity`);
    const atBoundary = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactItem,
        compactVault,
    ...
    await expect(compileContentDirectory({ rootDir: above })).rejects.toThrow(
      /encounter weight.*2\^32/i,
    );
  });

  it('caps individual and aggregate group quantities at the runtime-safe encounter limit', async () => {
    const individualBoundary = compactIndividualEncounter.replace(
      'maximumQuantity: 2',
      'maximumQuantity: 1024',
    );
    const boundaryRoot = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, individualBoundary),
    });
    await expect(compileContentDirectory({ rootDir: boundaryRoot })).resolves.toBeDefined();
    const individualAbove = await fixture({
      'content.yaml': contentFile(
        compactMonster,
        compactItem,
        compactVault,
    ...
    await expect(compileContentDirectory({ rootDir: individualAbove })).rejects.toThrow(
      /quantity.*runtime-safe.*1024/i,
    );

    const group = (secondMaximum: number) =>
    ...
    const groupBoundary = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, group(512)),
    });
    await expect(compileContentDirectory({ rootDir: groupBoundary })).resolves.toBeDefined();
    const groupAbove = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, group(513)),
    });
    await expect(compileContentDirectory({ rootDir: groupAbove })).rejects.toThrow(
      /group.*quantity.*1024/i,
    );
  });
  ...
  it('checks spawn-role weights and caps swarm allocation quantities', async () => {
    ...
    const swarm = (
      secondWeight: number,
      maximumSpawnQuantity = 256,
      maximumLivingChildren = 1023,
    ) =>
    ...
    const boundary = await fixture({
      'content.yaml': contentFile(source, compactItem, compactVault, swarm(2147483648)),
    });
    await expect(compileContentDirectory({ rootDir: boundary })).resolves.toBeDefined();
    for (const invalid of [
      swarm(2147483649),
      swarm(2147483648, 257),
      swarm(2147483648, 256, 1024),
    ]) {
      const root = await fixture({
        'content.yaml': contentFile(source, compactItem, compactVault, invalid),
      });
      await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
        /spawn-role weight.*2\^32|spawn quantity.*256|living children.*1023/i,
      );
    }
  });
  ...
  it('requires supernatural group collapse and a declared leader role', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, group),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /leader role captain|collapse requires supernaturalBond/i,
    );
  });
  ...
  it('validates swarm source tags and cap relationships', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, swarm),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /swarm-source|maximum living members|maximum floor actors/i,
    );
  });
  ...
  it('validates boss uniqueness, phase order, and reward kinds', async () => {
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, boss),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /maximumInstancesPerRun 1|strictly descending|resolves to monster|unknown loot-table/i,
    );
  });

  it('rejects a boss enhanced-loot graph that reaches its guaranteed unique item', async () => {
    const nested =
      '{kind: loot-table, id: loot-table.nested, name: Nested, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const loot =
      '{kind: loot-table, id: loot-table.boss, name: Boss loot, tags: [], rolls: 1, choices: [{contentId: null, lootTableId: loot-table.nested, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const boss =
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, compactVault, nested, loot, boss),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /enhanced loot.*guaranteed.*unique/i,
    );
  });
  ...
  it('rejects any ordinary loot graph that reaches another boss guaranteed unique item', async () => {
    ...
    const lootA =
      '{kind: loot-table, id: loot-table.boss-a, name: Boss A loot, tags: [], rolls: 1, choices: [{contentId: item.unique-b, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const lootB =
      '{kind: loot-table, id: loot-table.boss-b, name: Boss B loot, tags: [], rolls: 1, choices: [{contentId: item.ordinary, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const boss = (id: string, unique: string, loot: string) =>
    ...
    const root = await fixture({
      'content.yaml': contentFile(
        ...
        lootA,
        lootB,
        boss('encounter.boss-a', 'item.lantern', 'loot-table.boss-a'),
        boss('encounter.boss-b', 'item.unique-b', 'loot-table.boss-b'),
      ),
    });
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /boss-unique.*ordinary loot/i,
    );
  });
  ...
  it.each([
    ...
    const ordinary = compactItem.replace('item.lantern', 'item.ordinary');
    const loot =
      '{kind: loot-table, id: loot-table.boss, name: Boss loot, tags: [], rolls: 1, choices: [{contentId: item.ordinary, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    ...
    const root = await fixture({
      'content.yaml': contentFile(compactMonster, compactItem, ordinary, compactVault, loot, boss),
    });
    await expectCompileIssues(compileContentDirectory({ rootDir: root }), [
      {
        file: 'content.yaml',
  ...
  it('validates Champion and Echo template references and weaker limits', async () => {
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /Echo ability limit|loot-table reference item\.lantern resolves to item/i,
    );
  });
  ...
  it('requires every enabled Echo ability cap to be strictly below the Champion cap', async () => {
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /strictly below Champion ability limit/i,
    );
  });

  it('rejects enabled Echoes when current Champion combat minima cannot be strictly weakened', async () => {
    const weakMonster = compactMonster
      .replace('health: 4', 'health: 1')
      .replace('accuracy: 1', 'accuracy: 0')
      .replace('defense: 10', 'defense: 0');
    const loot =
      '{kind: loot-table, id: loot-table.echo, name: Echo loot, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const template =
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /strict minimum|strictly weaken|Echoes require Champion/i,
    );
  });

  it('rejects a Champion Echo loot graph that can reach a guaranteed boss-unique item', async () => {
    const leaf =
      '{kind: loot-table, id: loot-table.echo-leaf, name: Echo leaf, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const echo =
      '{kind: loot-table, id: loot-table.echo, name: Echo loot, tags: [], rolls: 1, choices: [{contentId: null, lootTableId: loot-table.echo-leaf, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const bossLoot =
      '{kind: loot-table, id: loot-table.boss, name: Boss loot, tags: [], rolls: 1, choices: [{contentId: item.lantern, lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1}]}';
    const boss =
      '{kind: encounter, id: encounter.boss, name: Boss, tags: [], model: boss, minDepth: 1, maxDepth: 5, environmentTags: [], requiredVaultTags: [], weight: 1, rarity: legendary, runAppearanceChance: 1, discoveryProtectionIncrement: 0, discoveryProtectionCap: 1, maximumInstancesPerRun: 1, placement: {minimumStairDistance: 1, minimumObjectiveDistance: 1, maximumMemberDistance: 0, allowedTerrainTags: [floor], requiresVaultSlot: false, failureMode: optional}, intentPresentation: {visible: true}, definition: {monsterId: monster.rat, phases: [], recoveryPerWorldTime: 0, recoveryCapPercent: 0, uniqueItemId: item.lantern, enhancedLootTableId: loot-table.boss, vaultTags: []}}';
    const template =
    ...
    await expect(compileContentDirectory({ rootDir: root })).rejects.toThrow(
      /Echo loot.*guaranteed.*unique|boss-unique/i,
    );
  });
