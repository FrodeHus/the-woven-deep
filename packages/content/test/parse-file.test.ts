...
describe('parseContentFile', () => {
  it('publishes and parses strict schema-v4 NPC content', () => {
    ...
    const validNpcYaml = `schemaVersion: 11
entries:
  - kind: npc
...
`;
  ...
  it('rejects source schema v2 with a stable version diagnostic', () => {
    ...
    ).toThrow(/legacy\.yaml.*schemaVersion.*expected 11/i);
  });
  ...
  it('publishes and parses strict schema-v7 achievement content', () => {
    const validAchievementYaml = `schemaVersion: 11
entries:
  - kind: achievement
...
`,
  ...
  it('parses strict score coefficients on the balance entry', () => {
    const validBalanceYaml = `schemaVersion: 11
entries:
...
`;
  ...
  it('parses the point-buy attribute table on the balance entry', () => {
    ...
    const validBalanceWithPointBuy = `schemaVersion: 11
entries:
...
`;
  ...
  it('cross-validates permanent merchants against lifetime fields', () => {
    ...
    const encounterYaml = (definition: string) => `schemaVersion: 11
entries:
...
`;
  ...
  it('requires the strongbox service to declare exactly one minimum and maximum use', () => {
    ...
    const encounterYaml = (servicesEntry: string) => `schemaVersion: 11
entries:
...
`;
  ...
  it('publishes and parses strict schema-v6 class content', () => {
    const validClassYaml = `schemaVersion: 11
entries:
  - kind: class
    id: class.wayfarer
    name: Wayfarer
...
`;
  ...
  it('rejects a trait declaring more than one modifier', () => {
    const traitWithTwoModifiers = `schemaVersion: 11
entries:
  - kind: trait
    id: trait.keen-eyed
    name: Keen-eyed
...
`;
  ...
  it('rejects a background declaring an unknown derived stat modifier', () => {
    const backgroundWithUnknownStat = `schemaVersion: 11
entries:
  - kind: background
...
`;
  ...
  it('requires a non-negative safe-integer threat on every monster', () => {
    const validMonsterYaml = `schemaVersion: 11
entries:
  - kind: monster
    id: monster.cave-rat
    name: Cave rat
