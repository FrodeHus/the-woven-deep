...
function validateEntry(value: unknown, index: number): ContentEntry {
  let input = value;
  let expectedVaultMetadata: Readonly<{ entranceCount: unknown; requiredSlotIds: unknown }> | undefined;
  ...
  if (!result.success) {
    ...
    throw new TypeError(`Invalid compiled content at entries.${index}.${issue.path.join('.')}: ${issue.message}`);
  }
  if (result.data.kind === 'vault') {
    if (expectedVaultMetadata?.entranceCount !== result.data.entranceCount
      || JSON.stringify(expectedVaultMetadata.requiredSlotIds) !== JSON.stringify(result.data.requiredSlotIds)) {
      throw new TypeError(`Invalid compiled content at entries.${index}: vault metadata does not match its layout`);
    }
  }
  if (normalizedJson(value) !== normalizedJson(result.data)) {
    throw new TypeError(`Invalid compiled content at entries.${index}: compiled entry is missing materialized fields`);
  }
...
export function validateCompiledContentPack(input: unknown): CompiledContentPack {
  ...
  if (suppliedVersion !== CONTENT_SCHEMA_VERSION) {
    throw new TypeError(`Unsupported content schema version ${String(suppliedVersion)}; expected ${CONTENT_SCHEMA_VERSION}`);
  }
  ...
  for (let index = 1; index < entries.length; index += 1) {
    if (entries[index - 1]!.id >= entries[index]!.id) {
      throw new TypeError(`Invalid compiled content at entries.${index}.id: identifiers must be unique and strictly increasing`);
    }
  }
  for (let index = 1; index < parsed.generationReport.foundationalCategories.length; index += 1) {
    if (parsed.generationReport.foundationalCategories[index - 1]! >= parsed.generationReport.foundationalCategories[index]!) {
      throw new TypeError('Invalid compiled content generation report: categories must be unique and strictly increasing');
    }
  }
