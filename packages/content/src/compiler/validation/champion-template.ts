...
function championTemplateIssues(
  ...
  if (located.length > 1) {
    issues.push(
      issue(
        located[1]!.file,
        '$.entries',
        `expected at most one fallen champion template; found ${located.length}`,
      ),
    );
  }
  for (const { entry, file } of located) {
    ...
    issues.push(
      ...referencedKindIssue(
        file,
        `${path}.fallbackMonsterId`,
        entry.fallbackMonsterId,
        'monster',
        byId,
      ),
    );
    issues.push(
      ...referencedKindIssue(file, `${path}.fallbackItemId`, entry.fallbackItemId, 'item', byId),
    );
    const fallbackItem = byId.get(entry.fallbackItemId);
    if (fallbackItem?.kind === 'item' && !fallbackItem.heirloomEligible) {
      issues.push(
        issue(file, `${path}.fallbackItemId`, 'Champion fallback item must be heirloom eligible'),
      );
    }
    issues.push(
      ...referencedKindIssue(
        file,
        `${path}.echoLootTableId`,
        entry.echoLootTableId,
        'loot-table',
        byId,
      ),
    );
    ...
    if (entry.echoAppearanceChance > 0 && fallbackMonster?.kind === 'monster') {
      const championHealth = Math.max(
        entry.minimumHealth,
        Math.min(entry.maximumHealth, fallbackMonster.health),
      );
      const championDamage = Math.min(
        entry.damageMaximum,
    ...
    const bossUniqueIds = new Set(
      [...byId.values()]
        .filter(
          (candidate): candidate is Extract<EncounterContentEntry, { readonly model: 'boss' }> =>
...
export function championTemplateEntriesIssues(
  ...
  const championTemplates = locatedEntries.filter(
    (located): located is LocatedContentEntry & { entry: FallenChampionTemplateContentEntry } =>
