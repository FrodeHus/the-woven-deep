...
function encounterIssues(
  ...
  if (encounter.model === 'group') {
    ...
    definition.roles.forEach((role, index) => {
      if (roleIds.has(role.roleId))
        issues.push(
          issue(file, `${path}.roles.${index}.roleId`, `duplicate group role ${role.roleId}`),
        );
      roleIds.add(role.roleId);
      issues.push(
        ...referencedKindIssue(
          file,
          `${path}.roles.${index}.monsterId`,
          role.monsterId,
          'monster',
          byId,
        ),
      ...
      if (monster?.kind === 'monster') {
        issues.push(
          ...validateParameters(
            file,
            `${path}.roles.${index}.behavior`,
            monster.behaviorId,
            role.behaviorParameters,
            BEHAVIOR_PARAMETER_SCHEMAS,
            'behavior',
          ),
        );
    ...
    if (!roleIds.has(definition.leaderRoleId)) {
      issues.push(
        issue(
          file,
          `${path}.leaderRoleId`,
          `leader role ${definition.leaderRoleId} is not declared in roles`,
        ),
      );
    }
    if (
      !checkedTotalWithin(
        definition.roles.map((role) => role.maximumQuantity),
        MAX_ENCOUNTER_MEMBERS,
      )
    ) {
      issues.push(
        issue(
          file,
          `${path}.roles`,
          `group maximum quantity exceeds runtime-safe limit ${MAX_ENCOUNTER_MEMBERS}`,
        ),
      );
    }
    if (definition.leaderDeathResponse === 'collapse' && !definition.supernaturalBond) {
      issues.push(
        issue(file, `${path}.supernaturalBond`, 'collapse requires supernaturalBond: true'),
      );
    }
    if (
      definition.leaderDeathResponse === 'collapse' &&
      definition.collapseRewards === 'individual' &&
      encounter.adminDescription === null
    ) {
      issues.push(
        issue(
          file,
          `$.entries.${encounter.id}.adminDescription`,
    ...
    issues.push(
      ...validateParameters(
        file,
        `${path}.response`,
        definition.leaderDeathResponse,
        definition.responseParameters,
        LEADER_RESPONSE_PARAMETER_SCHEMAS,
        'leader response',
      ),
    );
  ...
  if (encounter.model === 'swarm') {
    ...
    if (source?.kind === 'monster' && !source.tags.includes('swarm-source')) {
      issues.push(
        issue(
          file,
          `${path}.sourceMonsterId`,
          `swarm source ${source.id} requires tag swarm-source`,
        ),
      );
    }
    const roleIds = new Set<string>();
    definition.spawnRoles.forEach((role, index) => {
      if (roleIds.has(role.roleId))
        issues.push(
          issue(file, `${path}.spawnRoles.${index}.roleId`, `duplicate swarm role ${role.roleId}`),
        );
      roleIds.add(role.roleId);
      issues.push(
        ...referencedKindIssue(
          file,
          `${path}.spawnRoles.${index}.monsterId`,
          role.monsterId,
          'monster',
          byId,
        ),
    ...
    if (
      ...
      issues.push(
        issue(file, `${path}.spawnRoles`, `spawn-role weight total exceeds rollDie maximum 2^32`),
      );
    }
    ...
    if (definition.maximumSpawnQuantity > definition.maximumLivingChildren) {
      issues.push(
        issue(
          file,
          `${path}.maximumSpawnQuantity`,
          'maximum spawn quantity must not exceed maximum living children',
        ),
      );
    }
    if (definition.maximumLivingMembers < definition.maximumLivingChildren + 1) {
      issues.push(
        issue(
          file,
          `${path}.maximumLivingMembers`,
          'maximum living members must allow the source plus all living children',
        ),
      );
    }
    if (definition.maximumFloorActors < definition.maximumLivingMembers) {
      issues.push(
        issue(
          file,
          `${path}.maximumFloorActors`,
          'maximum floor actors must be at least maximum living members',
        ),
      );
    }
    issues.push(
      ...validateParameters(
        file,
        `${path}.response`,
        definition.sourceDestructionResponse,
        definition.responseParameters,
        SWARM_RESPONSE_PARAMETER_SCHEMAS,
        'swarm response',
      ),
  ...
  if (encounter.model === 'merchant') {
    ...
    issues.push(
      ...referencedKindIssue(
        file,
        `${path}.stockLootTableId`,
        definition.stockLootTableId,
        'loot-table',
        byId,
      ),
    );
    if (definition.maximumStockRolls < definition.minimumStockRolls) {
      issues.push(
        issue(
          file,
          `${path}.maximumStockRolls`,
          'maximum stock rolls must be at least minimum stock rolls',
        ),
      );
    }
    if (
      ...
      issues.push(
        issue(
          file,
          `${path}.maximumLifetime`,
          'maximum lifetime must be at least minimum lifetime',
        ),
      );
    }
    if (
      ...
      definition.departureWarningThresholds.forEach((threshold, index) => {
        if (threshold >= previous || threshold >= minimumLifetime) {
          issues.push(
            issue(
              file,
              `${path}.departureWarningThresholds.${index}`,
      ...
      if (serviceIds.has(service.serviceId))
        issues.push(
          issue(
            ...
            `duplicate merchant service ${service.serviceId}`,
          ),
        );
      serviceIds.add(service.serviceId);
      if (service.maximumUses < service.minimumUses)
        issues.push(
          issue(
            file,
            `${path}.services.${index}.maximumUses`,
            'maximum service uses must be at least minimum uses',
          ),
        );
      ...
      service.tierIds.forEach((tierId, tierIndex) => {
        if (tiers.has(tierId))
          issues.push(
            issue(
              file,
              `${path}.services.${index}.tierIds.${tierIndex}`,
              `duplicate service tier ${tierId}`,
            ),
          );
        tiers.add(tierId);
        ...
        if (!factionTier)
          issues.push(
            issue(
              file,
              `${path}.services.${index}.tierIds.${tierIndex}`,
              `service tier ${tierId} is absent from NPC faction`,
            ),
          );
    ...
    const bossUniqueIds = new Set(
      [...byId.values()]
        .filter((candidate) => candidate.kind === 'encounter' && candidate.model === 'boss')
            ...
            if (item.price <= 0)
              issues.push(
                issue(
                  file,
                  `${path}.stockLootTableId`,
                  `merchant stock item ${item.id} requires positive price`,
                ),
              );
            if (bossUniqueIds.has(item.id))
              issues.push(
                issue(
                  file,
                  `${path}.stockLootTableId`,
                  `merchant stock item ${item.id} is guaranteed unique`,
                ),
              );
            const reserved = item.tags.find((tag) =>
              ['heirloom', 'quest', 'objective', 'nontransferable'].includes(tag),
            );
            if (reserved)
              issues.push(
                issue(
                  file,
                  `${path}.stockLootTableId`,
                  `merchant stock item ${item.id} has reserved ${reserved} tag`,
                ),
              );
          }
        }
  ...
  const definition = encounter.definition;
  issues.push(
    ...referencedKindIssue(file, `${path}.monsterId`, definition.monsterId, 'monster', byId),
  );
  issues.push(
    ...referencedKindIssue(file, `${path}.uniqueItemId`, definition.uniqueItemId, 'item', byId),
  );
  issues.push(
    ...referencedKindIssue(
      file,
      `${path}.enhancedLootTableId`,
      definition.enhancedLootTableId,
      'loot-table',
      byId,
    ),
  );
  const visitLootForUnique = (tableId: string, visited = new Set<string>()): boolean => {
    ...
    return table.choices.some(
      (choice) =>
        choice.contentId === definition.uniqueItemId ||
  ...
  const duplicateUnique = [...byId.values()].filter(
    (candidate) =>
      candidate.kind === 'encounter' &&
      candidate.model === 'boss' &&
  ...
  if (duplicateUnique.length > 0) {
    issues.push(
      issue(
        file,
        `${path}.uniqueItemId`,
        `boss guaranteed unique item ${definition.uniqueItemId} is shared`,
      ),
    );
  }
  if (encounter.maximumInstancesPerRun !== 1) {
    issues.push(
      issue(
        file,
        `$.entries.${encounter.id}.maximumInstancesPerRun`,
        'boss encounters require maximumInstancesPerRun 1',
      ),
    );
  }
  ...
  definition.phases.forEach((phase, index) => {
    if (phaseIds.has(phase.phaseId))
      issues.push(
        issue(file, `${path}.phases.${index}.phaseId`, `duplicate boss phase ${phase.phaseId}`),
      );
    ...
    if (phase.healthThresholdPercent >= previousThreshold) {
      issues.push(
        issue(
          file,
          `${path}.phases.${index}.healthThresholdPercent`,
          'boss phase thresholds must be unique and strictly descending',
        ),
      );
    }
    ...
    issues.push(
      ...validateParameters(
        file,
        `${path}.phases.${index}.behavior`,
        phase.behaviorId,
        phase.behaviorParameters,
        BEHAVIOR_PARAMETER_SCHEMAS,
        'behavior',
      ),
    );
    phase.effects.forEach((effect, effectIndex) => {
      if (!(BOSS_PHASE_EFFECT_IDS as readonly string[]).includes(effect.effectId)) {
        issues.push(
          issue(
            file,
            `${path}.phases.${index}.effects.${effectIndex}.effectId`,
...
export function encounterEntriesIssues(
  ...
  if (
    !checkedTotalWithin(
      encounters.map(({ entry }) => (entry as EncounterContentEntry).weight),
      MAX_RANDOM_WEIGHT_TOTAL,
    )
  ) {
    const located = encounters.at(-1)!;
    issues.push(
      issue(located.file, '$.entries', 'encounter weight total exceeds rollDie maximum 2^32'),
    );
  }
  for (const { entry, file } of locatedEntries) {
    ...
    entry.requiredVaultTags.forEach((tag, index) => {
      if (!vaultTags.has(tag))
        issues.push(
          issue(
            file,
            `$.entries.${entry.id}.requiredVaultTags.${index}`,
            `unknown vault tag ${tag}`,
          ),
        );
    });
    if (entry.model === 'boss')
      entry.definition.vaultTags.forEach((tag, index) => {
        if (!vaultTags.has(tag))
          issues.push(
            issue(
              file,
              `$.entries.${entry.id}.definition.vaultTags.${index}`,
              `unknown vault tag ${tag}`,
            ),
          );
      });
