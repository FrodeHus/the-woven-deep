...
type IssueSink = { addIssue: (issue: { code: 'custom'; path: (string | number)[]; message: string }) => void };
...
const refineDiscoveryProtection = (entry: EncounterDiscovery, context: IssueSink): void => {
  ...
  if (entry.discoveryProtectionCap !== undefined && entry.discoveryProtectionCap < entry.runAppearanceChance) {
...
const merchantEncounterEntry = z.strictObject({
  ...encounterCommon,
