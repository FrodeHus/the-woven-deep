...
const conditionDuration = z.discriminatedUnion('mode', [
  z
    .strictObject({ mode: z.literal('timed'), default: safePositive, maximum: safePositive })
    .refine((value) => value.default <= value.maximum, {
      path: ['default'],
      message: 'default duration must not exceed maximum duration',
    }),
  z.strictObject({ mode: z.literal('permanent'), default: z.null(), maximum: z.null() }),
]);

export const conditionEntry = z
  .strictObject({
    ...base,
