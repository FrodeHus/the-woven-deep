...
import { createDemoRun, materializeMerchant, rollDie, type ActiveRun } from '../src/index.js';
...
describe('merchant stock materialization', () => {
  it('is deterministic, owns only merchant-stock RNG, and initializes finite merchant state', () => {
    ...
    const lifetime = rollDie(
      input.run.rng['merchant-stock'],
      encounter.definition.maximumLifetime - encounter.definition.minimumLifetime + 1,
    );

    expect(first).toEqual(second);
    expect(first.population.rolledLifetime).toBe(
      encounter.definition.minimumLifetime + lifetime.value - 1,
    );
    expect(first.population.departureAt).toBe(
      input.run.worldTime + first.population.rolledLifetime,
    );
    ...
    expect(first.actor).toMatchObject({
      actorId: 'actor.population.merchant-test.001',
      contentId: encounter.definition.npcId,
    ...
    expect(first.population.initialStockItemIds).toEqual(
      first.items.map((item) => item.itemId).sort(),
    );
    ...
    expect(first.items.map((item) => item.itemId)).toEqual(
      first.items.map(
        (_, index) => `item.${input.populationId}.stock.${String(index + 1).padStart(6, '0')}`,
      ),
    );
    expect(
      first.items.every(
        (item) => item.condition === 100 && item.enchantment === null && item.charges === null,
      ),
    ).toBe(true);
  ...
  it('rejects an empty depth-eligible stock graph before consuming caller state', () => {
    ...
    const entries = content.entries.map((entry) =>
      entry.kind === 'item'
        ? ({ ...entry, minDepth: 99, maxDepth: 100 } as ItemContentEntry)
        : entry,
    );
    const before = JSON.stringify(input.run.rng);

    expect(() => materializeMerchant({ ...input, content: { ...content, entries } })).toThrow(
      /merchant stock.*eligible/i,
    );
