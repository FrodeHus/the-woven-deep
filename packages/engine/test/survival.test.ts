...
import {
  advanceSurvival, createDemoContentPack, createDemoRun, hungerStage, resolveEffectSequence,
  type ItemInstance,
...
function balance(overrides: Partial<BalanceContentEntry> = {}): BalanceContentEntry {
  ...
  return { ...base, hungerMaximum: 100, hungerThresholds: thresholds,
    recoveryInterval: 5, recoveryAmount: 2,
    recoveryByHungerStage: { sated: 100, hungry: 50, weak: 0, starving: 0 }, ...overrides } as BalanceContentEntry;
}

function light(): ItemContentEntry {
  return {
    kind: 'item', id: 'item.torch', name: 'Torch', glyph: 'i', color: '#ffffff', tags: ['torch'],
    category: 'light', stackLimit: 1, price: 1, rarity: 'common', minDepth: 0, maxDepth: 20,
...
function fixture(overrides: Readonly<{ elapsed?: number; hunger?: number; fuel?: number; enabled?: boolean }> = {}) {
  const elapsed = overrides.elapsed ?? 7;
  const base = createDemoRun();
  const definition = light();
  const item: ItemInstance = {
    itemId: 'torch.1', contentId: definition.id, quantity: 1, condition: 100,
    enchantment: null, identified: true, charges: null, fuel: overrides.fuel ?? 12,
    enabled: overrides.enabled ?? true,
    location: { type: 'equipped', actorId: 'hero.demo', slot: 'off-hand' },
  };
  const content = { ...createDemoContentPack(), entries: [
    balance(), ...createDemoContentPack().entries.filter((entry) => entry.kind !== 'balance'), definition,
  ] };
  const state = { ...base, worldTime: elapsed, items: [item],
    survival: { ...base.survival, hungerReserve: overrides.hunger ?? 20,
...
describe('survival clocks', () => {
  ...
  it('uses the loaded hunger-stage modifiers without hidden constants', () => {
    const configured = balance({ hungerStageModifiers: {
      sated: {}, hungry: {}, weak: { defense: -3 }, starving: { defense: -6 },
    } });
  ...
  it('emits skipped hunger stages once in deterioration order', () => {
    ...
    expect(first.events.filter((event) => event.type === 'hunger.stage-changed')
      .map((event) => event.stage)).toEqual(['hungry', 'weak', 'starving']);
  ...
  it('does not drain disabled or backpack light items', () => {
    ...
    const result = advanceSurvival({ ...backpack, state: { ...backpack.state,
  ...
  it('restores only effective food reserve at the maximum', () => {
    ...
    const result = resolveEffectSequence({
      effects: [{ effectId: 'effect.hunger.restore', parameters: { amount: 20 }, requiresLivingTarget: true }],
      actors: input.state.actors, items: input.state.items, survival: input.state.survival,
      survivalActorId: 'hero.demo',
      content: input.content, sourceActorId: 'hero.demo', targetActorId: 'hero.demo',
      effectsState: input.state.rng.effects, worldTime: 0, eventId: 'event.eat',
      forceMoveDirection: { x: 1, y: 0 }, operations: {},
    });
  ...
  it('recovers on crossed absolute intervals only when safe and sufficiently fed', () => {
    ...
    const danger = advanceSurvival({ ...input, danger: true, state: { ...input.state, actors: [actor] } });
  ...
  it('does not recover while an active condition blocks recovery', () => {
    const input = fixture({ elapsed: 10, hunger: 90 });
    const condition = {
      kind: 'condition' as const, id: 'condition.no-recovery', name: 'No recovery', description: 'No recovery',
      tags: [], color: '#ffffff', duration: { mode: 'timed' as const, default: 20, maximum: 20 },
      stacking: { mode: 'refresh' as const, maximumStacks: 1 }, modifiersPerStack: {},
      traits: ['condition-trait.blocks-recovery' as const],
    };
    const actor = { ...input.state.actors[0]!, health: 10, conditions: [{ conditionId: condition.id,
      sourceActorId: null, appliedAt: 0, expiresAt: 20, stacks: 1 }] };
    const content = { ...input.content, entries: [...input.content.entries, condition] };
    const result = advanceSurvival({ ...input, content, state: { ...input.state, actors: [actor] } });
  ...
  it('applies starvation damage at saved absolute deadlines', () => {
    ...
    const state = { ...input.state, worldTime: 11, survival: { ...input.state.survival,
      hungerReserve: 0, hungerStage: 'starving' as const, nextStarvationAt: 7 } };
  ...
  it('rejects saved hunger values that disagree with the loaded balance data', () => {
    ...
    expect(() => validateContentBoundRun({ ...input.state, survival: {
      ...input.state.survival, hungerReserve: 101,
    } }, input.content)).toThrow(/hunger reserve/i);
    expect(() => validateContentBoundRun({ ...input.state, survival: {
      ...input.state.survival, hungerStage: 'sated',
    } }, input.content)).toThrow(/hunger stage/i);
  });
});
