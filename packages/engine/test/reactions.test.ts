...
function hostileDeparture() {
  ...
  return {
    run: { ...run, actors: [mover, ...attackers] },
    content: createDemoContentPack(),
    moverActorId: mover.actorId,
    from: { x: 2, y: 2 },
    to: { x: 1, y: 2 },
    eventId: 'event.move',
  };
...
describe('relationships and reactions', () => {
  it('stores relationship overrides as a sorted symmetric pair', () => {
    ...
    expect(changed.relationships).toEqual([
      {
        leftActorId: 'hero.demo',
        rightActorId: 'npc.neutral',
  ...
  it('creates no opportunity reaction for a neutral merchant fleeing past a monster', () => {
    ...
    const monster = actor({
      actorId: 'monster.hostile',
      disposition: 'hostile',
      x: 3,
      y: 2,
      awareActorIds: ['npc.merchant'],
      reactionReady: true,
    });
  ...
  it('makes a provoked merchant movement reaction-eligible through the hostile override', () => {
    ...
    const merchant = actor({ actorId: 'npc.merchant', disposition: 'neutral', x: 2, y: 2 });
    const run = setRelationship(
      { ...base, actors: [hero, merchant] },
      hero.actorId,
      'npc.merchant',
      'hostile',
    );
    expect(
      eligibleOpportunityAttackers({
        ...
        from: { x: 2, y: 2 },
        to: { x: 1, y: 2 },
      }).map((attacker) => attacker.actorId),
    ).toEqual([hero.actorId]);
  });
  ...
  it('resolves aware hostile reactions by stable actor ID before movement', () => {
    const result = resolveOpportunityAttacks({
      ...hostileDeparture(),
      resolveAttack: missedReaction,
    });
    expect(
      result.events
        .filter((event) => event.type === 'reaction.triggered')
        .map((event: any) => event.actorId),
    ).toEqual(['monster.a', 'monster.b']);
    expect(result.movementAllowed).toBe(true);
    expect(
      result.state.actors
        .filter((candidate) => candidate.actorId.startsWith('monster.'))
        .every((candidate) => !candidate.reactionReady),
    ).toBe(true);
  });
  ...
  it('cancels movement after a blocking condition but resolves captured living attackers', () => {
    ...
    const content: CompiledContentPack = {
      ...base.content,
      entries: [...base.content.entries, root],
    };
    let attacks = 0;
    const result = resolveOpportunityAttacks({
      ...base,
      ...
      resolveAttack: (input: any) => {
        ...
        const actors =
          attacks === 1
            ? input.actors.map((candidate: ActorState) =>
                candidate.actorId === input.targetActorId
                  ? {
                      ...candidate,
                      conditions: [
                        {
