...
import type {
  ContentEntry, EncounterContentEntry, ItemContentEntry, LootTableContentEntry,
  MerchantEncounterContentEntry, NpcContentEntry, NpcFactionContentEntry,
 ...
 */
export const merchantPropertyScenarioArbitrary: fc.Arbitrary<MerchantPropertyScenario> = fc.record({
  ...
  lifetimeBounds: fc.tuple(fc.integer({ min: 300, max: 5_000 }), fc.integer({ min: 300, max: 5_000 })),
  ...
  lowTier: fc.record({
    purchasePriceBps: fc.integer({ min: 1, max: 20_000 }), salePriceBps: fc.integer({ min: 1, max: 20_000 }),
    acceptsTrade: fc.boolean(), identify: fc.boolean(),
  }),
  highTier: fc.record({
    purchasePriceBps: fc.integer({ min: 1, max: 20_000 }), salePriceBps: fc.integer({ min: 1, max: 20_000 }),
    acceptsTrade: fc.boolean(), identify: fc.boolean(),
  ...
  const npc: NpcContentEntry = {
    kind: 'npc', id: 'npc.property-merchant', name: 'Property Merchant', tags: ['merchant'],
    glyph: '$', color: '#ffaa00', factionId,
    attributes: { might: 6, agility: 8, vitality: 9, wits: 10, resolve: 9 },
    health: 18, speed: 100, perception: 8, accuracy: 4, defense: 8,
    damage: { count: 1, sides: 3, bonus: 0 }, armor: 0,
  ...
  const stockItem: ItemContentEntry = {
    kind: 'item', id: 'item.property-stock', name: 'Property Stock', tags: [], glyph: '!', color: '#ffffff',
    category: 'potion', stackLimit: input.stackLimit, price: input.price, rarity: 'common',
    heirloomEligible: false, minDepth: 1, maxDepth: 10, actionCost: 100, equipment: null,
    combat: null, light: null, identification: { mode: 'known', poolId: null }, effects: [],
  };
  const trinket: ItemContentEntry = {
    ...stockItem, id: 'item.property-trinket', name: 'Property Trinket', category: 'ring',
    identification: { mode: 'instance', poolId: 'identification-pool.property' },
  };
  const trinketPool: ContentEntry = {
    kind: 'identification-pool', id: 'identification-pool.property', name: 'Property pool',
    tags: [], category: 'ring', verbs: ['Odd'], nouns: ['band'],
    visuals: [{ id: 'visual.property-band', glyph: '=', color: '#888888' }],
  };
  const stockTable: LootTableContentEntry = {
    kind: 'loot-table', id: 'loot-table.property-stock', name: 'Property Stock', tags: [], rolls: 1,
    choices: [{
      contentId: stockItem.id, lootTableId: null, weight: 1,
      minimumQuantity: 1, maximumQuantity: Math.min(2, input.stackLimit),
    }],
  };
  const tiers = [
    {
      tierId: 'low', name: 'Low', minimum: -100, maximum: input.tierCut,
      purchasePriceBps: input.lowTier.purchasePriceBps, salePriceBps: input.lowTier.salePriceBps,
      acceptsTrade: input.lowTier.acceptsTrade,
      serviceIds: input.lowTier.identify ? ['merchant-service.identify' as const] : [],
    },
    {
      tierId: 'high', name: 'High', minimum: input.tierCut + 1, maximum: 100,
      purchasePriceBps: input.highTier.purchasePriceBps, salePriceBps: input.highTier.salePriceBps,
  ...
  const faction: NpcFactionContentEntry = {
    kind: 'npc-faction', id: factionId, name: 'Property Faction', tags: [],
    minimumReputation: -100, maximumReputation: 100, startingReputation: input.startingReputation,
    tiers,
  };
  const serviceTierIds = tiers.filter((tier) => tier.serviceIds.length > 0)
    .map((tier) => tier.tierId).sort();
  ...
  const warningThresholds = [Math.floor(minimumLifetime / 2), Math.floor(minimumLifetime / 4),
    Math.floor(minimumLifetime / 8)].slice(0, input.warningCount)
    .filter((threshold, index, all) => threshold > 0 && all.indexOf(threshold) === index);
  const encounter: MerchantEncounterContentEntry = {
    kind: 'encounter', id: 'encounter.property-merchant', name: 'Property Merchant',
    adminDescription: null, tags: ['merchant'], model: 'merchant', minDepth: 1, maxDepth: 10,
    environmentTags: [], requiredVaultTags: [], weight: 1, rarity: 'uncommon',
    runAppearanceChance: 1, discoveryProtectionIncrement: 0, discoveryProtectionCap: 1,
    maximumInstancesPerRun: 1,
    placement: {
      minimumStairDistance: 0, minimumObjectiveDistance: 0, maximumMemberDistance: 0,
      allowedTerrainTags: ['floor'], requiresVaultSlot: false, failureMode: 'optional',
    },
    intentPresentation: { visible: true },
    definition: {
      npcId: npc.id, stockLootTableId: stockTable.id,
      minimumStockRolls: Math.min(rollA, rollB), maximumStockRolls: Math.max(rollA, rollB),
      merchantSaleBps: input.saleBps, merchantPurchaseBps: input.purchaseBps,
      acceptedCategories: ['potion'],
      services: serviceTierIds.length === 0 ? [] : [{
        serviceId: 'merchant-service.identify', basePrice: input.servicePrice,
        minimumUses: Math.min(useA, useB), maximumUses: Math.max(useA, useB),
        tierIds: serviceTierIds,
      }],
      minimumLifetime, maximumLifetime: Math.max(lifeA, lifeB),
  ...
  return {
    entries: [faction, npc, stockItem, trinket, trinketPool, stockTable, encounter] as readonly ContentEntry[],
