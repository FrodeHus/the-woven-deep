...
function definition(overrides: Partial<SwarmEncounterDefinition> = {}): SwarmEncounterDefinition {
  return {
    sourceMonsterId: 'monster.source',
    spawnRoles: [
    ...
    maximumLivingChildren: 4,
    maximumLivingMembers: 5,
    maximumFloorActors: 8,
    sourceDestructionResponse: 'stop',
    responseParameters: {},
    ...overrides,
  };
}
...
function fixture(overrides: Partial<SwarmEncounterDefinition> = {}, active = true) {
  ...
  const source: ActorState = {
    ...hero,
    actorId: 'actor.source',
    contentId: 'monster.source',
  ...
  const content: CompiledContentPack = {
    ...
    entries: [
      ...
      { ...roleMonster('monster.source', 'S'), tags: ['swarm-source'], health: 8 },
      roleMonster('monster.child-a', 'a'),
      roleMonster('monster.child-b', 'b'),
      encounter,
    ],
  };
  ...
  const state: ActiveRun = {
    ...base,
    worldTime: 10,
    activeFloorId: active ? hero.floorId : 'floor.inactive',
    actors: [hero, source],
    populations: [population],
    floors: [
      { ...base.floors[0]!, entities: [{ entityId: source.actorId, x: source.x, y: source.y }] },
    ],
    encounterDecisions: [
      {
        encounterId: encounter.id,
...
describe('swarm timers, placement, and caps', () => {
  it.each([
    ...
    'rejects bypassed unsafe %s before a partial swarm transition',
    (_label, overrides) => {
      ...
      expect(() =>
        advanceSwarms({
          ...
          sourceActionActorId: 'actor.source',
        }),
  ...
  it('rejects a saved swarm above its authored living caps', () => {
    const first = fixture();
    const spawned = advanceSwarms({
      state: first.state,
      content: first.content,
      eventId: 'event.overcap-validation',
      sourceActionActorId: 'actor.source',
    });
    const constrained = fixture({ maximumLivingChildren: 1, maximumLivingMembers: 2 }).content;
    expect(() => validateContentBoundRun(spawned.state, constrained)).toThrow(
      /swarm.*(cap|living)/i,
    );
  });
  ...
  it('keeps spawned actor occupancy authoritative after movement frees an origin', () => {
    ...
    const spawned = advanceSwarms({
      state: first.state,
      content: first.content,
      eventId: 'event.spawn-authoritative',
      sourceActionActorId: 'actor.source',
    });
    const child = spawned.state.actors.find((actor) =>
      actor.actorId.startsWith('actor.population.swarm.spawn.'),
    )!;
    ...
    const moved = {
      ...spawned.state,
      actors: spawned.state.actors.map((actor) =>
        actor.actorId === child.actorId ? { ...actor, x: 5, y: 5, health: 0 } : actor,
      ),
      worldTime: spawned.state.worldTime + 10,
      populations: spawned.state.populations.map((population) =>
        population.model === 'swarm'
          ? { ...population, nextSpawnAt: spawned.state.worldTime + 10 }
          : population,
      ),
    };
    const respawned = advanceSwarms({
      state: moved,
      content: first.content,
      eventId: 'event.respawn-authoritative',
      sourceActionActorId: 'actor.source',
    });
    expect(
      respawned.state.actors
        .filter((actor) => actor.health > 0 && actor.populationId === 'population.swarm')
        .some((actor) => actor.x === child.x && actor.y === child.y),
    ).toBe(true);
    expect(
      respawned.state.floors
        .find((candidate) => candidate.floorId === child.floorId)
        ?.entities.some((entity) => entity.entityId.startsWith('actor.population.swarm.spawn.')),
    ).toBe(false);
  });
  ...
  it('processes one world-time transition, fills legal row-major cells, and advances only encounters RNG', () => {
    ...
    expect(
      result.state.actors
        .filter((actor) => actor.populationRoleId === 'a' || actor.populationRoleId === 'b')
        .map(({ actorId, x, y }) => ({ actorId, x, y })),
    ).toEqual([
    ...
    expect(result.events).toContainEqual(
      expect.objectContaining({ type: 'swarm.members-created', quantity: 2 }),
    );
  });
  ...
  it('never overwrites occupied, blocked, stairs, objectives, features, or vault-reserved cells', () => {
    ...
    expect(
      result.state.actors
        .filter(
          (actor) => actor.populationId === 'population.swarm' && actor.actorId !== 'actor.source',
        )
        .every((actor) => !['2:1', '3:1', '4:1', '2:2'].includes(`${actor.x}:${actor.y}`)),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    const first = resolveSwarmSpawnAction({
      state,
      content,
      sourceActorId: 'actor.source',
      eventId: `event.cap.${level}`,
    });
    expect(first.events).toContainEqual(
      expect.objectContaining({ type: 'swarm.cap-reached', level }),
    );
    const second = resolveSwarmSpawnAction({
      state: {
        ...first.state,
        worldTime: 20,
        actors: first.state.actors.map((actor) =>
          actor.actorId === 'actor.source' ? { ...actor, energy: 100 } : actor,
        ),
      },
      content,
      sourceActorId: 'actor.source',
      eventId: `event.cap2.${level}`,
    });
  ...
  it('freezes inactive growth and schedules from re-entry without catch-up', () => {
    ...
    const inactive = advanceSwarms({
      state: { ...state, worldTime: 100 },
      content,
      eventId: 'event.inactive',
    });
    expect(inactive.state.populations[0]).toEqual(state.populations[0]);
    const reentered = advanceSwarms({
      state: { ...inactive.state, activeFloorId: 'floor.demo', activeFloorEnteredAt: 100 },
      content,
  ...
  it('requires the ready source to select and pay the configured spawn action', () => {
    ...
    const notReady = {
      ...
      actors: state.actors.map((actor) =>
        actor.actorId === 'actor.source' ? { ...actor, energy: 99 } : actor,
      ),
    };
    expect(
      chooseBehaviorAction({ state: notReady, actorId: 'actor.source', content }).type,
    ).not.toBe('swarm-spawn');
    ...
    const spawned = resolveSwarmSpawnAction({
      state,
      content,
      sourceActorId: 'actor.source',
    ...
    expect(spawned.events).toContainEqual(
      expect.objectContaining({ type: 'swarm.members-created' }),
    );
  });

  it.each(['dead', 'former'] as const)(
    'rejects a %s source without charging energy or spawning',
    (mode) => {
      const { state, content } = fixture();
      const guarded = {
        ...state,
        actors: state.actors.map((actor) =>
          actor.actorId === 'actor.source'
            ? { ...actor, health: mode === 'dead' ? 0 : actor.health }
            : actor,
        ),
        populations: state.populations.map((population) =>
          population.model === 'swarm' && mode === 'former'
  ...
  it('runs one due spawn through ready actor turn selection', () => {
    ...
    const result = resolveWorldStep({
      ...
      eventId: 'event.world-spawn',
    });
    expect(result.events).toContainEqual(
      expect.objectContaining({ type: 'actor.turn.started', actorId: 'actor.source' }),
    );
    expect(result.events).toContainEqual(
      expect.objectContaining({ type: 'swarm.members-created', quantity: 2 }),
    );
    expect(result.events).toContainEqual(
      expect.objectContaining({ type: 'actor.turn.completed', actionType: 'swarm-spawn' }),
    );
  });
  ...
  it('saves and emits spawn intent before a due non-adjacent source action', () => {
    ...
    const slowedContent = {
      ...
      entries: content.entries.map((entry) =>
        entry.kind === 'monster' && entry.id === 'monster.source' ? { ...entry, speed: 25 } : entry,
      ),
    };
    const slowedState = {
      ...state,
      actors: state.actors.map((actor) =>
        actor.actorId === 'actor.source' ? { ...actor, speed: 25 } : actor,
      ),
    };
    const result = resolveWorldStep({
      state: slowedState,
      content: slowedContent,
      action: { type: 'wait', actorId: state.hero.actorId, cost: 100 },
      eventId: 'event.spawn-intent',
    });
    const intentIndex = result.events.findIndex(
      (event) =>
        event.type === 'actor.intent-changed' &&
        event.actorId === 'actor.source' &&
        event.intent === 'spawn',
    );
    ...
    expect(
      result.state.actors.find((actor) => actor.actorId === 'actor.source')?.behaviorState.intent,
    ).toBe('spawn');
  });
  ...
  it('prioritizes adjacent legitimate attack over due spawn intent and action', () => {
    ...
    const adjacent = {
      ...state,
      actors: state.actors.map((actor) =>
        actor.actorId === state.hero.actorId ? { ...actor, x: 2, y: 3 } : actor,
      ),
    };
    const result = resolveWorldStep({
      state: adjacent,
      content,
      action: { type: 'wait', actorId: state.hero.actorId, cost: 100 },
      eventId: 'event.attack-before-spawn',
    });
    const sourceEvents = result.events.filter(
      (event) => 'actorId' in event && event.actorId === 'actor.source',
    );
    expect(sourceEvents).toContainEqual(
      expect.objectContaining({ type: 'actor.intent-changed', intent: 'attack' }),
    );
    expect(sourceEvents).toContainEqual(
      expect.objectContaining({ type: 'actor.turn.completed', actionType: 'bump-attack' }),
    );
  ...
  it('starts a new cap event episode after population falls below the cap', () => {
    ...
    const first = resolveSwarmSpawnAction({
      ...
      content,
      sourceActorId: 'actor.source',
      eventId: 'event.cap.first',
    });
    const child = first.state.actors.find(
      (actor) => actor.populationRoleId === 'a' || actor.populationRoleId === 'b',
    )!;
    const reduced = advanceSwarms({
      state: {
        ...first.state,
        worldTime: 20,
        actors: first.state.actors.map((actor) =>
          actor.actorId === child.actorId ? { ...actor, health: 0 } : actor,
        ),
      },
      content,
      eventId: 'event.cap.reset',
    });
    expect((reduced.state.populations[0] as SwarmPopulation).emittedCapLevels).not.toContain(
      'source',
    );
    const again = resolveSwarmSpawnAction({
      state: {
        ...reduced.state,
        actors: reduced.state.actors.map((actor) =>
          actor.actorId === 'actor.source' ? { ...actor, energy: 100 } : actor,
        ),
      },
      content,
      sourceActorId: 'actor.source',
      eventId: 'event.cap.again',
    });
    expect(again.events).toContainEqual(
      expect.objectContaining({ type: 'swarm.cap-reached', level: 'source' }),
    );
  });
});
...
describe('swarm shutdown responses', () => {
  const destroyed = (
    response: SwarmEncounterDefinition['sourceDestructionResponse'],
    parameters = {},
  ) => {
    const result = fixture({ sourceDestructionResponse: response, responseParameters: parameters });
    const child: ActorState = {
      ...result.state.actors[1]!,
      actorId: 'actor.child',
      contentId: 'monster.child-a',
      x: 4,
      populationRoleId: 'a',
      health: 4,
      maxHealth: 4,
      behaviorState: {
        ...result.state.actors[1]!.behaviorState,
        lastKnownTargets: [
          {
            targetActorId: result.state.hero.actorId,
            floorId: result.state.activeFloorId,
            x: 1,
            y: 1,
            observedAt: 9,
            source: 'sight',
            observerActorId: 'actor.child',
          },
        ],
      },
    };
    return {
      ...
      state: {
        ...result.state,
        actors: [result.state.actors[0]!, { ...result.state.actors[1]!, health: 0 }, child],
        populations: [
          {
            ...(result.state.populations[0] as SwarmPopulation),
  ...
  it('stops permanently and children never inherit timers', () => {
    ...
    expect(first.state.populations[0]).toMatchObject({
      shutdownState: 'stop',
      livingMemberIds: ['actor.child'],
    });
    expect(first.state.actors.find((actor) => actor.actorId === 'actor.child')).not.toHaveProperty(
      'nextSpawnAt',
    );
  });
  ...
  it('uses normal path-based flee behavior', () => {
    ...
    expect(
      result.state.actors.find((actor) => actor.actorId === 'actor.child')?.behaviorState,
    ).toMatchObject({ intent: 'flee', goal: { type: 'cell' }, investigation: { expiresAt: null } });
    const saved = result.state.actors.find((actor) => actor.actorId === 'actor.child')!
      .behaviorState.goal;
    const movedHidden = {
      ...result.state,
      actors: result.state.actors.map((actor) =>
        actor.actorId === result.state.hero.actorId ? { ...actor, x: 5, y: 3 } : actor,
      ),
    };
    expect(
      movedHidden.actors.find((actor) => actor.actorId === 'actor.child')!.behaviorState.goal,
    ).toEqual(saved);
  });
  ...
  it('applies deterministic timed decay once per interval', () => {
    ...
    expect(started.events).toContainEqual(
      expect.objectContaining({
        ...
        conditionId: 'condition.swarm-decay',
        actorId: 'actor.child',
      }),
    );
    const due = advanceSwarms({
      state: { ...started.state, worldTime: 15 },
      content,
      eventId: 'event.decay.due',
    });
    ...
    expect(
      advanceSwarms({ state: due.state, content, eventId: 'event.decay.repeat' }).state,
    ).toEqual(due.state);
    const death = advanceSwarms({
      state: { ...due.state, worldTime: 20 },
      content,
      eventId: 'event.decay.death',
    });
    expect(death.events).toContainEqual(
      expect.objectContaining({ type: 'actor.died', actorId: 'actor.child' }),
    );
  });

  it('bounds frenzy by its authored duration', () => {
    const { state, content } = destroyed('frenzy', {
      duration: 5,
      modifiers: { accuracy: 1, defense: 0, damage: 2 },
    });
    const started = advanceSwarms({ state, content, eventId: 'event.frenzy' });
    expect(started.state.populations[0]).toMatchObject({
      shutdownState: 'frenzy',
      shutdownExpiresAt: 15,
    });
    const ended = advanceSwarms({
      state: { ...started.state, worldTime: 15 },
      content,
      eventId: 'event.frenzy.end',
    });
    expect(
      ended.state.actors.find((actor) => actor.actorId === 'actor.child')?.behaviorState.intent,
    ).toBe('hold');
  });
  ...
  it('expires only the frenzy modifier while preserving legitimate member behavior state', () => {
    ...
    const behavioral = {
      ...started.state,
      actors: started.state.actors.map((actor) =>
        actor.actorId === 'actor.child'
          ? {
              ...actor,
              behaviorState: {
                ...actor.behaviorState,
                intent: 'approach' as const,
                goal,
                investigation: {
    ...
    expect(
      ended.state.actors.find((actor) => actor.actorId === 'actor.child')?.behaviorState,
  ...
  it('round-trips lifecycle state exactly', () => {
    const { state, content } = fixture();
    const advanced = resolveSwarmSpawnAction({
      state,
      content,
      sourceActorId: 'actor.source',
      eventId: 'event.save',
