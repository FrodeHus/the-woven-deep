...
import type {
  CompiledContentPack, EncounterContentEntry, FallenChampionTemplateContentEntry, ItemContentEntry,
  LootTableContentEntry, MonsterContentEntry,
} from '@woven-deep/content';
import {
  createDemoContentPack, createDemoRun, rollDie, selectHeirloom,
  type ActiveRun, type EquipmentSlot, type ItemInstance,
} from '../src/index.js';
...
const monster: MonsterContentEntry = {
  kind: 'monster', id: 'monster.boss', name: 'Boss', tags: ['boss'], glyph: 'B', color: '#aa7755',
  ...
  minDepth: 1, maxDepth: 20, threat: 12, rarity: 'legendary',
};
const bossLoot: LootTableContentEntry = { kind: 'loot-table', id: 'loot-table.boss', name: 'Boss loot',
  tags: [], rolls: 1, choices: [{ contentId: 'item.fallback', lootTableId: null, weight: 1,
    minimumQuantity: 1, maximumQuantity: 1 }] };
const bossEncounter: EncounterContentEntry = { kind: 'encounter', id: 'encounter.boss', name: 'Boss',
  tags: [], adminDescription: null, model: 'boss', minDepth: 1, maxDepth: 20, environmentTags: [],
  requiredVaultTags: [], weight: 1, rarity: 'legendary', runAppearanceChance: 1, discoveryProtectionIncrement: 0,
  discoveryProtectionCap: 1, maximumInstancesPerRun: 1, placement: { minimumStairDistance: 0,
    ...
    failureMode: 'optional' }, intentPresentation: { visible: true }, definition: { monsterId: monster.id, phases: [],
    recoveryPerWorldTime: 0, recoveryCapPercent: 0, uniqueItemId: 'item.boss-unique',
...
const template: FallenChampionTemplateContentEntry = { kind: 'fallen-champion-template',
  ...
  fallbackMonsterId: monster.id, fallbackItemId: 'item.fallback',
  ...
  echoDefensePercent: 80, echoAbilityLimit: 1, echoLootTableId: bossLoot.id,
...
function pack(extra: readonly ItemContentEntry[] = []): CompiledContentPack {
  ...
  return { ...base, entries: [...base.entries, monster, bossLoot, bossEncounter, template,
    itemDef('item.fallback', { rarity: 'common' }), ...extra] };
}

function equippedItem(itemId: string, contentId: string, slot: EquipmentSlot,
  ...
  return { itemId, contentId, quantity: 1, condition: 100, enchantment: null, identified: true,
    ...
    location: { type: 'equipped', actorId: 'hero.demo', slot }, ...overrides };
}

function deadRun(items: readonly ItemInstance[]): ActiveRun {
  const base = createDemoRun();
  return { ...base, items: [...items],
    conclusion: { completionType: 'died', cause: { killerContentId: null, depth: 1, turn: 10, worldTime: 1000 },
      concludedAtRevision: 1, finalized: false } };
}
...
describe('selectHeirloom', () => {
  it('selects deterministically from equipped items and advances only the run-records stream', () => {
    const content = pack([
      ...
      itemDef('item.crown', { rarity: 'legendary', equipment: { slots: ['head'], handedness: 'one-handed', reservedSlots: [] } }),
    ]);
    const deadHeroRun = deadRun([
      equippedItem('item.hero.sword', 'item.sword', 'main-hand', {
        condition: 62, charges: 3, fuel: 7,
        enchantment: { enchantmentId: 'enchantment.honed', modifiers: { meleeDamageBonus: 2, defense: -1, accuracy: 1 } },
      }),
    ...
    expect(first.snapshot).toEqual({
      contentId: 'item.sword', sourceItemId: 'item.hero.sword',
      enchantment: { enchantmentId: 'enchantment.honed', modifiers: { meleeDamageBonus: 2, defense: -1, accuracy: 1 } },
      condition: 62, charges: 3, fuel: 7, qualityRank: 2,
      displayName: 'Hero sword', glyph: '/', color: '#ddeeff', originatingHallRecordId: recordId,
    });
  ...
  it('excludes tagged, boss-unique, ineligible, and non-equipment items from candidacy', () => {
    const content = pack([
      itemDef('item.sword'),
      itemDef('item.relic', { tags: ['heirloom'], equipment: { slots: ['neck'], handedness: 'one-handed', reservedSlots: [] } }),
      itemDef('item.quest-seal', { tags: ['quest'], equipment: { slots: ['left-ring'], handedness: 'one-handed', reservedSlots: [] } }),
      itemDef('item.objective-key', { tags: ['objective'], equipment: { slots: ['right-ring'], handedness: 'one-handed', reservedSlots: [] } }),
      itemDef('item.bound-band', { tags: ['nontransferable'], equipment: { slots: ['hands'], handedness: 'one-handed', reservedSlots: [] } }),
      itemDef('item.boss-unique', { rarity: 'legendary', equipment: { slots: ['head'], handedness: 'one-handed', reservedSlots: [] } }),
      itemDef('item.mundane', { heirloomEligible: false, equipment: { slots: ['body'], handedness: 'one-handed', reservedSlots: [] } }),
  ...
  it('weights candidates by rarity weight plus quality rank bonus and walks cumulative weights', () => {
    const content = pack([
      ...
      itemDef('item.spear', { rarity: 'rare', equipment: { slots: ['off-hand'], handedness: 'one-handed', reservedSlots: [] } }),
    ]);
    const deadHeroRun = deadRun([
      equippedItem('item.hero.a-blade', 'item.blade', 'main-hand', {
        enchantment: { enchantmentId: 'enchantment.keen', modifiers: { accuracy: 1, defense: 2, weight: -3 } } }),
  ...
  it('keeps a depleted common item at positive weight so it can win a forced roll', () => {
    ...
    const deadHeroRun = deadRun([
      equippedItem('item.hero.stub', 'item.stub', 'main-hand', { condition: 0, charges: 0, fuel: 0 }),
    ]);
    const result = selectHeirloom({ run: deadHeroRun, content, template, recordId });
  ...
  it('records one unit from a stack and counts the stack as a single candidate', () => {
    ...
    const stacked = deadRun([equippedItem('item.hero.darts', 'item.dart', 'main-hand', { quantity: 5 })]);
  ...
  it('treats a two-handed item as one candidate', () => {
    const content = pack([
      itemDef('item.greatsword', { rarity: 'rare',
        equipment: { slots: ['main-hand'], handedness: 'two-handed', reservedSlots: ['off-hand'] } }),
      itemDef('item.charm', { rarity: 'common', equipment: { slots: ['neck'], handedness: 'one-handed', reservedSlots: [] } }),
    ]);
    const deadHeroRun = deadRun([
  ...
  it('returns the template fallback relic without consuming randomness when nothing is eligible', () => {
    ...
    expect(result.snapshot).toEqual({
      ...
      fuel: null, qualityRank: 0, displayName: 'Name of item.fallback', glyph: ')', color: '#c0c0c0',
  ...
  it('throws for a living hero', () => {
    ...
    const run = { ...deadRun([equippedItem('item.hero.sword', 'item.sword', 'main-hand')]), conclusion: null };
