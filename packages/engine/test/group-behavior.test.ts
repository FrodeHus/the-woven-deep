...
const ROLE = {
  roleId: 'guard',
  monsterId: 'monster.training-beetle',
  minimumQuantity: 1,
  maximumQuantity: 8,
  formationPreference: 'front' as const,
  behaviorParameters: {},
};
...
function pack(groupDefinition = definition()): CompiledContentPack {
  ...
  const monster = {
    ...
    id: 'monster.training-beetle',
    name: 'Training beetle',
    tags: [],
    glyph: 'g',
    color: '#888888',
    attributes: { might: 3, agility: 3, vitality: 3, wits: 3, resolve: 3 },
    health: 10,
    speed: 100,
    accuracy: 1,
    defense: 5,
...
function member(base: ActorState, id: string, x: number, y: number, directAt?: number): ActorState {
  return {
    ...
    disposition: 'hostile',
    behaviorId: 'behavior.approach-and-attack',
    populationId: 'population.group-test',
...
function fixture(
  options: Readonly<{
    positions?: readonly [string, number, number, number?][];
    groupDefinition?: GroupEncounterDefinition;
    active?: boolean;
  }> = {},
) {
  ...
  const members = positions.map(([id, x, y, at]) => {
    ...
    return id === 'monster.a'
      ? {
          ...
          populationPresentation: {
            name: 'Guard',
            glyph: groupDefinition.leaderAlternateGlyph ?? 'g',
            color: groupDefinition.leaderAccentColor,
            leader: true,
          },
        }
      : actor;
  ...
  const population: GroupPopulation = {
    populationId: 'population.group-test',
    encounterId: 'encounter.group-test',
    model: 'group',
    floorId: hero.floorId,
    createdAt: 0,
    livingMemberIds: members.map((actor) => actor.actorId).sort(),
    formerMemberIds: [],
    leaderActorId: 'monster.a',
    bonusActive: true,
...
describe('group communication and formations', () => {
  it('relays in sorted breadth-first order across connected chains', () => {
    ...
    expect(
      result.events
        ...
        .map((event) => ('actorId' in event ? event.actorId : null)),
    ).toEqual(['monster.b', 'monster.c']);
    expect(
      result.state.actors
        .filter((actor) => actor.populationId !== null)
        .map((actor) => actor.behaviorState.lastKnownTargets[0]?.source),
    ).toEqual(['sight', 'group', 'group']);
    expect(
      result.state.actors.find((actor) => actor.actorId === 'monster.c')?.behaviorState.goal,
  ...
  it('does not share across a gap larger than communication range', () => {
    ...
    expect(
      result.state.actors.find((actor) => actor.actorId === 'monster.b')?.behaviorState
        .lastKnownTargets,
    ).toEqual([]);
  });
      ...
      actor.actorId === 'monster.b'
        ? {
            ...
            behaviorState: {
              ...
              lastKnownTargets: [
                {
                  ...actor.behaviorState.lastKnownTargets[0]!,
                  x: 5,
                  observerActorId: 'monster.b',
                  observedAt: 12,
                },
              ],
            },
          }
        : actor.actorId === 'monster.c'
          ? {
              ...actor,
              behaviorState: {
                ...actor.behaviorState,
  ...
  it('freezes communication and goals on inactive floors', () => {
    ...
    expect(coordinateGroups({ state, content, eventId: 'event.inactive' })).toEqual({
      state,
      events: [],
    });
  });
  ...
  it.each(['cluster', 'line', 'screen', 'wedge', 'surround'] as const)(
    'assigns stable legal %s formation goals without mutating input',
    (formation) => {
      const groupDefinition = definition({
        formation,
        roles: [ROLE, { ...ROLE, roleId: 'rear', formationPreference: 'rear' }],
      });
      const { state, content } = fixture({
        groupDefinition,
        positions: [
      ...
      const groupActors = first.actors.filter(
        (candidate) => candidate.populationId === 'population.group-test',
      );
      expect(
        groupActors.find((actor) => actor.actorId === 'monster.a')?.behaviorState.goal,
      ).toBeNull();
      const formationGoals = groupActors
        .map((actor) => actor.behaviorState.goal)
  ...
  it('assigns center, front, rear, and flank roles to distinct preferred slots', () => {
    ...
    const { state, content } = fixture({
      groupDefinition: definition({ formation: 'wedge', roles, leaderRoleId: 'center' }),
      positions: [
        ['monster.a', 3, 2],
        ['monster.b', 1, 1],
        ['monster.c', 5, 1],
    ...
    const actors = state.actors.map((actor) =>
      roleByActor.has(actor.actorId)
        ? { ...actor, populationRoleId: roleByActor.get(actor.actorId)! }
        : actor,
    );
    const populations = state.populations.map((population) =>
      population.model === 'group'
        ? {
            ...population,
            roleMembership: [...roleByActor].map(([actorId, roleId]) => ({ actorId, roleId })),
          }
        : population,
    );
    const openFloor = state.floors.map((floor) =>
      floor.floorId === state.activeFloorId
        ? { ...floor, tiles: floor.tiles.map(() => 1 as const) }
        : floor,
    );
    const result = coordinateGroups({
      state: { ...state, actors, populations, floors: openFloor },
      content,
      eventId: 'event.roles',
    }).state;
    const goal = (actorId: string) =>
      result.actors.find((actor) => actor.actorId === actorId)!.behaviorState.goal!;
  ...
  it('keeps a front-preference leader as a stable anchor across sequential world steps', () => {
    ...
    let current = {
      ...
      relationships: [
        {
          leftActorId: state.hero.actorId,
          rightActorId: 'monster.a',
    ...
    for (let step = 0; step < 4; step += 1) {
      ...
      expect(current.actors.find((actor) => actor.actorId === 'monster.a')).toMatchObject({
        x: origin.x,
        y: origin.y,
        behaviorState: { intent: 'hold', goal: null },
      });
      expect(result.events).not.toContainEqual(
        expect.objectContaining({ type: 'actor.moved', actorId: 'monster.a' }),
      );
    }
  });
  ...
  it('holds without a formation goal when every improving formation cell is blocked', () => {
    ...
    const floors = [
      {
        ...floor,
        tiles: floor.tiles.map((_, index) =>
          open.has(`${index % floor.width}:${Math.floor(index / floor.width)}`)
            ? (1 as const)
            : (0 as const),
        ),
      },
    ...
    const relationships = ['monster.a', 'monster.b'].map((actorId) => ({
      leftActorId: state.hero.actorId,
      rightActorId: actorId,
      relationship: 'neutral' as const,
    }));
    const result = resolveWorldStep({
      state: { ...state, actors, floors, relationships },
      content,
      action: { type: 'wait', actorId: state.hero.actorId, cost: 100 },
      eventId: 'event.blocked-formation',
      maxInternalActions: 1,
    });
    ...
    expect(result.events).not.toContainEqual(
      expect.objectContaining({ type: 'actor.moved', actorId: 'monster.b' }),
    );
  });
});
...
describe('leaders and deterministic outcomes', () => {
  it('rejects group membership outside authored role quantities', () => {
    const { state, content } = fixture({
      groupDefinition: definition({ roles: [{ ...ROLE, maximumQuantity: 2 }] }),
    });
    expect(() => validateContentBoundRun(state, content)).toThrow(
      /group.*quantity|role.*quantity/i,
    );
  });
  ...
  it('reconciles an ordinary dead member before saves are encoded', () => {
    ...
    const dead = {
      ...
      actors: state.actors.map((actor) =>
        actor.actorId === 'monster.b' ? { ...actor, health: 0 } : actor,
      ),
    };
    const result = applyGroupLeaderOutcomes({
      state: dead,
      content,
      eventId: 'event.member-death',
    });
  ...
  it('resolves a defeated leader at the authoritative world-step boundary', () => {
    ...
    const dead = {
      ...state,
      actors: state.actors.map((actor) =>
        actor.actorId === 'monster.a' ? { ...actor, health: 0 } : actor,
      ),
    };
    const result = resolveWorldStep({
      state: dead,
      ...
      eventId: 'event.world-group',
      maxInternalActions: 2,
    });
  ...
  it('uses authored leader presentation and applies coordination only while alive', () => {
    const { state, content } = fixture();
    expect(
      state.actors.find((actor) => actor.actorId === 'monster.a')?.populationPresentation,
    ).toMatchObject({ glyph: 'L', color: '#ffcc00', leader: true });
    expect(groupCombatModifiers({ state, content, actorId: 'monster.b' })).toEqual({
      accuracy: 2,
      defense: 3,
      damage: 4,
    });
    ...
    expect(groupCombatModifiers({ state: dead, content, actorId: 'monster.b' })).toEqual({
      accuracy: 0,
      defense: 0,
      damage: 0,
    });
  ...
  it('publishes qualitative visible group outcomes without authoritative details', () => {
    ...
    const events = [
      {
        type: 'group.leader-defeated' as const,
        eventId: 'event.public',
        populationId: 'population.secret',
        actorId: 'monster.a',
      },
      {
        type: 'group.outcome-applied' as const,
        eventId: 'event.public',
        populationId: 'population.secret',
        actorId: 'monster.a',
        response: 'collapse' as const,
        individualRewards: true,
        collapsedMemberCount: 7,
      },
    ];
    expect(projectDomainEvents({ state, content, heroId: state.hero.actorId, events })).toEqual([
      {
        ...
        eventId: 'event.public',
        category: 'leader-defeated',
        actorId: 'monster.a',
        presentation: 'group.leader-defeated',
      },
      {
        type: 'population.notice',
        eventId: 'event.public',
        category: 'group-outcome',
        actorId: 'monster.a',
        presentation: 'leader-response.collapse',
      },
    ]);
    expect(
      JSON.stringify(projectDomainEvents({ state, content, heroId: state.hero.actorId, events })),
  ...
  it('applies frenzy combat modifiers through direct observation until deterministic expiry', () => {
    ...
    const dead = {
      ...state,
      actors: state.actors.map((actor) =>
        actor.actorId === 'monster.a' ? { ...actor, health: 0, energy: 0 } : actor,
      ),
    };
    const outcome = applyGroupLeaderOutcomes({
      state: dead,
      content,
      eventId: 'event.frenzy',
    }).state;
    const runAt = (worldTime: number) =>
      resolveWorldStep({
        state: {
          ...outcome,
          worldTime,
          ...
          actors: outcome.actors.map((actor) => ({
            ...actor,
            energy: actor.actorId === state.hero.actorId || actor.actorId === 'monster.b' ? 100 : 0,
          })),
        },
        content,
        action: { type: 'wait', actorId: state.hero.actorId, cost: 100 },
        eventId: `event.frenzy.${worldTime}`,
        maxInternalActions: 1,
      });
    const activeAttack = runAt(39).events.find(
      (event) => event.type === 'attack.hit' || event.type === 'attack.missed',
    )!;
    const expiredAttack = runAt(40).events.find(
      (event) => event.type === 'attack.hit' || event.type === 'attack.missed',
    )!;
  ...
  it.each(['panic', 'frenzy'] as const)(
    'rejects %s deadline overflow without a partial transition',
    (response) => {
      ...
      const prepared = {
        ...state,
        worldTime: Number.MAX_SAFE_INTEGER - 10,
        actors: state.actors.map((actor) =>
          actor.actorId === 'monster.a'
            ? { ...actor, health: 0 }
            : actor.actorId === 'monster.b'
      ...
      expect(() =>
        applyGroupLeaderOutcomes({
          state: prepared,
          content,
          eventId: `event.${response}.overflow`,
        }),
  ...
  it.each([
    ...
    'applies the %s response exactly once in sorted member order',
    (response, responseParameters) => {
      const { state, content } = fixture({
        groupDefinition: definition({ leaderDeathResponse: response, responseParameters }),
      });
      const dead = {
        ...state,
        actors: state.actors.map((actor) =>
      ...
      const second = applyGroupLeaderOutcomes({
        state: first.state,
        content,
        eventId: 'event.death',
      });
      ...
      const survivors = first.state.actors.filter(
        (actor) => actor.actorId === 'monster.b' || actor.actorId === 'monster.c',
      );
      if (response === 'panic')
        expect(survivors.every((actor) => actor.behaviorState.intent === 'hold')).toBe(true);
      if (response === 'disband')
        expect(survivors.every((actor) => actor.populationId === null)).toBe(true);
      if (response === 'surrender')
        expect(survivors.every((actor) => actor.disposition === 'neutral')).toBe(true);
      if (response === 'weaken' || response === 'frenzy') {
        expect(groupCombatModifiers({ state: first.state, content, actorId: 'monster.b' })).toEqual(
          responseParameters.modifiers,
        );
      }
      if (response === 'frenzy')
        expect(
          groupCombatModifiers({
            state: {
              ...first.state,
              worldTime: first.state.worldTime + responseParameters.duration,
            },
            content,
            actorId: 'monster.b',
          }),
      ...
      if (response === 'frenzy') {
        ...
        expect(first.state.populations[0] as GroupPopulation).toMatchObject({
          leaderResponseExpiresAt: 40,
        });
        const observed = {
          ...first.state,
          actors: first.state.actors.map((actor) =>
            actor.actorId === 'monster.b'
              ? {
                  ...actor,
                  behaviorState: {
        ...
        expect(groupCombatModifiers({ state: observed, content, actorId: 'monster.b' })).toEqual(
          responseParameters.modifiers,
        );
      }
    },
  ...
  it('saves a reachable panic retreat away from the latest hostile observation', () => {
    const { state, content } = fixture({
      groupDefinition: definition({
        ...
        responseParameters: { duration: 20 },
      }),
      positions: [
        ['monster.a', 2, 2],
        ['monster.b', 3, 1],
    ...
    const goal = survivor.behaviorState.goal as Extract<
      typeof survivor.behaviorState.goal,
      { type: 'cell' }
    >;
    expect(Math.max(Math.abs(goal.x - hero.x), Math.abs(goal.y - hero.y))).toBeGreaterThan(
      Math.max(Math.abs(survivor.x - hero.x), Math.abs(survivor.y - hero.y)),
    );
    expect(decodeActiveRun(encodeActiveRun(result.state))).toEqual(result.state);
  });
  ...
  it('clears hostile overrides on surrender and becomes hostile again only when attacked', () => {
    const { state, content } = fixture({
      groupDefinition: definition({ leaderDeathResponse: 'surrender', responseParameters: {} }),
      positions: [
        ['monster.a', 2, 2],
        ['monster.b', 2, 1],
      ],
    ...
    const hostile = {
      ...
      relationships: state.relationships.map((entry) =>
        entry.leftActorId === heroId && entry.rightActorId === 'monster.b'
          ? { ...entry, relationship: 'hostile' as const }
          : entry,
      ),
      actors: state.actors.map((actor) =>
        actor.actorId === 'monster.a' ? { ...actor, health: 0 } : actor,
    ...
    const surrendered = applyGroupLeaderOutcomes({
      state: hostile,
      content,
      eventId: 'event.surrender',
    }).state;
    expect(relationshipBetween(surrendered, heroId, 'monster.b')).toBe('neutral');
    const held = resolveWorldStep({
      state: surrendered,
      content,
      action: { type: 'wait', actorId: heroId, cost: 100 },
      eventId: 'event.surrender-hold',
      maxInternalActions: 1,
    });
    ...
    const attacked = resolveWorldStep({
      state: {
        ...surrendered,
        actors: surrendered.actors.map((actor) =>
          actor.actorId === heroId ? { ...actor, energy: 100 } : actor,
        ),
      },
      content,
      action: { type: 'attack', actorId: heroId, targetActorId: 'monster.b', cost: 100 },
      eventId: 'event.break-surrender',
      maxInternalActions: 0,
    });
    expect(relationshipBetween(attacked.state, heroId, 'monster.b')).toBe('hostile');
  });

  it.each(['none', 'individual'] as const)(
    'collapses supernatural members with %s reward policy',
    (collapseRewards) => {
      const { state, content } = fixture({
        groupDefinition: definition({
          leaderDeathResponse: 'collapse',
          responseParameters: {},
      ...
      expect(
        result.state.actors
          .filter((actor) => actor.populationId === 'population.group-test')
          .every((actor) => actor.health === 0),
      ).toBe(true);
      expect(result.events.filter((event) => event.type === 'actor.died')).toHaveLength(
        collapseRewards === 'individual' ? 2 : 0,
      );
      if (collapseRewards === 'individual')
        expect(
          result.events
            .filter((event) => event.type === 'actor.died')
            .every((event) => event.killerActorId === state.hero.actorId),
        ).toBe(true);
      expect(result.events.find((event) => event.type === 'group.outcome-applied')).toMatchObject({
        response: 'collapse',
        individualRewards: collapseRewards === 'individual',
        collapsedMemberCount: 2,
      });
    },
  );
  ...
  it('derives regroup intent from a formation goal before emitting or acting', () => {
    ...
    const neutral = {
      ...state,
      actors,
      relationships: ['monster.a', 'monster.b'].map((actorId) => ({
        leftActorId: state.hero.actorId,
        rightActorId: actorId,
        relationship: 'neutral' as const,
      })),
