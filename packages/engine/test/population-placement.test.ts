...
function individual(
  id: string,
  overrides: Partial<EncounterContentEntry> = {},
): EncounterContentEntry {
  return {
    kind: 'encounter',
    id,
...
function swarm(id: string): EncounterContentEntry {
  return individual(id, {
    model: 'swarm',
    definition: {
      sourceMonsterId: 'monster.test-a',
      spawnRoles: [{ roleId: 'child', monsterId: 'monster.test-b', weight: 1 }],
      spawnInterval: 10,
      minimumSpawnQuantity: 1,
      maximumSpawnQuantity: 1,
      placementRadius: 2,
      allowedTerrainTags: ['floor'],
      maximumLivingChildren: 2,
      maximumLivingMembers: 3,
      maximumFloorActors: 4,
...
function pack(
  ...
  return {
    ...
    entries: [
      ...base.entries,
      monster('monster.test-a'),
      monster('monster.test-b'),
      ...extras,
      ...encounters,
    ],
  };
...
function merchantFixture(overrides: Partial<MerchantEncounterContentEntry> = {}): Readonly<{
  npc: NpcContentEntry;
  stockItem: ItemContentEntry;
  stock: LootTableContentEntry;
  encounter: MerchantEncounterContentEntry;
}> {
  const npc: NpcContentEntry = {
    kind: 'npc',
    id: 'npc.test-merchant',
    name: 'Test Merchant',
    tags: ['merchant'],
    glyph: '$',
    color: '#ffaa00',
    factionId: 'npc-faction.test',
    attributes: { might: 1, agility: 2, vitality: 3, wits: 4, resolve: 5 },
    health: 9,
    speed: 80,
    perception: 6,
    accuracy: 2,
  ...
  const encounter = {
    ...(individual('encounter.test-merchant', {
      ...
      definition: {
        npcId: npc.id,
        stockLootTableId: stock.id,
        minimumStockRolls: 1,
        maximumStockRolls: 1,
        merchantSaleBps: 12000,
        merchantPurchaseBps: 6000,
        acceptedCategories: [stockItem.category],
        services: [],
        minimumLifetime: 10,
        maximumLifetime: 10,
        departureWarningThresholds: [5],
        aggressionResponse: 'flee',
        commerceReputationDelta: 1,
        aggressionReputationDelta: -1,
        deathReputationDelta: -1,
...
describe('population placement selection and composition', () => {
  it('composes a merchant as one neutral NPC with finite stock while preserving encounter stream behavior', () => {
    ...
    const result = placePopulation({
      run,
      floor: floor(),
      content: pack([encounter], [npc, stockItem, stock]),
    ...
    expect(result.createdActors[0]).toMatchObject({
      contentId: npc.id,
      disposition: 'neutral',
      behaviorId: npc.behaviorId,
  ...
  it('gates merchants by depth, instance cap, and stair distance without touching the merchant stream', () => {
    ...
    const shallowResult = placePopulation({
      run: runFor([shallow.encounter]),
      floor: floor(),
    ...
    const cappedResult = placePopulation({
      run: {
        ...cappedRun,
        encounterDecisions: cappedRun.encounterDecisions.map((decision) => ({
          ...decision,
          instancesCreated: capped.encounter.maximumInstancesPerRun,
        })),
      },
      floor: floor(),
      content: pack([capped.encounter], [capped.npc, capped.stockItem, capped.stock]),
    ...
    const distantResult = placePopulation({
      run: distantRun,
      floor: floor(),
  ...
  it('anchors a merchant to a matching mandatory vault slot without overwriting an occupied slot', () => {
    ...
    const generated = floor({
      ...
      vaults: [
        {
          placementId: 'vault-placement.arena',
          vaultId: 'vault.arena',
          x: 2,
          y: 1,
          width: 5,
      ...
      placementSlots: [
        {
          slotId: 'slot.arena-a',
          vaultPlacementId: 'vault-placement.arena',
          kind: 'monster',
          required: false,
          tags: ['arena'],
          x: 3,
  ...
  it('skips a merchant that cannot honor the objective distance without touching the merchant stream', () => {
    ...
    const generated = floor({
      vaults: [
        {
          placementId: 'vault-placement.goal',
          vaultId: 'vault.goal',
          x: 5,
          y: 1,
          width: 1,
      ...
      placementSlots: [
        {
          ...
          vaultPlacementId: 'vault-placement.goal',
          kind: 'objective',
          required: true,
          tags: ['goal'],
          x: 5,
  ...
  it('does not place a merchant where it would sever the required stair route', () => {
    ...
    const width = 7;
    const height = 3;
    const tiles = [0, 0, 0, 0, 0, 0, 0, 0, 4, 1, 1, 1, 5, 0, 0, 0, 0, 0, 0, 0, 0] as const;
    const generated = floor({
      width,
      height,
      tiles,
  ...
  it('rejects bypassed unsafe weights and quantities before consuming RNG or allocating members', () => {
    const oversized = individual('encounter.oversized', {
      definition: {
        monsterId: 'monster.test-a',
        minimumQuantity: 1,
        maximumQuantity: 1025,
      },
    });
    ...
    expect(() =>
      placePopulation({
        ...
        forcedEncounterId: oversized.id,
      }),
    ).toThrow(/population preflight.*quantity.*1024/i);
    ...
    expect(() =>
      placePopulation({ run: weightedRun, floor: floor(), content: pack(weighted) }),
  ...
  it('rejects a swarm placement whose initial saved timer would overflow', () => {
    ...
    const result = placePopulation({
      run: { ...runFor([entry]), worldTime: Number.MAX_SAFE_INTEGER - 5 },
      floor: floor(),
      content: pack([entry]),
      forcedEncounterId: entry.id,
    });
  ...
  it('filters by depth, environment, vault tags, eligibility, and the per-run limit', () => {
    ...
    const decisions = run.encounterDecisions.map((decision) =>
      decision.encounterId === 'encounter.limit'
        ? { ...decision, instancesCreated: 1 }
        : decision.encounterId === 'encounter.ineligible'
          ? { ...decision, eligible: false }
          : decision,
    );

    const result = placePopulation({
      run: { ...run, encounterDecisions: decisions },
      floor: floor(),
  ...
  it('uses code-unit encounter ordering before stable weighted selection', () => {
    ...
    const b = individual('encounter.b', {
      weight: 3,
      definition: {
        monsterId: 'monster.test-b',
        minimumQuantity: 1,
        maximumQuantity: 1,
      },
    });
  ...
  it('rolls inclusive role quantities and optional leadership in authored role order', () => {
    ...
    expect(result.createdActors.map((actor) => actor.actorId)).toEqual(
      expectedRoles.map(
        (_, index) => `actor.population.000001.${String(index + 1).padStart(3, '0')}`,
      ),
    );
  ...
  it('places up to and including a raised per-run cap (matching the raised early-game encounter caps)', () => {
    ...
    const belowCapResult = placePopulation({
      run: {
        ...belowCapRun,
        encounterDecisions: belowCapRun.encounterDecisions.map((decision) => ({
          ...decision,
          instancesCreated: 23,
        })),
      },
    ...
    if (belowCapResult.status === 'placed') {
      expect(
        belowCapResult.encounterDecisions.find((decision) => decision.encounterId === encounter.id)
          ?.instancesCreated,
      ).toBe(24);
    }
    ...
    const atCapResult = placePopulation({
      run: {
        ...atCapRun,
        encounterDecisions: atCapRun.encounterDecisions.map((decision) => ({
          ...decision,
          instancesCreated: 24,
        })),
      },
  ...
  it('advances fixed population allocation past existing actor identifiers', () => {
    ...
    const collidingActor = {
      ...base.actors[0]!,
      actorId: 'actor.population.000001.001',
      playerControlled: false,
      contentId: 'monster.test-a',
      behaviorId: 'behavior.approach-and-attack',
    };
    const run = {
      ...base,
      actors: [...base.actors, collidingActor].sort((left, right) =>
...
describe('atomic population placement', () => {
  it('finds a complete distance-bounded set when an earlier compatible point is a dead end', () => {
    ...
    expect(result.status).toBe('placed');
    if (result.status !== 'placed') return;
    expect(result.createdActors.map(({ x, y }) => [x, y])).toEqual([
      [3, 0],
      [1, 1],
      [3, 1],
      [1, 2],
  ...
  it('inherits mandatory anchor tags from the parent vault', () => {
    ...
    const generated = floor({
      vaults: [
        {
          placementId: 'vault-placement.parent',
          vaultId: vault.id,
          x: 3,
          y: 2,
          width: 1,
      ...
      placementSlots: [
        {
          slotId: 'slot.parent-monster',
          vaultPlacementId: 'vault-placement.parent',
          kind: 'monster',
          required: false,
          tags: [],
          x: 3,
  ...
  it('enumerates row-major cells while excluding actors, features, stairs, objectives, and vault slots', () => {
    ...
    const generated = floor({
      ...
      vaults: [
        {
          placementId: 'vault-placement.test',
          vaultId: 'vault.test',
          x: 5,
          y: 1,
          width: 2,
      ...
      placementSlots: [
        {
          slotId: 'slot.objective',
          vaultPlacementId: 'vault-placement.test',
          kind: 'objective',
          required: true,
          tags: ['goal'],
          x: 5,
    ...
    const occupyingActor = {
      ...baseRun.actors[0]!,
      actorId: 'monster.existing',
      contentId: 'monster.test-a',
      playerControlled: false,
    ...
    const run = {
      ...baseRun,
      actors: [...baseRun.actors, occupyingActor].sort((left, right) =>
        left.actorId < right.actorId ? -1 : 1,
      ),
      features: [
        ...baseRun.features,
        {
          featureId: 'feature.blocker',
          type: 'trap' as const,
          floorId: generated.floorId,
          x: 4,
          y: 1,
    ...
    expect(
      result.floor.entities.find((entity) => entity.entityId === result.createdActors[0]!.actorId),
    ).toBeUndefined();
  });
...
function corridorWithOneBranchFloor(
  width: number,
  height: number,
  branch: Readonly<{ x: number; y: number }>,
): FloorSnapshot {
  ...
  return floor({
    floorId: 'floor.density-branch',
    width,
    height,
    tiles: tiles as FloorSnapshot['tiles'],
    stairUp: { x: 1, y: 1 },
    stairDown: { x: width - 2, y: height - 2 },
    knowledge: createUnknownKnowledge(tiles.length),
  });
...
describe('vault item slot consumption', () => {
  const stockItemEntry: ItemContentEntry = {
    kind: 'item',
    id: 'item.test-cache-loot',
    name: 'Test Cache Loot',
    tags: [],
    glyph: '!',
    color: '#ffffff',
    category: 'potion',
  ...
  const itemLootTable: LootTableContentEntry = {
    kind: 'loot-table',
    id: 'loot-table.test-item-cache',
    name: 'Test Item Cache',
    tags: [],
    rolls: 1,
    choices: [
      {
        contentId: stockItemEntry.id,
        lootTableId: null,
        weight: 1,
        minimumQuantity: 1,
        maximumQuantity: 1,
  ...
  function itemCacheVault(
    id: string,
    slot: Readonly<{ lootTableId: string | null; contentId: string | null }>,
  ): VaultContentEntry {
    return {
      kind: 'vault',
      id,
  ...
  it('fills a vault item slot from its loot table at the slot position', () => {
    ...
    expect(
      placed.every((item) => item.itemId.startsWith('item.vault.slot.test.0.item-cache')),
    ).toBe(true);
  });
  ...
  it('fills a vault item slot from a direct content id at the slot position', () => {
    ...
    const vault = itemCacheVault('vault.item-cache-direct-test', {
      lootTableId: null,
  ...
  it('is a pure function of the floor seed', () => {
    ...
    const vault = itemCacheVault('vault.item-cache-deterministic-test', {
      lootTableId: itemLootTable.id,
      contentId: null,
    });
    const generated = itemCacheFloor(vault.id);
  ...
  it('does not refill an item slot the run already has an item placed at', () => {
    ...
    const vault = itemCacheVault('vault.item-cache-idempotent-test', {
      lootTableId: itemLootTable.id,
      contentId: null,
    });
    const generated = itemCacheFloor(vault.id);
