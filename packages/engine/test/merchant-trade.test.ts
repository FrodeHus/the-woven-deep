...
beforeAll(async () => {
  ...
  lampOil = content.entries.find(
    (entry): entry is ItemContentEntry => entry.kind === 'item' && entry.id === 'item.lamp-oil',
  )!;
  ration = content.entries.find(
    (entry): entry is ItemContentEntry =>
      entry.kind === 'item' && entry.id === 'item.travel-ration',
  )!;
...
function merchantRun(options: FixtureOptions = {}): ActiveRun {
  ...
  const stock: ItemInstance[] = [
    item('item.stock.lamp-oil', lampOil.id, 4, {
      type: 'merchant-stock',
      populationId: POPULATION_ID,
    }),
    item('item.stock.ration', ration.id, 2, {
      type: 'merchant-stock',
      populationId: POPULATION_ID,
    }),
  ];
...
function buyCommand(
  overrides: Partial<Extract<GameCommand, { type: 'trade-buy' }>> = {},
): GameCommand {
  return {
    type: 'trade-buy',
...
function sellCommand(
  overrides: Partial<Extract<GameCommand, { type: 'trade-sell' }>> = {},
): GameCommand {
  return {
    type: 'trade-sell',
    commandId: 'command.trade-sell',
    expectedRevision: 1,
    merchantPopulationId: POPULATION_ID,
    itemId: 'item.hero.sword',
    quantity: 1,
    ...overrides,
  };
}
...
function closeCommand(
  overrides: Partial<Extract<GameCommand, { type: 'trade-close' }>> = {},
): GameCommand {
  return {
    type: 'trade-close',
...
describe('trade-open', () => {
  ...
  it('rejects a missing, dead, busy, hostile, or departure-due merchant', () => {
    const missing = resolveCommand(
      merchantRun(),
      openCommand({ merchantActorId: 'actor.absent' }),
      context(),
    );
    ...
    const dead = {
      ...
      actors: base.actors.map((actor) =>
        actor.actorId === MERCHANT_ACTOR_ID ? { ...actor, health: 0 } : actor,
      ),
    };
    expect(resolveCommand(dead, openCommand(), context()).result).toMatchObject({
      status: 'invalid',
      reason: 'merchant.unavailable',
    });
    ...
    const fleeing = {
      ...base,
      populations: base.populations.map((population) => ({
        ...population,
        lifecycle: 'fleeing' as const,
      })),
    };
    expect(resolveCommand(fleeing, openCommand(), context()).result).toMatchObject({
      status: 'invalid',
      reason: 'merchant.unavailable',
    });
    ...
    const hostile = {
      ...base,
      relationships: [
        { leftActorId: MERCHANT_ACTOR_ID, rightActorId: HERO_ID, relationship: 'hostile' as const },
      ],
    };
    expect(resolveCommand(hostile, openCommand(), context()).result).toMatchObject({
      status: 'invalid',
      reason: 'merchant.unavailable',
    });

    const due = { ...base, worldTime: merchantPopulation(base).departureAt };
    expect(resolveCommand(due, openCommand(), context()).result).toMatchObject({
      status: 'invalid',
      reason: 'merchant.unavailable',
    });
  });
  ...
  it('never opens two trades and consumes no randomness on failure', () => {
    ...
    const again = resolveCommand(
      opened,
      openCommand({ commandId: 'command.trade-open-2', expectedRevision: 1 }),
      context(),
    );
  ...
  it('replays identical commands, rejects conflicting reuse, and rejects stale revisions', () => {
    ...
    const conflict = resolveCommand(
      first.state,
      openCommand({ merchantActorId: 'actor.other' }),
      context(),
    );
    ...
    const stale = resolveCommand(
      first.state,
      openCommand({ commandId: 'command.trade-open-stale' }),
      context(),
    );
  ...
  it('rejects every ordinary command while a trade is open', () => {
    ...
    const ordinary: readonly GameCommand[] = [
      ...
      {
        type: 'pickup',
        commandId: 'command.blocked-pickup',
        expectedRevision: 1,
        itemId: 'item.hero.oil',
        quantity: 1,
      },
      {
        type: 'rest',
        commandId: 'command.blocked-rest',
        expectedRevision: 1,
        until: 'healed',
        maximumDuration: 100,
      },
...
describe('trade-buy', () => {
  it('buys a full stock stack, merging into compatible backpack stacks at current-tier prices', () => {
    ...
    expect(
      bought.state.items.find((entry) => entry.itemId === 'item.stock.lamp-oil'),
    ).toBeUndefined();
    expect(bought.state.items.find((entry) => entry.itemId === 'item.hero.oil')).toMatchObject({
      quantity: 6,
    ...
    expect(bought.events).toEqual([
      {
        type: 'trade.bought',
        eventId: 'command.trade-buy',
        merchantPopulationId: POPULATION_ID,
        itemId: 'item.stock.lamp-oil',
        contentId: lampOil.id,
        quantity: 4,
        unitPrice: unit,
        total: unit * 4,
        currency: opened.hero.currency - unit * 4,
      },
    ]);
  });
  ...
  it('buys a partial stack into a new backpack stack derived from the command id', () => {
    ...
    const bought = resolveCommand(
      opened,
      buyCommand({ itemId: 'item.stock.ration', quantity: 1 }),
      context(),
    );
    expect(bought.result).toMatchObject({ status: 'applied', revision: 2, turn: 0 });
    expect(bought.state.hero.currency).toBe(opened.hero.currency - unit);
    expect(bought.state.items.find((entry) => entry.itemId === 'item.stock.ration')).toMatchObject({
      quantity: 1,
      location: { type: 'merchant-stock', populationId: POPULATION_ID },
    });
    expect(bought.state.items.find((entry) => entry.itemId === 'command.trade-buy')).toMatchObject({
      contentId: ration.id,
      quantity: 1,
      location: { type: 'backpack', actorId: HERO_ID },
    });
    expect(merchantPopulation(bought.state).stockItemIds).toEqual([
      'item.stock.lamp-oil',
      'item.stock.ration',
    ]);
    expect(bought.events).toEqual([
      {
        ...
        eventId: 'command.trade-buy',
        merchantPopulationId: POPULATION_ID,
        itemId: 'item.stock.ration',
        contentId: ration.id,
        quantity: 1,
        unitPrice: unit,
        total: unit,
        currency: opened.hero.currency - unit,
      },
    ]);
  });
  ...
  it('relocates a fully bought stack that cannot merge, keeping its item id', () => {
    ...
    const bought = resolveCommand(
      opened,
      buyCommand({ itemId: 'item.stock.ration', quantity: 2 }),
      context(),
    );
  ...
  it.each([
    ...
    [
      'with a mismatched merchant',
      {},
      buyCommand({ merchantPopulationId: 'population.other' }),
      'trade.merchant-mismatch',
      true,
    ],
    [
      'for an item outside the merchant stock',
      {},
      buyCommand({ itemId: 'item.hero.oil' }),
    ...
    [
      'with a conflicting split id',
      {},
      buyCommand({ commandId: 'item.hero.sword', itemId: 'item.stock.ration', quantity: 1 }),
      'item.id-conflict',
      true,
    ],
...
describe('trade-sell', () => {
  it('sells a full backpack stack into merchant stock at current-tier prices', () => {
    ...
    expect(sold.state.items.find((entry) => entry.itemId === 'item.hero.sword')).toMatchObject({
      location: { type: 'merchant-stock', populationId: POPULATION_ID },
      quantity: 1,
    });
    expect(merchantPopulation(sold.state).stockItemIds).toEqual([
      'item.hero.sword',
      'item.stock.lamp-oil',
      'item.stock.ration',
    ]);
  ...
  it('sells a partial stack into a new stock item derived from the command id', () => {
    ...
    const sold = resolveCommand(
      opened,
      sellCommand({ itemId: 'item.hero.oil', quantity: 1 }),
      context(),
    );
    expect(sold.result).toMatchObject({ status: 'applied' });
    expect(sold.state.hero.currency).toBe(opened.hero.currency + unit);
    expect(sold.state.items.find((entry) => entry.itemId === 'item.hero.oil')).toMatchObject({
      quantity: 1,
      location: { type: 'backpack', actorId: HERO_ID },
    });
    expect(sold.state.items.find((entry) => entry.itemId === 'command.trade-sell')).toMatchObject({
      contentId: lampOil.id,
      quantity: 1,
      location: { type: 'merchant-stock', populationId: POPULATION_ID },
    });
    ...
    const equipped = {
      ...
      actors: base.actors.map((actor) =>
        actor.actorId === HERO_ID
          ? { ...actor, equipment: { ...actor.equipment, 'main-hand': 'item.hero.sword' } }
          : actor,
      ),
      items: base.items.map((entry) =>
        entry.itemId === 'item.hero.sword'
          ? {
              ...entry,
              location: { type: 'equipped', actorId: HERO_ID, slot: 'main-hand' } as const,
            }
          : entry,
      ),
    };
  ...
  it("rejects an item in another actor's backpack", () => {
    ...
    const foreign = {
      ...
      items: base.items.map((entry) =>
        entry.itemId === 'item.hero.sword'
          ? { ...entry, location: { type: 'backpack', actorId: MERCHANT_ACTOR_ID } as const }
          : entry,
      ),
    };
  ...
  it.each([
    ...
    [
      'with a mismatched merchant',
      sellCommand({ merchantPopulationId: 'population.other' }),
      'trade.merchant-mismatch',
      true,
      {},
    ],
    ...
    [
      'for an unaccepted category',
      sellCommand({ itemId: 'item.hero.ring' }),
      'trade.item-unacceptable',
      true,
      {},
    ],
    [
      'for more than the held quantity',
      sellCommand({ itemId: 'item.hero.oil', quantity: 3 }),
      'item.quantity',
      true,
      {},
    ],
    ...
    [
      'when the payout would overflow currency',
      sellCommand(),
    ...
    [
      'with a conflicting split id',
      sellCommand({ commandId: 'item.stock.ration', itemId: 'item.hero.oil', quantity: 1 }),
      'item.id-conflict',
      true,
      {},
    ],
...
describe('trade-close and commerce consequence', () => {
    ...
    expect(closed.state.reputations).toEqual([
      {
        ...
        value: faction.startingReputation + encounter.definition.commerceReputationDelta,
      },
    ]);
    expect(closed.events).toEqual([
      {
        ...
        merchantPopulationId: POPULATION_ID,
        reason: 'player',
        completedCommerce: true,
      },
      {
        type: 'reputation.changed',
        eventId: 'command.trade-close',
        factionId: faction.id,
        previous: faction.startingReputation,
        delta: encounter.definition.commerceReputationDelta,
        value: faction.startingReputation + encounter.definition.commerceReputationDelta,
        reason: 'commerce',
      },
    ]);

    const reopened = resolveCommand(
      closed.state,
      openCommand({ commandId: 'command.trade-open-2', expectedRevision: 3 }),
      context(),
    ).state;
    const resold = resolveCommand(
      reopened,
      sellCommand({ commandId: 'command.trade-sell-2', expectedRevision: 4 }),
      context(),
    ).state;
    const closedAgain = resolveCommand(
      resold,
      closeCommand({ commandId: 'command.trade-close-2', expectedRevision: 5 }),
      context(),
    );
  ...
  it('rejects close without a session or against the wrong merchant', () => {
    const closedWithoutSession = resolveCommand(
      merchantRun(),
      closeCommand({ expectedRevision: 0 }),
      context(),
    );
    expect(closedWithoutSession.result).toMatchObject({
      status: 'invalid',
      reason: 'trade.required',
    });

    const opened = openedRun();
    const mismatched = resolveCommand(
      opened,
      closeCommand({ merchantPopulationId: 'population.other' }),
      context(),
    );
    expect(mismatched.result).toMatchObject({
      status: 'invalid',
      reason: 'trade.merchant-mismatch',
    });
  ...
  it('closes an invalidated session without granting the commerce bonus', () => {
    ...
    const merchantDead = {
      ...bought,
      actors: bought.actors.map((actor) =>
        actor.actorId === MERCHANT_ACTOR_ID ? { ...actor, health: 0 } : actor,
      ),
    };
    ...
    const moved = {
      ...bought,
      actors: bought.actors.map((actor) =>
        actor.actorId === MERCHANT_ACTOR_ID ? { ...actor, x: 4, y: 3 } : actor,
      ),
    };
  ...
  it('emits the automatic close before resolving the next ordinary command', () => {
    ...
    const waited = resolveCommand(
      merchantDead,
      { type: 'wait', commandId: 'command.wait-after-close', expectedRevision: 1 },
      context(),
    );
    ...
    expect(waited.events[0]).toEqual({
      type: 'trade.closed',
      eventId: 'command.wait-after-close',
      merchantPopulationId: POPULATION_ID,
      reason: 'death',
      completedCommerce: false,
    });
