...
function content(condition: ConditionContentEntry): CompiledContentPack {
  ...
  return {
    ...base,
    entries: [...base.entries.filter((entry) => entry.id !== condition.id), condition],
  };
}
...
function applyFixture(
  condition: ConditionContentEntry,
  actor = createDemoRun().actors[0]!,
  duration?: number,
) {
  return {
    actors: [actor],
...
describe('condition deadlines', () => {
  it('resolves traits and linear modifiers by content rather than condition ID', () => {
    ...
    expect(actorHasConditionTrait(actor, 'condition-trait.incapacitated', content(condition))).toBe(
      true,
    );
  ...
  it('rejects missing definitions, unsafe modifiers, and invalid saved instances', () => {
    ...
    expect(() =>
      actorHasConditionTrait(actor, 'condition-trait.incapacitated', createDemoContentPack()),
    ).toThrow(/condition\.missing.*definition/i);
    const unsafe = definition({
      id: 'condition.unsafe',
      modifiersPerStack: { defense: Number.MAX_SAFE_INTEGER },
      stacking: { mode: 'intensify', maximumStacks: 2 },
    });
    expect(() => conditionModifiers(actorWith(unsafe.id, 2), content(unsafe))).toThrow(
      /safe integer/i,
    );
    expect(() => validateActiveConditions([actorWith(unsafe.id, 3)], content(unsafe))).toThrow(
      /maximumStacks/i,
    );
  });
  ...
  it.each([
    ...
    'applies %s stacking with a refreshed source and deadline',
    (_label, stacking, beforeStacks, afterStacks) => {
  ...
  it('uses authored timed duration and permanent null deadlines', () => {
    ...
    expect(
      applyCondition(applyFixture(timed, createDemoRun().actors[0]!, 7)).actors[0]?.conditions[0]
        ?.expiresAt,
    ).toBe(17);
    ...
    expect(() => applyCondition(applyFixture(permanent, createDemoRun().actors[0]!, 1))).toThrow(
      /permanent.*duration/i,
    );
  });
  ...
  it('expires conditions at their absolute world-time deadline without mutation', () => {
    ...
    const result = advanceConditions({
      actors: [actor],
      worldTime: 12,
      eventId: 'event.condition',
    });
  ...
  it('retains permanent and future conditions', () => {
    ...
    const conditions = [
      ...
      {
        conditionId: 'condition.permanent',
        sourceActorId: null,
        appliedAt: 0,
        expiresAt: null,
        stacks: 1,
