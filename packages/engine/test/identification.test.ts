...
function pool(category: 'potion' | 'ring' = 'potion'): IdentificationPoolContentEntry {
  return {
    ...
    id: `identification-pool.${category}s`,
    name: `${category} unidentified names`,
    tags: [],
    category,
    verbs: ['Bubbling', 'Dancing', 'Smoking'],
    nouns: ['flask', 'phial', 'vial'],
    visuals: [
      { id: 'visual.blue', glyph: '!', color: '#4466aa' },
...
function contentWith(...items: readonly ItemContentEntry[]) {
  ...
  const pools = [...new Set(items.map((item) => item.category))].map((category) =>
    pool(category === 'ring' ? 'ring' : 'potion'),
  );
...
describe('per-run item identification', () => {
  it('allocates a stable bijection and advances only effects rng', () => {
    ...
    const names = Object.values(result.identification.appearanceByContentId).map(
      (appearanceId) => unidentifiedPresentation({ content, appearanceId }).name,
    );
    ...
    const alternate = allocateIdentificationMap({
      content,
      rng: {
        ...run.rng,
        effects: [11, 22, 33, 44],
    ...
    expect(Object.values(alternate.identification.appearanceByContentId)).not.toEqual(
      Object.values(result.identification.appearanceByContentId),
    );
  });
  ...
  it('allocates the identification map when a generated run is created', () => {
    ...
    expect(Object.keys(run.identification.appearanceByContentId)).toEqual([
      'item.harm',
      'item.heal',
    ]);
  ...
  it('rejects incomplete, duplicated, and unknown saved appearance mappings', () => {
    const content = contentWith(potion('item.heal'), potion('item.harm'));
    const allocated = allocateIdentificationMap({
      content,
      rng: createDemoRun().rng,
    }).identification;
    const [harmAppearance, healAppearance] = ['item.harm', 'item.heal'].map(
      (id) => allocated.appearanceByContentId[id]!,
    );
    const valid = {
      ...createDemoRun(),
      identification: { ...allocated, knownAppearanceIds: [healAppearance] },
    };
      ...
      validateContentBoundRun(
        {
          ...
          identification: {
            ...
            appearanceByContentId: { 'item.heal': healAppearance },
          },
        },
        content,
      ),
    ).toThrow(/identification map/i);
    expect(() =>
      validateContentBoundRun(
        {
          ...valid,
          identification: {
            ...valid.identification,
            appearanceByContentId: { 'item.harm': harmAppearance, 'item.heal': harmAppearance },
          },
        },
        content,
      ),
    ).toThrow(/unique names/i);
    expect(() =>
      validateContentBoundRun(
        {
          ...valid,
          identification: {
            ...valid.identification,
            knownAppearanceIds: ['appearance.unknown'],
          },
        },
        content,
      ),
    ).toThrow(/known appearance/i);
  });

  it('identifies one enchanted instance without revealing its twin', () => {
    const base = createDemoRun();
    const first: ItemInstance = {
      itemId: 'item.sword.1',
      contentId: 'item.sword',
      quantity: 1,
      condition: 100,
      enchantment: { enchantmentId: 'enchantment.fire', modifiers: { meleeAccuracy: 1 } },
      identified: false,
      charges: null,
      fuel: null,
      enabled: null,
    ...
    const result = identifyItem({
      run: { ...base, items: [first, second] },
      itemId: first.itemId,
      eventId: 'event.identify',
    });
    expect(result.state.items.find((item) => item.itemId === first.itemId)!.identified).toBe(true);
    expect(result.state.items.find((item) => item.itemId === second.itemId)!.identified).toBe(
      false,
    );
  });
  ...
  it('completely identifies a shuffled item revealing the appearance before the instance', () => {
    ...
    const result = identifyItemCompletely({
      run,
      content,
      itemId: item.itemId,
      eventId: 'event.identify',
    ...
    expect(result.events).toEqual([
      {
        ...
        eventId: 'event.identify',
        appearanceId: identification.appearanceByContentId[definition.id],
        contentId: definition.id,
      },
    ...
    expect(result.state.identification.knownAppearanceIds).toEqual([
      identification.appearanceByContentId[definition.id],
    ]);
  ...
  it('completely identifying an identified item is a no-op', () => {
    ...
    const run = {
      ...
      identification: {
        ...identification,
        knownAppearanceIds: [identification.appearanceByContentId[definition.id]!],
      },
    };
    const result = identifyItemCompletely({
      run,
  ...
  it('projects shuffled appearance without hidden content or enchantment', () => {
    ...
    const item: ItemInstance = {
      ...
      quantity: 2,
      condition: 100,
      enchantment: { enchantmentId: 'enchantment.secret', modifiers: { defense: 1 } },
      identified: false,
      charges: null,
      fuel: null,
      enabled: null,
    ...
    expect(projected).toMatchObject({
      name: expect.stringMatching(/^(Bubbling|Dancing|Smoking) (flask|phial|vial)$/),
  ...
  it('projects an instance-identified item under its random run name until identified', () => {
    ...
    const projected = projectItem({
      run: { ...createDemoRun(), items: [item], identification },
      content,
      itemId: item.itemId,
    });
