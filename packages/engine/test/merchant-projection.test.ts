...
describe('active trade projection', () => {
  ...
  it('exposes exact currency, stock quotes, sale offers, and service targets in a valid session', () => {
    ...
    expect(projected.trade?.stock[0]?.item).toEqual(
      projectItem({ run: runWithTrade, content, itemId: stockId }),
    );
    expect(projected.trade?.stock).toEqual([
      {
        item: projectItem({ run: runWithTrade, content, itemId: 'item.stock.lamp-oil' }),
        quantity: 4,
        unitPrice: quoteMerchantPurchase({
          basePrice: lampOil.price,
          merchantBps: encounter.definition.merchantSaleBps,
          factionBps: tier.purchasePriceBps,
        }),
      },
      {
        item: projectItem({ run: runWithTrade, content, itemId: 'item.stock.ration' }),
        quantity: 2,
        unitPrice: quoteMerchantPurchase({
          basePrice: ration.price,
          merchantBps: encounter.definition.merchantSaleBps,
          factionBps: tier.purchasePriceBps,
        }),
      },
    ...
    expect(projected.trade?.services).toEqual([
      {
        ...
        unitPrice: quoteMerchantService({
          basePrice: population.services[0]!.basePrice,
          factionBps: tier.purchasePriceBps,
        }),
  ...
  it('projects unidentified stock as its appearance-only representation', () => {
    ...
    const projected = projectGameplayState({ state: runWithTrade, content });
    const ring = projected.trade?.stock.find(
      (entry) => (entry.item as { itemId?: string }).itemId === 'item.stock.ring',
    );
    expect(ring).toBeDefined();
    expect(ring?.item).toEqual(
      projectItem({ run: runWithTrade, content, itemId: 'item.stock.ring' }),
    );
  ...
  it('is absent when the modal session invariants no longer hold', () => {
    ...
    const separated: ActiveRun = {
      ...
      actors: opened.actors.map((actor) =>
        actor.actorId === HERO_ID ? { ...actor, x: 4, y: 3 } : actor,
      ),
    };
