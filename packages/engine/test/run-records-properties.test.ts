...
function template(): FallenChampionTemplateContentEntry {
  return {
    ...
    id: 'fallen-champion-template.property',
    name: 'Property Champion',
    tags: [],
    fallbackMonsterId: 'monster.property',
    fallbackItemId: FALLBACK_ITEM_ID,
    minimumHealth: 18,
    maximumHealth: 180,
    attributeMaximum: 30,
    damageMaximum: 30,
    ...
    echoAbilityLimit: 2,
    echoLootTableId: 'loot-table.property-echo',
    heirloomSelection: {
...
function echoLootTable(): LootTableContentEntry {
  return {
    kind: 'loot-table',
    id: 'loot-table.property-echo',
    name: 'Property Echo',
    tags: [],
    rolls: 1,
    choices: [
      {
        contentId: FALLBACK_ITEM_ID,
        lootTableId: null,
        weight: 1,
        minimumQuantity: 1,
        maximumQuantity: 1,
...
function scenarioRun(scenario: MerchantPropertyScenario, pack: CompiledContentPack): ActiveRun {
  ...
  const run: ActiveRun = {
    ...
    actors: [
      { ...heroActor, equipment: { ...heroActor.equipment, 'main-hand': EQUIPPED_ITEM_ID } },
    ],
  ...
  const stockDefinition = pack.entries.find(
    (entry) => entry.kind === 'item' && entry.id === 'item.property-stock',
  );
  ...
  const heroItems: ItemInstance[] = [
    ...
    {
      itemId: SELL_ITEM_ID,
      contentId: 'item.property-stock',
      quantity: Math.min(3, stackLimit),
      condition: 100,
      enchantment: null,
      identified: true,
      charges: null,
    ...
    {
      itemId: TRINKET_ITEM_ID,
      contentId: 'item.property-trinket',
      quantity: 1,
      condition: 100,
      enchantment: null,
      identified: false,
  ...
  return {
    ...
    items: [...materialized.items, ...heroItems].sort((left, right) =>
...
function planCommand(
  state: ActiveRun,
  scenario: MerchantPropertyScenario,
  index: number,
): GameCommand {
  const plan = scenario.plans[index]!;
  const common = {
    commandId: `command.run-records-property-${index}`,
    expectedRevision: state.revision,
  };
  const merchant = merchantPopulation(state);
  if (plan.kind === 'open')
    return { ...common, type: 'trade-open', merchantActorId: MERCHANT_ACTOR_ID };
  if (plan.kind === 'close')
    return { ...common, type: 'trade-close', merchantPopulationId: merchant.populationId };
  if (plan.kind === 'buy') {
  ...
  if (plan.kind === 'attack')
    return { ...common, type: 'attack', targetActorId: MERCHANT_ACTOR_ID };
...
function assertMetricsValid(metrics: RunMetrics): void {
  for (const counter of METRIC_COUNTERS) {
    expect(
      Number.isSafeInteger(metrics[counter]) && metrics[counter] >= 0,
...
function execute(
  ...
  initial: ActiveRun,
  reload: boolean,
  observe?: (state: ActiveRun, previous: ActiveRun) => void,
): Execution {
  let state = initial;
...
function forceHeroDeath(state: ActiveRun, pack: CompiledContentPack): ActiveRun {
  const dying: ActiveRun = {
    ...
    actors: state.actors.map((actor) =>
      actor.actorId === state.hero.actorId ? { ...actor, health: 1 } : actor,
    ),
    survival: {
      ...state.survival,
      hungerReserve: 0,
      hungerStage: 'starving',
      nextStarvationAt: state.worldTime + 1,
    },
  };
  const killing = resolveCommand(
    dying,
    {
      type: 'wait',
      commandId: 'command.run-records-property-death',
      expectedRevision: dying.revision,
    },
    { content: pack },
  );
...
describe('mixed ordinary/trade run-records properties', () => {
  it('holds every run-records invariant across 512 seeded scenarios', () => {
    fc.assert(
      fc.property(merchantPropertyScenarioArbitrary, (scenario) => {
        ...
        const continuous = execute(scenario, pack, initial, false, (state) => {
          ...
          const heroAlive =
            state.actors.find((actor) => actor.actorId === state.hero.actorId)!.health > 0;
        ...
        expect(
          concluded.actors.find((actor) => actor.actorId === concluded.hero.actorId)!.health,
        ).toBe(0);
        ...
        for (const command of [
          {
            type: 'wait' as const,
            commandId: 'command.after-1',
            expectedRevision: concluded.revision,
          },
          {
            type: 'move' as const,
            direction: 'east' as const,
            commandId: 'command.after-2',
            expectedRevision: concluded.revision,
          },
        ]) {
        ...
        const finalizedB = finalizeRun({
          run: decodeActiveRun(encodeActiveRun(concluded)),
          content: pack,
          lifetime: createInMemoryRunRecordRepository().lifetime(),
        });
        ...
        if (heirloom.sourceItemId !== null) {
          const source = savedConcluded.items.find(
            (item) => item.itemId === heirloom.sourceItemId,
          )!;
