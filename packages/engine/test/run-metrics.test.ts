...
const ACTOR_FIXTURES: Readonly<
  Record<string, Readonly<{ contentId: string; populationId: string | null }>>
> = {
  'actor.individual-member': {
    contentId: 'monster.individual-foe',
    populationId: 'population.individual-1',
  },
  ...
  'actor.champion-actor': {
    contentId: 'monster.individual-foe',
    populationId: 'population.champion-1',
  },
...
function fixtureState(): ActiveRun {
  ...
  const individual: IndividualPopulation = {
    model: 'individual',
    populationId: 'population.individual-1',
    encounterId: 'encounter.individual',
    floorId: base.activeFloorId,
    createdAt: 0,
    livingMemberIds: [],
    formerMemberIds: ['actor.individual-member'],
  };
  const group: GroupPopulation = {
    model: 'group',
    populationId: 'population.group-1',
    encounterId: 'encounter.group',
    floorId: base.activeFloorId,
    createdAt: 0,
    livingMemberIds: [],
    formerMemberIds: ['actor.group-member'],
    leaderActorId: null,
    bonusActive: false,
    roleMembership: [],
    sharedKnowledge: [],
  ...
  const swarm: SwarmPopulation = {
    model: 'swarm',
    populationId: 'population.swarm-1',
    encounterId: 'encounter.swarm',
    floorId: base.activeFloorId,
    createdAt: 0,
    livingMemberIds: [],
    formerMemberIds: ['actor.swarm-member'],
    sourceActorId: 'actor.swarm-member',
    nextSpawnAt: 0,
    spawnedCount: 0,
    peakLivingSize: 1,
    shutdownState: null,
  ...
  const boss: BossPopulation = {
    model: 'boss',
    populationId: 'population.boss-1',
    encounterId: 'encounter.boss',
    floorId: base.activeFloorId,
    createdAt: 0,
    livingMemberIds: [],
    formerMemberIds: ['actor.boss-actor'],
    actorId: 'actor.boss-actor',
    currentPhaseId: null,
    crossedPhaseIds: [],
  ...
  const champion: ChampionPopulation = {
    ...
    createdAt: 0,
    livingMemberIds: [],
    formerMemberIds: ['actor.champion-actor'],
    actorId: 'actor.champion-actor',
    hallRecordId: 'hall.champion-1',
    rank: 1,
    defeated: true,
    rewardCreated: false,
    equipmentContentIds: [],
...
function diedEvent(input: Readonly<{ actorId: string; killerActorId: string }>): DomainEvent {
  ...
  return {
    type: 'actor.died',
    eventId: `event.${input.actorId}.died`,
...
describe('foldRunMetrics', () => {
  ...
  it('leaves every counter unchanged for a non-hero kill', () => {
    const folded = fold([
      diedEvent({ actorId: 'actor.group-member', killerActorId: 'actor.individual-member' }),
    ]);
    ...
    const folded = fold([
      {
        ...
        eventId: 'event.damage',
        actorId: HERO,
        sourceActorId: 'actor.group-member',
        amount: 5,
        health: 15,
      },
    ]);
    ...
    const folded = fold([
      {
        type: 'trade.bought',
        eventId: 'event.buy',
        merchantPopulationId: 'population.merchant-1',
        itemId: 'item.a',
        contentId: 'item.a',
        quantity: 3,
        unitPrice: 10,
  ...
  it('credits hero feature.revealed to discoveriesRevealed', () => {
    const folded = fold([
      { type: 'feature.revealed', eventId: 'event.reveal', actorId: HERO, featureId: 'feature.a' },
    ]);
  ...
  it('throws when a counter would exceed safe integer arithmetic', () => {
    ...
    expect(() =>
      fold([diedEvent({ actorId: 'actor.group-member', killerActorId: HERO })], saturated),
