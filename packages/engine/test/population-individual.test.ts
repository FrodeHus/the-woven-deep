...
describe('individual population behavior', () => {
  it('holds while unaware', () => {
    ...
    expect(
      chooseBehaviorAction({
        state: { ...run, actors: [run.actors[0]!, actor] },
  ...
  it('saves a patrol goal and truthful broad intent before applying its move', () => {
    ...
    const result = resolveWorldStep({
      state: { ...run, floors: [darkFloor], actors: [run.actors[0]!, actor] },
      content,
      eventId: 'event.patrol-state',
  ...
  it.each([
    ...
    'holds with saved hold intent for a %s patrol waypoint outside its floor',
    (_label, waypoint) => {
      const run = createDemoRun();
      const darkFloor = { ...run.floors[0]!, ambient: { color: [0, 0, 0] as const, strength: 0 } };
      ...
      const content = pack(
        definition(actor.contentId, {
          behaviorId: 'behavior.patrol',
          behaviorParameters: { waypoints: [waypoint] },
        }),
      );
      const result = resolveWorldStep({
        state: { ...run, floors: [darkFloor], actors: [run.actors[0]!, actor] },
        content,
        eventId: `event.patrol-${_label}`,
        action: { type: 'wait', actorId: run.hero.actorId, cost: 100 },
      });
      expect(
        result.state.actors.find((candidate) => candidate.actorId === actor.actorId),
  ...
  it('approaches a directly observed hostile and saves memory, goal, and intent before movement', () => {
    ...
    expect(saved.behaviorState).toMatchObject({
      intent: 'approach',
      goal: { type: 'actor', targetActorId: run.hero.actorId },
      lastKnownTargets: [
        { targetActorId: run.hero.actorId, x: 1, y: 1, observedAt: 0, source: 'sight' },
      ],
    });
    expect(result.events.map((event) => event.type)).toEqual([
      'hero.waited',
    ...
    expect(JSON.stringify(result.publicEvents)).not.toMatch(
      /lastKnownTargets|observerActorId|"goal"|"path"/,
    );
  });
  ...
  it('attacks an adjacent directly observed hostile', () => {
    ...
    expect(
      result.events.some((event) => event.type === 'attack.hit' || event.type === 'attack.missed'),
    ).toBe(true);
    expect(
      result.state.actors.find((candidate) => candidate.actorId === actor.actorId)?.behaviorState
        .intent,
    ).toBe('attack');
  });
  ...
  it('holds after hostility changes even while the former target remains visible', () => {
    const run = createDemoRun();
    const actor = monster('monster.pacified', {
      energy: 100,
      behaviorState: {
        intent: 'approach',
        goal: { type: 'actor', targetActorId: run.hero.actorId },
    ...
    const result = resolveWorldStep({
      state: {
        ...run,
        actors: [run.actors[0]!, actor],
        relationships: [
          {
            leftActorId: run.hero.actorId,
            rightActorId: actor.actorId,
            relationship: 'neutral',
          },
        ],
      },
      content: pack(definition(actor.contentId)),
      eventId: 'event.pacified',
      action: { type: 'wait', actorId: run.hero.actorId, cost: 100 },
    });
    expect(
      result.state.actors.find((candidate) => candidate.actorId === actor.actorId),
  ...
  it('investigates a last-known cell without tracking the unseen target', () => {
    const run = createDemoRun();
    const actor = monster('monster.investigator', {
      x: 4,
      y: 3,
      behaviorState: {
        intent: 'approach',
        goal: { type: 'cell', floorId: 'floor.demo', x: 1, y: 3 },
        lastKnownTargets: [
          {
            targetActorId: run.hero.actorId,
            floorId: 'floor.demo',
            x: 1,
            y: 3,
            observedAt: 4,
            source: 'sound',
            observerActorId: 'monster.investigator',
          },
        ],
  ...
  it('abandons a search after reaching the last-known cell', () => {
    ...
    const actor = monster('monster.searcher', {
      x: 3,
      y: 3,
      energy: 100,
      behaviorState: {
        ...
        goal: { type: 'cell', floorId: 'floor.demo', x: 3, y: 3 },
        lastKnownTargets: [],
        investigation: { floorId: 'floor.demo', x: 3, y: 3, startedAt: 0, expiresAt: 20 },
      },
    });
    const result = resolveWorldStep({
      state: { ...run, floors: [dark], actors: [run.actors[0]!, actor] },
      content: pack(definition(actor.contentId)),
      eventId: 'event.search-complete',
      action: { type: 'wait', actorId: run.hero.actorId, cost: 100 },
    });
    expect(
      result.state.actors.find((candidate) => candidate.actorId === actor.actorId)?.behaviorState,
  ...
  it('holds when its investigation goal is unreachable', () => {
    const run = createDemoRun();
    const actor = monster('monster.trapped', {
      x: 1,
      y: 1,
      behaviorState: {
        intent: 'approach',
        goal: { type: 'cell', floorId: 'floor.demo', x: 5, y: 3 },
        lastKnownTargets: [],
        investigation: { floorId: 'floor.demo', x: 5, y: 3, startedAt: 0, expiresAt: null },
      },
    });
    const floor = {
      ...run.floors[0]!,
      tiles: run.floors[0]!.tiles.map((tile, index) => (index === 9 || index === 15 ? 0 : tile)),
    };
    expect(
      chooseBehaviorAction({
        state: { ...run, floors: [floor], actors: [actor] },
        actorId: actor.actorId,
        content: pack(definition(actor.contentId)),
      }),
    ).toMatchObject({ type: 'wait' });
  ...
  it('freezes actors on inactive floors', () => {
    ...
    expect(
      result.state.actors.find((candidate) => candidate.actorId === actor.actorId)?.energy,
    ).toBe(100);
  });
  ...
  it('marks an encounter only after the hero legitimately observes a member', () => {
    ...
    const actor = monster('monster.population', {
      ...
      y: 1,
      energy: 0,
      populationId: 'population.visible',
    ...
    const result = resolveWorldStep({
      ...
      content: pack(definition(actor.contentId), entry),
      eventId: 'event.encounter-observed',
      action: { type: 'wait', actorId: run.hero.actorId, cost: 100 },
    });
  ...
  it('does not mark generated membership encountered while every member is dark and unseen', () => {
    ...
    const actor = monster('monster.hidden-population', {
      ...
      y: 1,
      energy: 0,
      populationId: 'population.hidden',
      populationPresentation: {
        name: 'Hidden monster',
        glyph: 'm',
        color: '#aa4444',
    ...
    const result = resolveWorldStep({
      ...
      content: pack(definition(actor.contentId), entry),
      eventId: 'event.encounter-unseen',
      action: { type: 'wait', actorId: run.hero.actorId, cost: 100 },
    });
  ...
  it('moves a dead individual-population member into formerMemberIds and keeps the run savable', () => {
    ...
    const actor = monster('monster.individual-victim', {
      ...
      health: 1,
      maxHealth: 1,
      populationId: 'population.individual-victim',
      populationPresentation: {
        name: 'Victim monster',
        glyph: 'm',
        color: '#aa4444',
    ...
    const initial: ActiveRun = {
      ...run,
      actors: [hero, actor],
      rng: { ...run.rng, combat: combatStateProducing(20) },
      encounterDecisions: [
        {
          encounterId: entry.id,
          baseProbability: 1,
    ...
    const resolution = resolveCommand(
      ...
      {
        type: 'attack',
        commandId: 'command.individual-kill',
        expectedRevision: 0,
    ...
    expect(
      resolution.state.actors.find((candidate) => candidate.actorId === actor.actorId)?.health,
    ).toBeLessThanOrEqual(0);
    const population = resolution.state.populations.find(
      (candidate) => candidate.populationId === 'population.individual-victim',
    )!;
