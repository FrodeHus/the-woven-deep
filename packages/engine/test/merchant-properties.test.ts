...
function planCommand(
  ...
  const common = {
    commandId: `command.merchant-property-${index}`,
  ...
  it('keeps every merchant invariant across 512 seeded mixed-command scenarios', () => {
    fc.assert(
      fc.property(merchantPropertyScenarioArbitrary, (scenario) => {
        ...
        const continuous = execute(
          scenario,
          pack,
          initial,
          false,
          (state, events, command, previous) => {
            ...
            for (const event of events) {
              if (
                event.type === 'trade.bought' ||
                event.type === 'trade.sold' ||
