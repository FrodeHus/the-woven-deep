...
describe('mutable dungeon features', () => {
  it('changes movement, sight, and light geometry when a door opens', () => {
    ...
    const perception = refreshKnowledge({
      floor: { ...floor, tiles: featureTiles(opened.run, floor.floorId) },
  ...
  it('refuses to close an occupied doorway without changing the run', () => {
    ...
    const occupant = {
      ...hero,
      actorId: 'monster.occupant',
      contentId: 'monster.occupant',
    ...
    expect(closeDoor({ run, actorId: hero.actorId, featureId: 'door.1' })).toEqual({
      ok: false,
      reason: 'door.occupied',
    });
  });
  ...
  it('applies an open-door command atomically and keeps the result saveable', () => {
    ...
    const result = resolveCommand(
      { ...base, actors: [hero], features: [door()] },
      {
        type: 'open-door',
  ...
  it('searches through the timed command path and saves revealed geometry', () => {
    const base = createDemoRun();
    const feature = {
      featureId: 'secret.easy',
      type: 'secret' as const,
      floorId: 'floor.demo',
      x: 2,
      y: 1,
      contentId: null,
      coverTileId: 0 as const,
  ...
  it('saves an actor standing in a revealed secret passage', () => {
    ...
    const feature = {
      ...
      discoveryDifficulty: 1,
      discovery: {
        discoveredByActorIds: ['hero.demo'],
        progressByActorId: { 'hero.demo': 1 },
  ...
  it('records one passive contribution per saved illumination context', () => {
    ...
    const feature = {
      ...
      coverTileId: 0 as const,
      state: 'hidden' as const,
      discoveryDifficulty: 100,
      discovery: { discoveredByActorIds: [], progressByActorId: {}, attemptedContextKeys: [] },
    };
    const input = {
      run: { ...base, features: [feature] },
      actorId: 'hero.demo',
      illumination: 128,
    ...
    expect(applyPassiveDiscovery({ ...input, run: once.run }).run.features).toEqual(
      once.run.features,
    );
  ...
  it('reveals a hidden trap before its triggered effects', () => {
    const definition: TrapContentEntry = {
      kind: 'trap',
      id: 'trap.dart',
      name: 'Dart trap',
      glyph: '^',
  ...
  it("folds the hero's statModifiers into disarm rolls", () => {
    ...
    const baseline = disarmTrap({
      run: { ...base, features: [feature] },
      content,
      actorId: 'hero.demo',
      featureId: feature.featureId,
      eventId: 'event.disarm-baseline',
    });
