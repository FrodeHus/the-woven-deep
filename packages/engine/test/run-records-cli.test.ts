...
const reviewedHashesPath = fileURLToPath(new URL('./fixtures/run-records-demo-hashes.json', import.meta.url));

function runDemo(...arguments_: string[]) {
  return spawnSync(process.execPath, [script, ...arguments_], { cwd: repositoryRoot, encoding: 'utf8' });
}
...
describe('run records demonstration CLI', () => {
  it('repeats reviewed hashes across separate processes and visibly proves every milestone outcome', () => {
    ...
    expect(first.stdout).toMatch(/leader group felled by a production attack[\s\S]*group\.leader-defeated/);
    expect(first.stdout).toMatch(/swarm contained by destroying its source[\s\S]*swarm\.source-destroyed/);
    expect(first.stdout).toMatch(/rare boss encountered and phase-changed[\s\S]*boss\.phase-changed/);
    expect(first.stdout).toMatch(/travelling merchant trade session[\s\S]*merchantPopulationId/);
    expect(first.stdout).toMatch(/merchant provoked by a production attack[\s\S]*merchant\.provoked/);
    ...
    expect(Object.keys(reviewed).sort()).toEqual(['eventHash', 'projectionHash', 'recordHash', 'saveHash']);
  ...
  it('never prints hidden run-records or scheduling state', () => {
    ...
    for (const field of ['fallenHeroDecisions', 'encounterDecisions', 'concludedAtRevision',
  ...
  it('rejects a reviewed-hash mismatch when the content drifts', () => {
    ...
    try {
      ...
      expect(result.stderr).toContain('run records demo failed: reviewed run records demo hashes do not match');
    } finally {
