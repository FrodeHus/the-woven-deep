...
describe('full floor generation', () => {
  it('publishes exact stable floor and report bytes with a required authored vault', () => {
    ...
    expect(
      first.floor.tiles[first.floor.stairUp!.y * first.floor.width + first.floor.stairUp!.x],
    ).toBe(4);
    expect(
      first.floor.tiles[first.floor.stairDown!.y * first.floor.width + first.floor.stairDown!.x],
    ).toBe(5);
    expect(
      analyzeConnectivity({
        width: first.floor.width,
        height: first.floor.height,
        tiles: first.floor.tiles,
        start: first.floor.stairUp!,
        target: first.floor.stairDown!,
      }).connected,
    ).toBe(true);
    ...
    const expected = readFileSync(
      new URL('./fixtures/generated-floor-seed-1.json', import.meta.url),
      'utf8',
    ).trimEnd();
  ...
  it('shares bounded retries with required population rejection before using the fallback', () => {
    ...
    const required = {
      ...source,
      placement: { ...source.placement, failureMode: 'required' as const },
    };
    ...
    expect(
      generated.populationPlacement?.status === 'placed'
        ? generated.populationPlacement.createdActors.length
        : 0,
    ).toBeGreaterThan(0);
  });
});
