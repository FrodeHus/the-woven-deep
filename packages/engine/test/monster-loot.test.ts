...
import type { CompiledContentPack, ItemContentEntry, LootTableContentEntry, MonsterContentEntry } from '@woven-deep/content';
import { createDemoContentPack, createDemoRun, dropMonsterLoot, type ActiveRun, type ActorState } from '../src/index.js';

function itemDef(id: string): ItemContentEntry {
  return { kind: 'item', id, name: id, tags: [], glyph: '*', color: '#ffaa00', category: 'misc',
    stackLimit: 10, price: 1, rarity: 'common', minDepth: 1, maxDepth: 20, actionCost: 100,
    equipment: null, combat: null, light: null, identification: { mode: 'known', poolId: null }, effects: [] };
}

const lootTable: LootTableContentEntry = { kind: 'loot-table', id: 'loot-table.cave-rat', name: 'Cave rat loot',
  tags: [], rolls: 1, choices: [
    { contentId: 'item.rat-tail', lootTableId: null, weight: 1, minimumQuantity: 1, maximumQuantity: 1 },
  ] };

function monster(overrides: Readonly<{ lootTableId: string | null; dropChance: number }>): MonsterContentEntry {
  return {
    kind: 'monster', id: 'monster.cave-rat', name: 'Cave rat', tags: [], glyph: 'r', color: '#8a7766',
    attributes: { might: 4, agility: 8, vitality: 4, wits: 2, resolve: 2 }, health: 6,
    speed: 100, accuracy: 4, defense: 2, perception: 6, damage: { count: 1, sides: 3, bonus: 0 }, armor: 0,
    ...
    minDepth: 1, maxDepth: 20, threat: 1, rarity: 'common',
    lootTableId: overrides.lootTableId, dropChance: overrides.dropChance,
  };
}

function fixtureWith(overrides: Readonly<{ lootTableId: string | null; dropChance: number }>): Readonly<{
  run: ActiveRun; state: ActiveRun; content: CompiledContentPack; deadActor: ActorState;
}> {
  const base = createDemoRun();
  const monsterEntry = monster(overrides);
  const content: CompiledContentPack = { ...createDemoContentPack(),
    entries: [...createDemoContentPack().entries, monsterEntry, lootTable, itemDef('item.rat-tail')] };
