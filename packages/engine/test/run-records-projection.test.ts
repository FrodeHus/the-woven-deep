...
import {
  createDemoContentPack, createDemoRun, deriveHallRecordId, emptyRunMetrics, finalizeRun,
  projectGameplayState, projectRunConclusion, stableJson,
  type ActiveRun, type LifetimeState,
} from '../src/index.js';
...
function emptyLifetime(): LifetimeState {
  return { conqueredChampionRecordIds: [], grantedAchievementIds: [], discoveryProtection: [], totals: emptyRunMetrics() };
}
...
describe('projectRunConclusion', () => {
  it('returns null while the run has not concluded', () => {
    expect(projectRunConclusion({ run: createDemoRun(), record: null, achievements: [] })).toBeNull();
  });
  ...
  it('exposes completion facts and metrics, but no score or heirloom, before finalization', () => {
    ...
    expect(projected).toMatchObject({
      completionType: 'died', cause: run.conclusion!.cause, metrics: run.metrics,
      finalized: false, score: null, heirloom: null, achievements: [],
    });
  });
  ...
  it('exposes the full score breakdown, heirloom, and achievement grants once finalized', () => {
    ...
    const projected = projectRunConclusion({ run: finalized.run, record: finalized.record, achievements: grants });
    expect(projected).toMatchObject({
  ...
  it('throws when the supplied record does not carry this run\'s derived record ID', () => {
    ...
    const run = concludedRun();
    const finalized = finalizeRun({ run, content, lifetime: emptyLifetime() });
    const mismatched = { ...finalized.record, recordId: deriveHallRecordId([9, 9, 9, 9], run.contentHash) };
    expect(() => projectRunConclusion({ run: finalized.run, record: mismatched, achievements: [] }))
      .toThrow(/record/i);
  });
});
