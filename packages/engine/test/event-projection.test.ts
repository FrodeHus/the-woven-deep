...
function fixture() {
  ...
  const content = {
    ...createDemoContentPack(),
    entries: [...createDemoContentPack().entries, monsterDefinition()],
  };
  ...
  return {
    state: { ...base, floors: [floor], actors: [base.actors[0]!, attacker] },
    content,
    events,
  };
}
...
function merchantEventFixture(visible: boolean) {
  ...
  const population: MerchantPopulation = {
    populationId: 'population.merchant',
    encounterId: 'encounter.merchant',
    floorId: base.activeFloorId,
    createdAt: 0,
    livingMemberIds: [merchant.actorId],
    formerMemberIds: [],
    model: 'merchant',
    actorId: merchant.actorId,
    npcId: merchant.contentId,
  ...
  const events: DomainEvent[] = [
    ...
    },
    {
      ...
      populationId: population.populationId,
      actorId: merchant.actorId,
      sourceActorId: base.hero.actorId,
      response: 'flee',
    },
    {
    ...
    },
    {
      ...
      populationId: population.populationId,
      actorId: merchant.actorId,
      killerActorId: base.hero.actorId,
      destroyedStockItemIds: ['item.destroyed-secret'],
    },
  ];
...
describe('public event projection', () => {
  ...
  it('projects broad intent only for a currently visible actor', () => {
    ...
    expect(
      projectDomainEvents({ ...visible, events: [event], heroId: visible.state.hero.actorId }),
    ).toEqual([event]);
  });
    ...
    const events: DomainEvent[] = [
      {
        ...
        floorId: visible.state.activeFloorId,
        model: 'group',
        actorIds: [actorId, 'actor.unseen'],
      },
      {
        type: 'population.encountered',
        eventId,
        populationId: 'population.secret',
        encounterId: 'encounter.secret',
        actorId,
      ...
      },
      {
        ...
        populationId: 'population.secret',
        actorId,
        targetActorId: visible.state.hero.actorId,
        floorId: visible.state.activeFloorId,
        x: 9,
        y: 9,
        observedAt: 99,
        observerActorId: 'actor.observer-secret',
      },
      {
      ...
      },
      {
        ...
        actorId,
        encounterId: 'encounter.secret',
        uniqueItemId: 'item.unopened-secret',
        itemIds: ['item.roll-secret'],
      },
      {
      ...
      },
      {
        ...
        rank: 1,
        itemId: 'item.secret',
        contentId: 'item.content-secret',
        originatingHallRecordId: 'hall.secret',
        displayName: 'Observed heirloom',
        glyph: ')',
        color: '#ffffff',
        fallback: false,
      },
  ...
  it('never publishes combat rolls even when both participants are visible', () => {
    ...
    const output = projectDomainEvents({
      ...visible,
      events: [visible.events[0]!],
      heroId: visible.state.hero.actorId,
    });
    expect(output).toEqual([
      {
        ...
        eventId: 'command.wait',
        outcome: 'hit',
        attackerActorId: 'monster.hidden',
        targetActorId: visible.state.hero.actorId,
        attackerName: 'Hidden monster',
      },
    ]);
  ...
  it('uses both prior and next observation without revealing the hidden endpoint', () => {
    ...
    const state = {
      ...input.state,
      hero: { ...input.state.hero, sightRadius: 1 },
      floors: [
  ...
  it('redacts hidden damage and death sources while retaining a visible victim transition', () => {
    ...
    const state = {
      ...input.state,
      hero: { ...input.state.hero, sightRadius: 2 },
      floors: [
        { ...input.state.floors[0]!, ambient: { color: [255, 255, 255] as const, strength: 255 } },
      ],
  ...
  it('suppresses hidden victim transitions even when the source is visible, and suppresses both hidden', () => {
    ...
    const heroKill: DomainEvent = {
      type: 'actor.died',
      eventId: 'event.hero-kill',
      actorId: hiddenVictim.actorId,
      contentId: hiddenVictim.contentId,
      killerActorId: state.hero.actorId,
    };
    const hiddenKill: DomainEvent = {
      ...heroKill,
      eventId: 'event.hidden-kill',
      killerActorId: hiddenKiller.actorId,
    };
    expect(
      projectDomainEvents({
        state,
        content: input.content,
        events: [heroKill, hiddenKill],
  ...
  it("suppresses an off-screen monster's loot.dropped while projecting a visible one", () => {
    const input = fixture();
    const droppedItemDefinition = {
      kind: 'item' as const,
      id: 'item.dropped-loot-content',
      name: 'Dropped loot',
      tags: [],
      glyph: '*',
      color: '#ffaa00',
    ...
    const content = {
      ...input.content,
      entries: [...input.content.entries, droppedItemDefinition],
    };
    const droppedItem = {
      itemId: 'item.dropped-loot',
      contentId: droppedItemDefinition.id,
      quantity: 1,
      condition: 100,
      enchantment: null,
      identified: true,
    ...
    expect(
      projectDomainEvents({
        ...
        content,
        events: [event],
        heroId: stateWithItem.hero.actorId,
      }),
    ).toEqual([]);
    const visible = {
      ...stateWithItem,
    ...
    expect(output).toEqual([
      {
        ...
        eventId: 'event.merchant',
        category: 'merchant-departure-warning',
        actorId: 'actor.merchant',
        presentation: 'merchant.departure-warning.500',
        displayName: 'Travelling Lampwright',
      },
      {
        type: 'population.notice',
        eventId: 'event.merchant',
        category: 'merchant-provoked',
        actorId: 'actor.merchant',
        presentation: 'merchant.provoked.flee',
        displayName: 'Travelling Lampwright',
      },
      {
        type: 'population.notice',
        eventId: 'event.merchant',
        category: 'merchant-stock-dropped',
        actorId: 'actor.merchant',
        presentation: 'merchant.stock-dropped',
        displayName: 'Travelling Lampwright',
      },
      {
        type: 'population.notice',
        eventId: 'event.merchant',
        category: 'merchant-died',
        actorId: 'actor.merchant',
        presentation: 'merchant.died',
        displayName: 'Travelling Lampwright',
      },
    ]);
    const json = stableJson(output);
  ...
  it('notices an encountered same-floor departure without stock, and suppresses off-floor departures', () => {
    const departed: DomainEvent = {
      ...
      populationId: 'population.merchant',
      actorId: 'actor.merchant',
      stockItemIds: ['item.unsold-secret'],
    };
    ...
    const gone = {
      ...
      state: {
        ...base.state,
        actors: base.state.actors.filter((actor) => actor.actorId !== 'actor.merchant'),
      },
      events: [departed],
    };
    expect(projectDomainEvents({ ...gone, heroId: gone.state.hero.actorId })).toEqual([
      {
        type: 'population.notice',
        eventId: 'event.merchant',
        category: 'merchant-departed',
        actorId: null,
        presentation: 'merchant.departed',
      },
    ]);
    const offFloor = {
      ...gone,
      state: {
        ...gone.state,
        populations: gone.state.populations.map((population) => ({
          ...population,
          floorId: 'floor.elsewhere',
        })),
      } as ActiveRun,
    };
    ...
    const unencountered = {
      ...
      state: {
        ...gone.state,
        encounterDecisions: gone.state.encounterDecisions.map((decision) => ({
          ...decision,
          encountered: false,
        })),
      },
    ...
    expect(
      projectDomainEvents({ ...unencountered, heroId: unencountered.state.hero.actorId }),
    ).toEqual([]);
  });
    ...
    const events: DomainEvent[] = [
      {
        type: 'trade.closed',
        eventId: 'event.merchant',
        merchantPopulationId: 'population.merchant',
        reason: 'departure',
