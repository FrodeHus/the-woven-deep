...
function houseDoorPosition(run: ActiveRun): Readonly<{ x: number; y: number }> {
  const slot = townFloor(run).placementSlots.find((candidate) => candidate.tags.includes('house-door'));
...
function atHouseDoor(run: ActiveRun): ActiveRun {
  ...
  return validateActiveRun({
    ...run,
    actors: run.actors.map((actor) => actor.actorId === hero.actorId ? { ...actor, ...cell } : actor),
  });
}
...
function farFromHouseDoor(run: ActiveRun): ActiveRun {
  ...
  for (let y = 0; y < floor.height; y += 1) {
    for (let x = 0; x < floor.width; x += 1) {
      ...
      return validateActiveRun({
        ...run,
        actors: run.actors.map((actor) => actor.actorId === hero.actorId ? { ...actor, x, y } : actor),
      });
    }
  }
...
function backpackItem(itemId: string, contentId: string, quantity: number, heroActorId: string,
  overrides: Partial<ItemInstance> = {}): ItemInstance {
  return {
    itemId, contentId, quantity, condition: 100, enchantment: null, identified: true,
    charges: null, fuel: null, enabled: null, location: { type: 'backpack', actorId: heroActorId }, ...overrides,
  };
}
...
function withItems(run: ActiveRun, items: readonly ItemInstance[]): ActiveRun {
  return validateActiveRun({
    ...run,
    items: [...run.items, ...items].sort((left, right) => left.itemId < right.itemId ? -1 : left.itemId > right.itemId ? 1 : 0),
  });
}
...
describe('house deposit/withdraw legality matrix', () => {
  it('rejects a deposit while off the town floor', () => {
    ...
    const withItem = withItems(base, [backpackItem('item.house-test.a', 'item.iron-sword', 1, hero.actorId)]);
    const stairDown = townFloor(withItem).stairDown!;
    const onStairs = validateActiveRun({
      ...withItem,
      actors: withItem.actors.map((actor) => actor.actorId === hero.actorId ? { ...actor, ...stairDown } : actor),
    });
    const notTown = descendToNextFloor(onStairs, context()).state;
    const resolved = resolveCommand(notTown, {
      type: 'house-deposit', commandId: 'command.deposit', expectedRevision: notTown.revision,
      itemId: 'item.house-test.a', quantity: 1,
    }, context());
  ...
  it('rejects a deposit while not adjacent to the house door', () => {
    const base = townRun();
    const hero = heroActor(base);
    const run = withItems(farFromHouseDoor(base), [backpackItem('item.house-test.a', 'item.iron-sword', 1, hero.actorId)]);
    const resolved = resolveCommand(run, {
      type: 'house-deposit', commandId: 'command.deposit', expectedRevision: run.revision,
      itemId: 'item.house-test.a', quantity: 1,
    }, context());
    expect(resolved.result).toMatchObject({ status: 'invalid', reason: 'item.unavailable' });
  });
  ...
  it('rejects a deposit that would exceed house capacity', () => {
    ...
    const filler = Array.from({ length: base.house.capacity }, (_, index) =>
      backpackItem(`item.house-full.${index}`, 'item.iron-sword', 1, 'house-owner', { location: { type: 'house' } }));
    const run = withItems(base, [...filler, backpackItem('item.house-test.a', 'item.wooden-shield', 1, hero.actorId)]);
    const resolved = resolveCommand(run, {
      type: 'house-deposit', commandId: 'command.deposit', expectedRevision: run.revision,
      itemId: 'item.house-test.a', quantity: 1,
    }, context());
  ...
  it('rejects a withdrawal that would exceed backpack capacity', () => {
    ...
    const run = withItems(base, [...filler, backpackItem('item.house-test.a', 'item.wooden-shield', 1, hero.actorId, { location: { type: 'house' } })]);
    const resolved = resolveCommand(run, {
      type: 'house-withdraw', commandId: 'command.withdraw', expectedRevision: run.revision,
      itemId: 'item.house-test.a', quantity: 1,
    }, context());
  ...
  it('splits a partial quantity on deposit and withdraw, leaving the remainder where it started', () => {
    ...
    const deposited = resolveCommand(run, {
      type: 'house-deposit', commandId: 'command.deposit-split', expectedRevision: run.revision,
    ...
    const carried = deposited.state.items.find((item) =>
      item.location.type === 'house' && item.contentId === 'item.wooden-arrows');
    expect(carried?.quantity).toBe(4);

    const withdrawn = resolveCommand(deposited.state, {
      type: 'house-withdraw', commandId: 'command.withdraw-split', expectedRevision: deposited.state.revision,
      itemId: carried!.itemId, quantity: 1,
    }, context());
  ...
  it('round-trips a whole enchanted stack through the house, preserving its identity exactly', () => {
    const base = atHouseDoor(townRun());
    const hero = heroActor(base);
    const enchanted: ItemInstance = backpackItem('item.house-test.enchanted', 'item.iron-sword', 1, hero.actorId, {
      enchantment: { enchantmentId: 'enchantment.test', modifiers: { defense: 2 } },
      condition: 87,
    });
    const run = withItems(base, [enchanted]);
    const deposited = resolveCommand(run, {
      type: 'house-deposit', commandId: 'command.deposit-whole', expectedRevision: run.revision,
      itemId: enchanted.itemId, quantity: 1,
    }, context());
    expect(deposited.result.status).toBe('applied');
    ...
    const withdrawn = resolveCommand(deposited.state, {
      type: 'house-withdraw', commandId: 'command.withdraw-whole', expectedRevision: deposited.state.revision,
      itemId: enchanted.itemId, quantity: 1,
    }, context());
    expect(withdrawn.result.status).toBe('applied');
    const backInBackpack = withdrawn.state.items.find((item) => item.itemId === enchanted.itemId)!;
    expect(backInBackpack).toEqual({ ...enchanted, location: { type: 'backpack', actorId: hero.actorId } });
  });
});
...
describe('house command reducer wiring', () => {
  it('is revision-only: no turn, worldTime, or survival change', () => {
    const base = atHouseDoor(townRun());
    const hero = heroActor(base);
    const run = withItems(base, [backpackItem('item.house-test.wire', 'item.iron-sword', 1, hero.actorId)]);
    const resolved = resolveCommand(run, {
      type: 'house-deposit', commandId: 'command.wire', expectedRevision: run.revision,
      itemId: 'item.house-test.wire', quantity: 1,
    }, context());
    expect(resolved.result).toMatchObject({ status: 'applied', revision: run.revision + 1, turn: run.turn });
  ...
  it('records the command and is replay-idempotent on a duplicate submission', () => {
    const base = atHouseDoor(townRun());
    const hero = heroActor(base);
    const run = withItems(base, [backpackItem('item.house-test.dup', 'item.iron-sword', 1, hero.actorId)]);
    const command = {
      type: 'house-deposit' as const, commandId: 'command.dup', expectedRevision: run.revision,
      itemId: 'item.house-test.dup', quantity: 1,
    };
