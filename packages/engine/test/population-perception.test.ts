...
describe('population perception and saved memory', () => {
  it('records only directly visible, illuminated targets', () => {
    const observations = visibleTargetObservations({
      observerActorId: 'monster.watcher',
      floorId: 'floor.one',
      width: 3,
      visibilityWords: [0b111],
      illuminationIntensity: [1, 0, 1],
      observedAt: 10,
      actors: [
  ...
  it('records perceivable sound and investigates its last known cell', () => {
    const observation = soundTargetObservation({
      observerActorId: 'monster.listener',
      targetActorId: 'hero.one',
      floorId: 'floor.one',
      x: 4,
      y: 2,
      observedAt: 20,
    });
    const state = updateActorMemory({
      state: emptyActorBehaviorState(),
      observations: [observation],
      investigationDuration: 100,
    });
    expect(state.lastKnownTargets).toEqual([observation]);
    expect(state.investigation).toEqual({
      floorId: 'floor.one',
      x: 4,
      y: 2,
      startedAt: 20,
  ...
  it('does not track a target that later moves unseen', () => {
    const seen = soundTargetObservation({
      observerActorId: 'monster.one',
      targetActorId: 'hero.one',
      floorId: 'floor.one',
      x: 2,
      y: 2,
    ...
    const first = updateActorMemory({
      state: emptyActorBehaviorState(),
      observations: [seen],
      investigationDuration: null,
    });
    const later = updateActorMemory({
      state: first,
      observations: [],
      investigationDuration: null,
    });
...
describe('merchant observation materializes faction reputation', () => {
  ...
  function merchantObservationRun(): ActiveRun {
    ...
    const occupied = new Set([
      ...
      ...base.features
        .filter((feature) => feature.floorId === floor.floorId)
    ...
    const merchantActor = {
      ...heroActor,
      actorId: 'actor.merchant.observed',
      ...
      populationId: 'population.merchant.observed',
    ...
    const stock = {
      itemId: 'item.merchant.observed.stock',
    ...
    const population = {
      populationId: merchantActor.populationId,
    ...
    return {
      ...base,
      actors: [...base.actors, merchantActor].sort((left, right) =>
        left.actorId < right.actorId ? -1 : left.actorId > right.actorId ? 1 : 0,
      ),
      items: [...base.items, stock].sort((left, right) =>
        left.itemId < right.itemId ? -1 : left.itemId > right.itemId ? 1 : 0,
      ),
      populations: [...base.populations, population].sort((left, right) =>
  ...
  it('materializes the authored starting reputation exactly once on first legitimate observation', () => {
    ...
    const wait: GameCommand = {
      type: 'wait',
      commandId: 'command.observe-merchant',
    ...
    expect(
      observed.state.encounterDecisions.find(
        (decision) => decision.encounterId === 'encounter.travelling-lampwright',
      )?.encountered,
    ).toBe(true);
    expect(observed.state.reputations).toEqual([
      { factionId: faction.id, value: faction.startingReputation },
    ]);

    const adjusted: ActiveRun = {
      ...observed.state,
      reputations: observed.state.reputations.map((entry) =>
        entry.factionId === faction.id ? { ...entry, value: 123 } : entry,
      ),
    };
    const second: GameCommand = {
      type: 'wait',
      commandId: 'command.observe-merchant-again',
  ...
  it('rejects a persisted state that observed a merchant without materializing its reputation', () => {
    const run = merchantObservationRun();
    const corrupted: ActiveRun = {
      ...run,
      encounterDecisions: run.encounterDecisions.map((decision) =>
        decision.encounterId === 'encounter.travelling-lampwright'
          ? { ...decision, encountered: true }
          : decision,
      ),
    };
    expect(() => validateContentBoundRun(corrupted, content)).toThrow(
      /reputation was never materialized/,
    );
    expect(() =>
      validateContentBoundRun(
        {
          ...corrupted,
          reputations: [{ factionId: faction.id, value: faction.startingReputation }],
        },
        content,
      ),
    ).not.toThrow();
