...
function withHeroHealth(state: ActiveRun, health: number): ActiveRun {
  return {
    ...
    actors: state.actors.map((actor) =>
      actor.actorId === state.hero.actorId ? { ...actor, health } : actor,
    ),
  };
}
...
describe('concludeRunOnHeroDeath', () => {
  ...
  it('leaves a living hero unchanged and appends no event', () => {
    ...
    const events: DomainEvent[] = [
      { type: 'hero.waited', eventId: 'command.alive', heroId: state.hero.actorId, x: 1, y: 1 },
    ];
  ...
  it('credits the killer from the last hero actor.died event and appends run.concluded', () => {
    ...
    const killingEvents: DomainEvent[] = [
      {
        type: 'attack.hit',
        eventId: 'command.fatal',
        actorId: killer.actorId,
        targetActorId: deadHeroState.hero.actorId,
        naturalRoll: 20,
        total: 30,
        defense: 8,
        critical: true,
      ...
      },
      {
        type: 'actor.damaged',
        eventId: 'command.fatal',
        actorId: deadHeroState.hero.actorId,
        sourceActorId: killer.actorId,
        amount: 20,
        health: 0,
      },
      {
        ...
        eventId: 'command.fatal',
        actorId: deadHeroState.hero.actorId,
        contentId: deadHeroState.actors.find(
          (actor) => actor.actorId === deadHeroState.hero.actorId,
        )!.contentId,
    ...
    expect(concluded.state.conclusion).toEqual({
      ...
      cause: {
        ...
        depth: 3,
        turn: 12,
        worldTime: deadHeroState.worldTime,
      },
  ...
  it('credits no killer for an environmental death with no hero actor.died event in the transition', () => {
    ...
    const events: DomainEvent[] = [
      { type: 'hero.waited', eventId: 'command.starve', heroId: state.hero.actorId, x: 1, y: 1 },
    ];
  ...
  it('credits no killer when the killer is the hero itself (self-kill)', () => {
    const state = withHeroHealth(createDemoRun(), 0);
    const heroContentId = state.actors.find(
      (actor) => actor.actorId === state.hero.actorId,
    )!.contentId;
    const events: DomainEvent[] = [
      {
        type: 'actor.damaged',
  ...
  it('is idempotent when the run is already concluded', () => {
    ...
    const events: DomainEvent[] = [
      {
        type: 'actor.died',
        eventId: 'command.again',
        actorId: state.hero.actorId,
        contentId: state.actors[0]!.contentId,
        killerActorId: state.hero.actorId,
      },
    ];
