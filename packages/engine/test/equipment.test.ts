...
describe('equipment planning and item lights', () => {
  ...
  it('reserves both hands and plans shield displacement before equipping', () => {
    ...
    const shieldItem = item('item.shield.1', shield.id, {
      type: 'equipped',
      actorId: 'hero.demo',
      slot: 'off-hand',
    });
    ...
    const hero = {
      ...base.actors[0]!,
      equipment: { ...base.actors[0]!.equipment, 'off-hand': shieldItem.itemId },
    };
    const run = { ...base, actors: [hero], items: [axeItem, shieldItem] };
    expect(
      equipmentPlan({
        run,
        content: pack(axe, shield),
        actorId: 'hero.demo',
        itemId: axeItem.itemId,
        slot: 'main-hand',
      }),
    ).toEqual({
      ok: true,
      equip: [{ itemId: axeItem.itemId, slot: 'main-hand' }],
      unequip: [shieldItem.itemId],
  ...
  it('fails atomically when displaced equipment cannot fit in the backpack', () => {
    ...
    const swordItem = item('item.sword.1', swordDefinition.id, {
      type: 'equipped',
      actorId: 'hero.demo',
      slot: 'main-hand',
    });
    const axeItem = item('item.axe.1', axe.id, { type: 'backpack', actorId: 'hero.demo' });
    const filler = item('item.filler.1', fillerDefinition.id, {
      type: 'backpack',
      actorId: 'hero.demo',
    });
    const hero = {
      ...base.actors[0]!,
      equipment: {
        ...base.actors[0]!.equipment,
        'main-hand': swordItem.itemId,
        'off-hand': shieldItem.itemId,
      },
    };
    const run = {
      ...base,
      hero: { ...base.hero, backpackCapacity: 2 },
      actors: [hero],
    ...
    expect(
      equipItem({
        run,
        content: pack(axe, shield, fillerDefinition, swordDefinition),
        actorId: 'hero.demo',
        itemId: axeItem.itemId,
        slot: 'main-hand',
      }),
  ...
  it('applies base and hidden enchantment modifiers only while equipped', () => {
    ...
    const sources = equipmentModifiers({
      run: { ...base, actors: [hero], items: [shieldItem] },
      content,
      actorId: 'hero.demo',
    });
  ...
  it('emits light only from enabled equipped or floor-placed fueled items', () => {
    ...
    const base = createDemoRun();
    const lit = {
      ...item('item.lantern.1', lantern.id, {
        type: 'equipped',
        actorId: 'hero.demo',
        slot: 'off-hand',
      }),
    ...
    expect(
      itemLightSources({
        run: { ...base, items: [{ ...lit, location: { type: 'backpack', actorId: 'hero.demo' } }] },
        content: pack(lantern),
        floorId: 'floor.demo',
      }),
    ).toEqual([]);
  ...
  it('rejects one-handed content that reserves the other hand', () => {
    ...
    expect(() => validateContentBoundRun(createDemoRun(), pack(invalid))).toThrow(
      /handedness.*reserve/i,
    );
  });
  ...
  it('toggles and refuels item-backed lights through saved commands', () => {
    ...
    const oilItem = {
      ...item('item.oil.1', oil.id, { type: 'backpack', actorId: 'hero.demo' }),
      quantity: 3,
    };
    const hero = {
      ...base.actors[0]!,
    ...
    const refueled = resolveCommand(
      run,
      {
        type: 'refuel',
        commandId: 'command.refuel',
        expectedRevision: 0,
        itemId: lanternItem.itemId,
        fuelItemId: oilItem.itemId,
        quantity: 3,
      },
      { content },
    );
    ...
    expect(itemLightSources({ run: toggled.state, content, floorId: 'floor.demo' })).toHaveLength(
      1,
    );
  ...
  it('refreshes dark-floor perception from a newly enabled equipped light', () => {
    ...
    const base = createDemoRun();
    const lanternItem = {
      ...item('item.lantern-dark.1', lantern.id, {
        type: 'equipped',
        actorId: 'hero.demo',
        slot: 'off-hand',
      }),
    ...
    const result = resolveCommand(
      { ...base, actors: [hero], items: [lanternItem], floors: [floor] },
      {
        type: 'toggle-light',
        commandId: 'command.light-dark',
        expectedRevision: 0,
        itemId: lanternItem.itemId,
        enabled: true,
      },
