...
function lootTable(): LootTableContentEntry {
  return {
    kind: 'loot-table',
    id: 'loot-table.world-step-drop',
    name: 'World step drop',
    tags: [],
    rolls: 1,
    choices: [
      {
        contentId: 'item.world-step-drop',
        lootTableId: null,
        weight: 1,
        minimumQuantity: 1,
        maximumQuantity: 1,
...
function advanceUntilMonsterDead(
  input: Readonly<{
    state: ActiveRun;
  ...
  for (let attempt = 0; attempt < 20; attempt += 1) {
    ...
    const step = resolveWorldStep({
      ...
      eventId: `command.attack-${attempt}`,
      action: {
        type: 'bump-attack',
    ...
    state = step.state;
    events = step.events;
    publicEvents = step.publicEvents;
    internalActions = step.internalActions;
  }
...
describe('atomic world steps', () => {
  it('makes confirmed aggression hostile before the attack roll and saves it', () => {
    ...
    const result = resolveWorldStep({
      state: { ...state, actors: [state.actors[0]!, target] },
      content,
  ...
  it('applies the hero action then actors until the hero is selected again', () => {
    ...
    const result = resolveWorldStep({
      state: { ...state, actors: [state.actors[0]!, enemy] },
      content,
    ...
    expect(JSON.stringify(result.publicEvents)).not.toMatch(
      /naturalRoll|rolledDice|rolledDamage|defense/,
    );
  });
  ...
  it("folds the hero's statModifiers into their combat defense", () => {
    ...
    const result = resolveWorldStep({
      state: { ...boosted, actors: [boosted.actors[0]!, enemy] },
      content,
      eventId: 'command.move',
      action: { type: 'move', actorId: 'hero.demo', to: { x: 2, y: 1 }, cost: 100 },
    });
  ...
  it('does not partially mutate input when one actor behavior fails', () => {
    const state = createDemoRun();
    const enemy = monster('monster.invalid-behavior', {
      energy: 100,
    ...
    expect(() =>
      resolveWorldStep({
        state: input,
        content,
        eventId: 'command.invalid-behavior',
        action: { type: 'wait', actorId: 'hero.demo', cost: 100 },
      }),
  ...
  it('resolves hostile opportunity attacks before applying movement', () => {
    ...
    const enemy = monster('monster.reactor', {
      x: 3,
      y: 2,
      energy: 0,
      reactionReady: true,
    ...
    const content = contentWithMonster(enemy.contentId);
    const result = resolveWorldStep({
      state: { ...state, actors: [hero, enemy] },
      content,
      eventId: 'command.depart',
  ...
  it('omits unseen actor activity from the public event sequence', () => {
    ...
    const result = resolveWorldStep({
      state: { ...state, floors: [darkFloor], actors: [state.actors[0]!, enemy] },
      content,
  ...
  it('emits an ordinary population encounter exactly once with public redaction', () => {
    ...
    const enemy = {
      ...monster('monster.population', { energy: 0 }),
      x: 2,
      y: 1,
      populationId: 'population.visible',
      populationRoleId: 'individual',
    };
    const encounter: EncounterContentEntry = {
      kind: 'encounter',
      id: 'encounter.visible',
      name: 'Visible encounter',
      ...
      populations: [
        {
          ...
          createdAt: 0,
          model: 'individual' as const,
          livingMemberIds: [enemy.actorId],
          formerMemberIds: [],
        },
      ],
    };
    const first = resolveWorldStep({
      state,
      content: withEncounter,
      eventId: 'command.encounter-1',
    ...
    expect(first.publicEvents).toContainEqual({
      type: 'population.notice',
      eventId: 'command.encounter-1',
      category: 'encountered',
      actorId: enemy.actorId,
      presentation: 'population.encountered',
    });
    const second = resolveWorldStep({
      state: first.state,
      content: withEncounter,
      eventId: 'command.encounter-2',
      action: { type: 'wait', actorId: base.hero.actorId, cost: 100 },
    });
  ...
  it('returns the saved public sequence when a duplicate command is retried after visibility changes', () => {
    ...
    const first = resolveCommand(
      { ...state, floors: [darkFloor], actors: [state.actors[0]!, enemy] },
      command,
      { content },
    );
    const later = {
      ...first.state,
      floors: first.state.floors.map((floor) => ({
        ...floor,
  ...
  it('drops monster loot on a killing blow and stays replay-stable', () => {
    ...
    const replay = advanceUntilMonsterDead({
      state: input,
      content,
      heroId: state.hero.actorId,
      targetActorId: enemy.actorId,
  ...
  it('does not drop loot for a monster with no loot table configured', () => {
    ...
    const content: CompiledContentPack = {
      ...
      entries: [
        ...createDemoContentPack().entries,
        {
          ...monsterDefinition(enemy.contentId),
          health: 1,
          defense: -1_000,
          lootTableId: null,
          dropChance: 0,
