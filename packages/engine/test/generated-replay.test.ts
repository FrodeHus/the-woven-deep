...
function direction(from: Readonly<{ x: number; y: number }>, to: Readonly<{ x: number; y: number }>): Direction {
...
function commandsFor(run: ActiveRun): readonly GameCommand[] {
  ...
  const route = analyzeConnectivity({ width: floor.width, height: floor.height, tiles: floor.tiles,
    start: floor.stairUp!, target: floor.stairDown! }).route;
  ...
  const moves = walkableRoute.slice(1).map<GameCommand>((point, index) => ({
    type: 'move', commandId: `command.generated-${index + 1}`, expectedRevision: index,
    direction: direction(walkableRoute[index]!, point),
  }));
  const wait: GameCommand = { type: 'wait', commandId: 'command.generated-wait', expectedRevision: 4 };
  return [
    ...moves,
    wait,
    { ...moves[0]! },
...
describe('generated save and replay continuity', () => {
  ...
  it('preserves exact generated terrain, light, knowledge, projection, and replay across save/reload', () => {
    ...
    const splitFloor = after.state.floors.find((floor) => floor.floorId === after.state.activeFloorId)!;
    const continuousFloor = continuous.state.floors.find((floor) => floor.floorId === continuous.state.activeFloorId)!;
    ...
    expect(continuous.steps.slice(-2).map((step) => step.result.status)).toEqual(['applied', 'rejected']);
