...
describe('run-records continuous-versus-split replay', () => {
  it.each(RUN_RECORDS_REPLAY_BOUNDARIES)('is byte-identical when reloading at %s', (boundary) => {
    ...
    expect(stableJson(split.records.map((entry) => entry.commandResult))).toBe(
      stableJson(continuous.records.map((entry) => entry.commandResult)),
    );
    expect(stableJson(split.records.map((entry) => entry.authoritativeEvents))).toBe(
      stableJson(continuous.records.map((entry) => entry.authoritativeEvents)),
    );
    expect(stableJson(split.records.map((entry) => entry.publicEvents))).toBe(
      stableJson(continuous.records.map((entry) => entry.publicEvents)),
    );
    expect(stableJson(split.records.map((entry) => entry.projection))).toBe(
      stableJson(continuous.records.map((entry) => entry.projection)),
    );
    // Identical finalization record, deltas, and authoritative finalization events.
    expect(stableJson(split.finalization?.record)).toBe(
      stableJson(continuous.finalization?.record),
    );
    expect(stableJson(split.finalization?.deltas)).toBe(
      stableJson(continuous.finalization?.deltas),
    );
    expect(stableJson(split.finalization?.events)).toBe(
      stableJson(continuous.finalization?.events),
    );
  });

  it('is equivalent when every named boundary reloads', () => {
    const continuous = runRunRecordsDemo(pack);
    const split = runRunRecordsDemo(
      pack,
      new Set(RUN_RECORDS_REPLAY_BOUNDARIES.map((_, index) => index)),
    );
  ...
  it('drives the full milestone scenario through production command resolution', () => {
    ...
    const concluded = conclusion.authoritativeEvents.find(
      (event) => event.type === 'run.concluded',
    );
    ...
    const finalizedEvents = result.finalization!.events.filter(
      (event) => event.type === 'run.finalized',
    );
