...
describe('floor knowledge packing', () => {
  ...
  it.each([-1, 1.5, Number.NaN, Number.POSITIVE_INFINITY, Number.MAX_SAFE_INTEGER + 1])(
    ...
    (cellCount) => {
      ...
      expect(() => validateKnowledgePacking({ exploredWords: [], rememberedTerrainWords: [] }, cellCount)).toThrow(/cell count/);
    },
  );
  ...
  it.each([-1, 1.5, Number.NaN, Number.POSITIVE_INFINITY, 0x1_0000_0000])(
    ...
    (word) => {
      expect(() => validateKnowledgePacking({ exploredWords: [word], rememberedTerrainWords: [0xffff_ffff] }, 8)).toThrow(/unsigned 32-bit/);
      expect(() => validateKnowledgePacking({ exploredWords: [0], rememberedTerrainWords: [word] }, 8)).toThrow(/unsigned 32-bit/);
    },
  );
  ...
  it('rejects nonzero remembered terrain padding and both disagreement directions', () => {
    ...
    expect(() => validateKnowledgePacking({ exploredWords: [0], rememberedTerrainWords: [0xffff_fff1] }, 8)).toThrow(/disagree/);
  });

  it('rejects packed terrain values that are neither tile IDs nor unknown', () => {
    expect(() => validateKnowledgePacking({ exploredWords: [1], rememberedTerrainWords: [0xffff_fff7] }, 8)).toThrow(/tile ID/);
  });
  ...
  it('accepts empty knowledge and valid unsigned high-bit words', () => {
    ...
    expect(() => validateKnowledgePacking({ exploredWords: [], rememberedTerrainWords: [] }, 0)).not.toThrow();
    expect(() => validateKnowledgePacking({ exploredWords: [0x8000_0000], rememberedTerrainWords: [
      0xffff_ffff, 0xffff_ffff, 0xffff_ffff, 0x1fff_ffff,
    ] }, 32)).not.toThrow();
  });

  it('rejects duplicate, out-of-range, and malformed remembered tiles', () => {
    const empty = createUnknownKnowledge(10);
    expect(() => rememberTiles(empty, 10, [{ index: 0, tile: 1 }, { index: 0, tile: 2 }])).toThrow(/unique/);
