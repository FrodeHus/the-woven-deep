...
import type {
  AchievementContentEntry, CompiledContentPack, ContentEntry, EncounterContentEntry,
  FallenChampionTemplateContentEntry, ItemContentEntry,
} from '@woven-deep/content';
import {
  createDemoContentPack, createDemoRun, deriveHallRecordId, emptyRunMetrics, encodeRunSeed,
  evaluateDiscoveryProtection, finalizeRun, scoreRun, selectHeirloom,
  type ActiveRun, type EncounterRunDecision, type FallenHeroRunDecision, type ItemInstance,
  type LifetimeState,
} from '../src/index.js';

function itemDef(id: string, overrides: Partial<ItemContentEntry> = {}): ItemContentEntry {
  return { kind: 'item', id, name: `Name of ${id}`, tags: [], glyph: ')', color: '#c0c0c0',
    category: 'weapon', stackLimit: 1, price: 10, rarity: 'rare', heirloomEligible: true, minDepth: 1, maxDepth: 20,
    actionCost: 100, equipment: { slots: ['main-hand'], handedness: 'one-handed', reservedSlots: [] },
    combat: null, light: null, identification: { mode: 'known', poolId: null }, effects: [], ...overrides };
}

const template: FallenChampionTemplateContentEntry = { kind: 'fallen-champion-template',
  id: 'fallen-champion-template.core', name: "The Deep's Champion", tags: ['champion'],
  fallbackMonsterId: 'monster.boss', fallbackItemId: 'item.fallback',
...
const championAchievement: AchievementContentEntry = { kind: 'achievement', id: 'achievement.z-champion',
  name: 'Defeated the Champion', tags: [], description: 'First champion defeat.', criteriaId: 'first-champion-defeat' };
const echoAchievement: AchievementContentEntry = { kind: 'achievement', id: 'achievement.a-echo',
  name: 'Silenced an Echo', tags: [], description: 'First echo defeat.', criteriaId: 'first-echo-defeat' };

function encounterDef(id: string): EncounterContentEntry {
  return { kind: 'encounter', id, name: `Encounter ${id}`, tags: [], adminDescription: null, model: 'individual',
    minDepth: 1, maxDepth: 20, environmentTags: [], requiredVaultTags: [], weight: 1, rarity: 'common',
    runAppearanceChance: 0.5, discoveryProtectionIncrement: 0.1, discoveryProtectionCap: 0.5,
    maximumInstancesPerRun: 1, placement: { minimumStairDistance: 0, minimumObjectiveDistance: 0,
      maximumMemberDistance: 0, allowedTerrainTags: ['floor'], requiresVaultSlot: false, failureMode: 'optional' },
...
function pack(extra: readonly ContentEntry[] = []): CompiledContentPack {
  ...
  return { ...base, entries: [...base.entries, template, itemDef('item.fallback', { rarity: 'common' }), ...extra] };
}
...
function equippedItem(itemId: string, contentId: string): ItemInstance {
  return { itemId, contentId, quantity: 1, condition: 100, enchantment: null, identified: true,
    charges: null, fuel: null, enabled: null,
...
function concludedRun(overrides: Partial<ActiveRun> = {}): ActiveRun {
  ...
  return { ...base,
    metrics: { ...emptyRunMetrics(), kills: 3, threatDefeated: 12, deepestDepth: 4, turnsElapsed: 120,
      discoveriesRevealed: 2 },
    reputations: [{ factionId: 'faction.lampwrights', value: 5 }],
    conclusion: { completionType: 'died', cause: { killerContentId: null, depth: 4, turn: 120, worldTime: 12_000 },
      concludedAtRevision: 9, finalized: false },
    ...overrides };
}

function emptyLifetime(overrides: Partial<LifetimeState> = {}): LifetimeState {
  return { conqueredChampionRecordIds: [], grantedAchievementIds: [], discoveryProtection: [],
    totals: emptyRunMetrics(), ...overrides };
}

const championRecordId = `record.${'1'.repeat(32)}.${'b'.repeat(16)}`;
const echoRecordId = `record.${'2'.repeat(32)}.${'c'.repeat(16)}`;

function decision(role: 'champion' | 'echo', overrides: Partial<FallenHeroRunDecision> = {}): FallenHeroRunDecision {
  return { hallRecordId: role === 'champion' ? championRecordId : echoRecordId, rank: role === 'champion' ? 1 : 2,
    role, gateRoll: null, retained: true, encountered: true, defeated: true, ...overrides };
}

describe('finalizeRun', () => {
  it('throws for an unconcluded run', () => {
    const run = { ...concludedRun(), conclusion: null };
    expect(() => finalizeRun({ run, content: pack(), lifetime: emptyLifetime() })).toThrow(/conclud/i);
  });
  ...
  it('advances only the run-records stream and leaves the rest of the run untouched', () => {
    ...
    expect(finalized.run).toEqual({ ...run,
  ...
  it('assembles the hall record from the concluded run', () => {
    const content = pack([itemDef('item.sword'), itemDef('item.crown',
      { rarity: 'legendary', equipment: { slots: ['head'], handedness: 'one-handed', reservedSlots: [] } })]);
  ...
  describe('achievement grants', () => {
    ...
    const cases: readonly GrantCase[] = [
      { label: 'retained champion defeat grants first-champion-defeat',
        decisions: [decision('champion')], lifetime: emptyLifetime(),
        expectedGrantIds: ['achievement.z-champion'], expectedNewlyConquered: [championRecordId] },
      { label: 'champion already conquered in a previous life grants nothing new',
        ...
        expectedGrantIds: [], expectedNewlyConquered: [] },
      { label: 'already-granted champion achievement never regrants, but the conquest is still recorded',
        decisions: [decision('champion')],
        lifetime: emptyLifetime({ grantedAchievementIds: ['achievement.z-champion'] }),
        expectedGrantIds: [], expectedNewlyConquered: [championRecordId] },
      { label: 'a non-retained champion decision grants nothing',
        ...
        lifetime: emptyLifetime(), expectedGrantIds: [], expectedNewlyConquered: [] },
      ...
      { label: 'first lifetime echo defeat grants first-echo-defeat',
        decisions: [decision('echo')], lifetime: emptyLifetime(),
        expectedGrantIds: ['achievement.a-echo'], expectedNewlyConquered: [] },
      ...
      { label: 'both first defeats grant both achievements in sorted achievement-ID order',
        decisions: [decision('champion'), decision('echo')], lifetime: emptyLifetime(),
        expectedGrantIds: ['achievement.a-echo', 'achievement.z-champion'],
        expectedNewlyConquered: [championRecordId] },
    ];

    it.each(cases)('$label', ({ decisions, lifetime, expectedGrantIds, expectedNewlyConquered }) => {
      const content = pack([championAchievement, echoAchievement]);
      const run = concludedRun({ fallenHeroDecisions: decisions });
      const finalized = finalizeRun({ run, content, lifetime });
      expect(finalized.deltas.achievementGrants.map((grant) => grant.achievementId)).toEqual(expectedGrantIds);
    ...
    it('grants carry the achievement criteria and name', () => {
      ...
      expect(finalized.deltas.achievementGrants).toEqual([
        { achievementId: 'achievement.a-echo', criteriaId: 'first-echo-defeat', name: 'Silenced an Echo' },
        { achievementId: 'achievement.z-champion', criteriaId: 'first-champion-defeat', name: 'Defeated the Champion' },
      ]);
    });
  ...
  it('emits run.finalized first, then achievement.granted per grant in sorted achievement-ID order', () => {
    ...
    expect(finalized.events).toEqual([
      { type: 'run.finalized', eventId, recordId: finalized.record.recordId,
        completionType: 'died', scoreTotal: finalized.record.score.total },
      { type: 'achievement.granted', eventId, achievementId: 'achievement.a-echo',
        criteriaId: 'first-echo-defeat', name: 'Silenced an Echo' },
      { type: 'achievement.granted', eventId, achievementId: 'achievement.z-champion',
        criteriaId: 'first-champion-defeat', name: 'Defeated the Champion' },
    ]);
  ...
  it('computes discovery protection updates over the run decisions, sorted by encounter ID', () => {
    ...
    const encounterDecisions: readonly EncounterRunDecision[] = [
      { encounterId: 'encounter.a-shrine', baseProbability: 0.5, protectionBonus: 0.1, effectiveProbability: 0.4,
        eligible: true, reachedEligibleDepth: true, encountered: false, instancesCreated: 0 },
      { encounterId: 'encounter.b-warden', baseProbability: 0.5, protectionBonus: 0.2, effectiveProbability: 0.3,
    ...
    expect(finalized.deltas.discoveryProtectionUpdates.map((update) => update.encounterId))
      .toEqual(['encounter.a-shrine', 'encounter.b-warden']);
  });
});
