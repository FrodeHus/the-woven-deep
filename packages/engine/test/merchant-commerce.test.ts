...
describe('merchant price quotes', () => {
  ...
  it('rejects negative, noninteger, and unsafe inputs before arithmetic', () => {
    for (const invalid of [-1, 0.5, Number.NaN, Number.POSITIVE_INFINITY, 2 ** 53]) {
      expect(() =>
        quoteMerchantPurchase({ basePrice: invalid, merchantBps: 12500, factionBps: 11000 }),
      ).toThrow(RangeError);
      expect(() =>
        quoteMerchantPurchase({ basePrice: 3, merchantBps: invalid, factionBps: 11000 }),
      ).toThrow(RangeError);
      expect(() =>
        quoteMerchantPurchase({ basePrice: 3, merchantBps: 12500, factionBps: invalid }),
      ).toThrow(RangeError);
      expect(() =>
        quoteMerchantSale({ basePrice: invalid, merchantBps: 5000, factionBps: 9000 }),
      ).toThrow(RangeError);
      expect(() => quoteMerchantService({ basePrice: invalid, factionBps: 11000 })).toThrow(
        RangeError,
      );
  ...
  it('rejects overflowing basis-point products instead of using floating point', () => {
    ...
    expect(() =>
      quoteMerchantService({ basePrice: Number.MAX_SAFE_INTEGER, factionBps: 10000 }),
    ).toThrow(RangeError);
  });
});
...
describe('faction reputation', () => {
  it('reads the authored starting value while no record exists', () => {
    ...
    expect(
      factionReputation(runWithReputations([{ factionId: faction.id, value: 77 }]), faction),
    ).toBe(77);
  });
  ...
  it('starts a missing record from the authored starting value and emits exact deltas', () => {
    ...
    expect(changed.event).toEqual({
      ...
      delta: 25,
      value: faction.startingReputation + 25,
      reason: 'commerce',
    });
    expect(changed.state.reputations).toEqual([
      { factionId: faction.id, value: faction.startingReputation + 25 },
    ]);
    expect(run.reputations).toEqual([]);
  });

  it('clamps to the faction maximum and minimum while reporting the requested delta', () => {
    const atMaximum = runWithReputations([
      { factionId: faction.id, value: faction.maximumReputation },
    ]);
    const raised = changeReputation({
      run: atMaximum,
      faction,
      delta: 50,
      reason: 'commerce',
      eventId: 'event.commerce-2',
    });
    expect(raised.event.value).toBe(faction.maximumReputation);
    expect(raised.event).toMatchObject({
      previous: faction.maximumReputation,
      delta: 50,
      reason: 'commerce',
    });
    expect(raised.state.reputations).toEqual([
      { factionId: faction.id, value: faction.maximumReputation },
    ]);

    const nearMinimum = runWithReputations([
      { factionId: faction.id, value: faction.minimumReputation + 10 },
    ]);
    const lowered = changeReputation({
      run: nearMinimum,
      faction,
      delta: -300,
      reason: 'death',
      eventId: 'event.death-1',
    });
    expect(lowered.event).toMatchObject({
      previous: faction.minimumReputation + 10,
      delta: -300,
      value: faction.minimumReputation,
      reason: 'death',
    });
    expect(lowered.state.reputations).toEqual([
      { factionId: faction.id, value: faction.minimumReputation },
    ]);
  });
  ...
  it('rejects noninteger and overflowing deltas without partial mutation', () => {
    const boundless = {
      ...faction,
      minimumReputation: Number.MIN_SAFE_INTEGER,
      maximumReputation: Number.MAX_SAFE_INTEGER,
    };
...
describe('merchant item acceptance', () => {
  ...
  it('requires backpack ownership at command time', () => {
    const onFloor = backpackItem(sword, {
      location: { type: 'floor', floorId: 'floor.demo', x: 1, y: 1 },
    });
    const inStock = backpackItem(sword, {
      location: { type: 'merchant-stock', populationId: 'population.merchant' },
    });
  ...
  it('rejects nonpositive prices and unaccepted categories', () => {
    expect(
      merchantAcceptsItem(backpackItem(sword), { ...sword, price: 0 }, encounter, new Set()),
    ).toBe(false);
  ...
  it('rejects protected content tags and heirloom metadata', () => {
    for (const tag of ['heirloom', 'quest', 'objective', 'nontransferable']) {
      expect(
        merchantAcceptsItem(
          backpackItem(sword),
          { ...sword, tags: [...sword.tags, tag] },
          encounter,
          new Set(),
        ),
      ).toBe(false);
    }
    const heirloom = backpackItem(sword, {
      heirloom: {
        displayName: 'Ada’s Blade',
        glyph: '/',
        color: '#c2c6c8',
        originatingHallRecordId: 'hall.record-1',
        originatingRank: 1,
        sourceItemId: null,
      },
    });
  ...
  it('rejects boss-guaranteed unique items derived from authored encounters', () => {
    ...
    expect(merchantAcceptsItem(backpackItem(sword), sword, encounter, new Set([sword.id]))).toBe(
      false,
    );
  });
