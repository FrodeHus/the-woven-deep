...
const divergentCommands = fileURLToPath(
  new URL('../fixtures/demo.divergent.commands', import.meta.url),
);
...
describe('engine demo CLI', () => {
  it('verifies deterministic replay across a real save and reload', () => {
    const result = spawnSync(process.execPath, [script, '--verify', commands], {
      encoding: 'utf8',
    });
  ...
  it('fails safely when the command file is missing', () => {
    const result = spawnSync(process.execPath, [script, '--verify', missingCommands], {
      encoding: 'utf8',
    });
  ...
  it('fails safely when the uninterrupted comparison diverges', () => {
    const result = spawnSync(process.execPath, [script, '--verify', commands, divergentCommands], {
      encoding: 'utf8',
    });
