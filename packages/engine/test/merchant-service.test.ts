...
beforeAll(async () => {
  ...
  potion = content.entries.find(
    (entry): entry is ItemContentEntry =>
      entry.kind === 'item' && entry.id === 'item.crimson-potion',
  )!;
});
...
function merchantRun(options: FixtureOptions = {}): ActiveRun {
  ...
  const heroItems: ItemInstance[] = [
    ...
    item(
      ...
      ring.id,
      1,
      { type: 'equipped', actorId: HERO_ID, slot: 'left-ring' },
      { identified: false },
    ),
    item(
      'item.hero.potion',
    ...
    item(
      ...
      ring.id,
      1,
      { type: 'backpack', actorId: MERCHANT_ACTOR_ID },
      { identified: false },
    ),
    item(
      'item.stock.ring',
      ring.id,
      1,
      { type: 'merchant-stock', populationId: POPULATION_ID },
      { identified: false },
    ),
  ];
...
function service(run: ActiveRun): MerchantServiceState {
  return merchantPopulation(run).services.find(
    (entry) => entry.serviceId === 'merchant-service.identify',
  )!;
}
...
describe('trade-service identification', () => {
  it('identifies an unidentified backpack instance at the current-tier price', () => {
    ...
    expect(
      resolved.state.items.find((entry) => entry.itemId === 'item.hero.unknown')?.identified,
    ).toBe(true);
    ...
    expect(resolved.events).toEqual([
      {
        ...
        eventId: 'command.identify',
        merchantPopulationId: POPULATION_ID,
        serviceId: 'merchant-service.identify',
        targetItemId: 'item.hero.unknown',
        price,
        currency: runWithTrade.hero.currency - price,
        remainingUses: 1,
      },
  ...
  it('identifies an equipped unidentified instance', () => {
    ...
    const resolved = resolveCommand(
      runWithTrade,
      serviceCommand({ targetItemId: 'item.hero.worn' }),
      context(),
    );
    expect(resolved.result).toMatchObject({ status: 'applied', revision: 2, turn: 0 });
    expect(
      resolved.state.items.find((entry) => entry.itemId === 'item.hero.worn')?.identified,
    ).toBe(true);
  ...
  it('reveals a shuffled appearance and marks the instance identified', () => {
    ...
    const resolved = resolveCommand(
      runWithTrade,
      serviceCommand({ targetItemId: 'item.hero.potion' }),
      context(),
    );
    expect(resolved.result).toMatchObject({ status: 'applied', revision: 2, turn: 0 });
    expect(resolved.state.identification.knownAppearanceIds).toEqual([appearanceId]);
    expect(
      resolved.state.items.find((entry) => entry.itemId === 'item.hero.potion')?.identified,
    ).toBe(true);
    expect(resolved.events).toEqual([
      {
        type: 'trade.service-purchased',
  ...
  it('decrements exactly one use per success and persists uses across save/load', () => {
    ...
    const second = resolveCommand(
      reloaded,
      serviceCommand({
        commandId: 'command.identify-2',
        expectedRevision: 2,
        targetItemId: 'item.hero.worn',
      }),
      context(),
    );
    ...
    const exhausted = resolveCommand(
      second.state,
      serviceCommand({
        commandId: 'command.identify-3',
        expectedRevision: 3,
        targetItemId: 'item.hero.potion',
      }),
      context(),
    ...
    expect(exhausted.result).toMatchObject({
      status: 'invalid',
      reason: 'trade.service-unavailable',
    });
  ...
  it.each([
    [
      ...
      {},
      serviceCommand({ expectedRevision: 0 }),
      'trade.required',
      false,
    ],
    [
    ...
    [
      ...
      {},
      serviceCommand({ targetItemId: 'item.absent' }),
      'trade.target-invalid',
      true,
    ],
    [
      'for an already identified target',
      {},
      serviceCommand({ targetItemId: 'item.hero.sword' }),
      'trade.target-invalid',
      true,
    ],
    [
      "for another actor's item",
      {},
      serviceCommand({ targetItemId: 'item.foreign.ring' }),
      'trade.target-invalid',
      true,
    ],
    [
      'for a merchant stock item',
      {},
      serviceCommand({ targetItemId: 'item.stock.ring' }),
      'trade.target-invalid',
      true,
    ],
  ...
  it('requires both the faction tier allow-list and the merchant offer tier ids', () => {
    ...
    const withoutOffer = {
      ...opened,
      populations: opened.populations.map((population) =>
        population.model === 'merchant' ? { ...population, services: [] } : population,
      ),
    };
    expect(
      validateTradeCommand({
    ...
    const tierGated = {
      ...
      populations: opened.populations.map((population) =>
        population.model === 'merchant'
          ? {
              ...population,
              services: population.services.map((entry) => ({ ...entry, tierIds: ['trusted'] })),
            }
          : population,
      ),
    };
