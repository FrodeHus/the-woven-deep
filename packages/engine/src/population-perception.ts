...
export function mergeLastKnownTargets(
  memories: readonly LastKnownTarget[],
...
export function visibleTargetObservations(
  input: Readonly<{
  ...
  if (!Number.isSafeInteger(input.width) || input.width <= 0)
    throw new RangeError('perception width must be positive');
  return input.actors
    ...
    .filter((actor) => {
      ...
      return (
        index >= 0 &&
        index < input.illuminationIntensity.length &&
...
export function soundTargetObservation(
  input: Readonly<{
    observerActorId: string;
...
export function updateActorMemory(
  input: Readonly<{
  ...
  if (
    input.investigationDuration !== null &&
  ...
  return {
    ...
    investigation: {
      ...
      x: selected.x,
      y: selected.y,
      startedAt: selected.observedAt,
      expiresAt:
        input.investigationDuration === null
          ? null
          : selected.observedAt + input.investigationDuration,
    },
  };
}
