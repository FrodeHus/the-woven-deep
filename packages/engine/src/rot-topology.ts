...
function corridorShape(value: unknown, width: number, height: number): RotCorridorShape | null {
  ...
  if (
    candidate._startX! < 0 ||
    candidate._startX! >= width ||
    candidate._endX! < 0 ||
    candidate._endX! >= width ||
    candidate._startY! < 0 ||
    candidate._startY! >= height ||
    candidate._endY! < 0 ||
    candidate._endY! >= height
  )
    return null;
  return {
...
export function createRotTopology(
  width: number,
  height: number,
  ...
  try {
    return withRotSeed(foldSeed(state), () => {
      ...
      if (
        roomShapes.some(
          (room) =>
            Object.values(room).some((value) => !Number.isSafeInteger(value)) ||
            room.left < 0 ||
            room.top < 0 ||
            room.right >= width ||
            room.bottom >= height ||
      ...
      const rooms: RoomBounds[] = roomShapes.map((room, index) => ({
        roomId: `room.${index}`,
        ...room,
      }));

      /* rot-js 2.2.1 and generatorVersion 2 pin this narrow adapter boundary. */
      const corridorShapes = digger
        .getCorridors()
        .map((corridor) => corridorShape(corridor, width, height));
      if (corridorShapes.some((corridor) => corridor === null))
        return { ok: false, code: 'topology.invalid-geometry' };
      ...
      const corridors: CorridorRecord[] = copied.map((corridor, index) => ({
        corridorId: `corridor.${index}`,
        ...corridor,
      }));
      if (!tiles.some((tile) => tile === 1 || tile === 2))
        return { ok: false, code: 'topology.empty' };
