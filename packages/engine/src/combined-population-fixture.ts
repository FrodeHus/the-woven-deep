...
import {
  createPopulationDemoRun, populationDemoCommands, populationDemoScenario, resolvePopulationDemoCommand,
  validatePopulationInvariants, type PopulationDemoInput, type PopulationDemoScenario,
...
export const COMBINED_POPULATION_REPLAY_BOUNDARIES = [
  'before-group-relay', 'before-trade-open', 'before-trade-buy', 'before-trade-close',
  'before-source-spawn', 'before-leader-death', 'before-boss-threshold',
...
export type CombinedDemoInput =
  | Readonly<{ kind: 'population'; boundary: typeof COMBINED_POPULATION_REPLAY_BOUNDARIES[number]; input: PopulationDemoInput }>
  | Readonly<{ kind: 'merchant'; boundary: typeof COMBINED_POPULATION_REPLAY_BOUNDARIES[number]; command: GameCommand }>;
...
function adjacentFreeCell(run: ActiveRun, floorId: string, target: Readonly<{ x: number; y: number }>,
  ...
  const occupied = new Set(run.actors.filter((actor) => actor.actorId !== excludedActorId
    && actor.floorId === floorId && actor.health > 0).map((actor) => `${actor.x}:${actor.y}`));
 ...
 */
export function createCombinedPopulationDemoRun(pack: CompiledContentPack, scenarioSeed = 0): ActiveRun {
  let run = createPopulationDemoRun(pack, scenarioSeed);
  const merchantEncounter = pack.entries.find((entry): entry is MerchantEncounterContentEntry =>
    entry.kind === 'encounter' && entry.model === 'merchant' && !entry.definition.permanent);
  if (!merchantEncounter) throw new Error('combined population fixture requires a non-permanent merchant encounter');
  const groupPopulation = run.populations.find((population) => population.model === 'group')!;
  const floorId = groupPopulation.floorId;
  const floor = run.floors.find((candidate) => candidate.floorId === floorId)!;
  const placement = placePopulation({ run, floor, content: pack, forcedEncounterId: merchantEncounter.id });
  if (placement.status !== 'placed') {
    throw new Error(`combined population fixture could not place ${merchantEncounter.id}: ${placement.reason}`);
  }
  run = {
    ...run,
    actors: [...run.actors, ...placement.createdActors].sort((left, right) => compareCodeUnits(left.actorId, right.actorId)),
    populations: [...run.populations, placement.population].sort((left, right) => compareCodeUnits(left.populationId, right.populationId)),
    items: [...run.items, ...placement.createdItems].sort((left, right) => compareCodeUnits(left.itemId, right.itemId)),
    floors: run.floors.map((candidate) => candidate.floorId === placement.floor.floorId ? placement.floor : candidate),
  ...
  const merchantPopulation = run.populations.find((population): population is MerchantPopulation =>
  ...
  run = {
    ...run,
    actors: run.actors.map((actor) => actor.actorId === merchantActor.actorId ? { ...actor, ...cell } : actor),
    floors: run.floors.map((candidate) => candidate.floorId !== floorId ? candidate : {
      ...candidate,
      entities: candidate.entities.map((entity) => entity.entityId === merchantActor.actorId ? { ...entity, ...cell } : entity),
    }),
  };
  ...
  const knowledge = refreshKnowledge({
    floor: refreshedFloor, hero: heroPerception(run.hero, refreshedHero),
    actors: new Map(run.actors.filter((actor) => actor.floorId === floorId).map((actor) => [actor.actorId, actor] as const)),
  }).knowledge;
  run = { ...run, floors: run.floors.map((candidate) => candidate.floorId === floorId ? { ...candidate, knowledge } : candidate) };
 ...
 */
export function combinedPopulationDemoCommands(initial: ActiveRun,
  scenario: PopulationDemoScenario): readonly CombinedDemoInput[] {
  const merchant = initial.populations.find((population): population is MerchantPopulation =>
    population.model === 'merchant')!;
  const buyItemId = merchant.stockItemIds[0];
  if (buyItemId === undefined) throw new Error('combined population fixture requires merchant stock to buy');
  ...
  const pushMerchant = (boundary: typeof COMBINED_POPULATION_REPLAY_BOUNDARIES[number],
    command: GameCommand): CombinedDemoInput => {
  ...
  result.push(pushMerchant('before-trade-open', {
    type: 'trade-open', merchantActorId: merchant.actorId, commandId: '', expectedRevision: 0,
  }));
  result.push(pushMerchant('before-trade-buy', {
    type: 'trade-buy', merchantPopulationId: merchant.populationId, itemId: buyItemId, quantity: 1,
    commandId: '', expectedRevision: 0,
  }));
  result.push(pushMerchant('before-trade-close', {
    type: 'trade-close', merchantPopulationId: merchant.populationId, commandId: '', expectedRevision: 0,
  }));
  for (const input of populationInputs.slice(1)) result.push(pushPopulation(input));
  return result;
}

function resolveCombinedMerchantCommand(state: ActiveRun, command: GameCommand,
  ...
  if (replay.state.worldTime < state.worldTime) throw new Error(`combined population fixture time reversed at ${command.commandId}`);
  validatePopulationInvariants(replay.state, pack);
  validateMerchantInvariants(replay.state, pack);
  return { state: replay.state, result: step.result, authoritativeEvents: recorded?.events ?? [],
...
export function resolveCombinedPopulationDemoCommand(state: ActiveRun, input: CombinedDemoInput,
  pack: CompiledContentPack): Readonly<{ state: ActiveRun; result: CommandResult; authoritativeEvents: readonly DomainEvent[];
    publicEvents: readonly PublicEvent[]; projection: GameplayProjection }> {
...
export function runCombinedPopulationDemo(pack: CompiledContentPack, reloadBefore: ReadonlySet<number> = new Set(),
  scenario: PopulationDemoScenario = populationDemoScenario(0)): CombinedPopulationDemoResult {
  ...
  for (const [index, input] of combinedPopulationDemoCommands(initial, scenario).entries()) {
    ...
    if (resolved.result.status !== 'applied') {
      throw new Error(`combined population demo command was rejected: ${stableJson(resolved.result)}`);
    }
    records.push({
      boundary: input.boundary, command: input.kind === 'population' ? input.input.command : input.command,
      commandResult: resolved.result, authoritativeEvents: resolved.authoritativeEvents,
      publicEvents: resolved.publicEvents, projection: resolved.projection,
    });
  }
  return { initial, state, records };
}

export function combinedPopulationDemoEquivalent(left: CombinedPopulationDemoResult,
  right: CombinedPopulationDemoResult): boolean {
