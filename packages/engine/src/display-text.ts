...
function cleanedDisplayText(value: string, emptyLabel = EMPTY_DISPLAY_LABEL): string {
  const cleaned = value.normalize('NFC').replace(/[\p{Cc}\p{Cf}]/gu, '').normalize('NFC');
...
export function boundedSuffixedDisplay(value: string, suffix: string): string {
  ...
  return [...cleanedDisplayText(value)].slice(0, DISPLAY_LIMIT - suffixPoints.length).join('')
    + suffixPoints.join('');
}
