...
import {
  ...
  type OpaqueId,
  type TradeBuyCommand,
  type TradeCloseReason,
  type TradeCommand,
  type TradeSellCommand,
...
export function isTradeCommand(command: GameCommand): command is TradeCommand {
  return (
    command.type === 'trade-open' ||
    command.type === 'trade-buy' ||
    command.type === 'trade-sell' ||
    command.type === 'trade-service' ||
...
export function merchantFaction(
  content: CompiledContentPack,
  factionId: OpaqueId,
): NpcFactionContentEntry {
...
function merchantPerceived(
  ...
  return (
    index !== undefined &&
    isPerceivedCell(perception.visibilityWords, perception.illumination, index)
  );
}
...
type PlanResult =
  | Readonly<{ ok: true; plan: TransactionPlan }>
  | Readonly<{ ok: false; reason: InvalidActionReason }>;
...
function planBuy(
  ...
  if (
    !source ||
    source.location.type !== 'merchant-stock' ||
  ...
  try {
    unitPrice = quoteMerchantPurchase({
      ...
      merchantBps: session.encounter.definition.merchantSaleBps,
  ...
  if (!deposit.ok) {
    return {
      ok: false,
      reason: deposit.reason === 'inventory.full' ? 'trade.capacity' : deposit.reason,
    };
  }
  const remainsInStock = deposit.items.some(
    (candidate) =>
      candidate.itemId === source.itemId && candidate.location.type === 'merchant-stock',
  );
  const stockItemIds = remainsInStock
    ? session.population.stockItemIds
  ...
  return {
    ...
    plan: {
      ...
      unitPrice,
      total,
      currency: state.hero.currency - total,
      items: deposit.items,
      stockItemIds,
    },
  };
...
function planSell(
  ...
  if (
    !heroOwned ||
    !merchantAcceptsItem(source, definition, session.encounter, guaranteedUniqueItemIds(content))
  ) {
  ...
  try {
    unitPrice = quoteMerchantSale({
      basePrice: definition.price,
      merchantBps: session.encounter.definition.merchantPurchaseBps,
  ...
  const items = state.items.map((candidate) =>
    candidate.itemId === source.itemId
      ? partial
        ? { ...source, quantity: source.quantity - command.quantity }
        : sold
      : candidate,
  );
  if (partial) items.push(sold);
  items.sort((left, right) => compareCodeUnits(left.itemId, right.itemId));
  const stockItemIds = [...session.population.stockItemIds, soldId].sort((left, right) =>
...
type ServicePlanResult =
  Readonly<{ ok: true; plan: ServicePlan }> | Readonly<{ ok: false; reason: InvalidActionReason }>;
...
function planService(
  input: Readonly<{
    state: ActiveRun;
    content: CompiledContentPack;
    command: TradeServiceCommand;
    session: MerchantSession;
  }>,
): ServicePlanResult {
  const { state, content, command, session } = input;
  const service = session.population.services.find(
    (candidate) => candidate.serviceId === command.serviceId,
  );
  if (
    !service ||
    service.remainingUses <= 0 ||
  ...
  if (command.serviceId === 'merchant-service.strongbox') {
    ...
    if (state.house.upgradesPurchased >= 1)
      return { ok: false, reason: 'trade.service-unavailable' };
    let price: number;
    try {
...
export function closeTrade(
  ...
  const closed: DomainEvent = {
    ...
    reason: input.reason,
    completedCommerce: trade.completedCommerce,
  };
  const base: ActiveRun = { ...input.state, activeTrade: null };
  if (
    input.reason !== 'player' ||
    !trade.completedCommerce ||
    population === undefined ||
    population.commerceBonusApplied
  ) {
  ...
  return {
    state: {
      ...changed.state,
      populations: replaceMerchantPopulation(changed.state, {
        ...population,
        commerceBonusApplied: true,
      }),
    },
 ...
 */
export function closeTradeIfInvalid(
  input: Readonly<{
    state: ActiveRun;
    content: CompiledContentPack;
    eventId: OpaqueId;
...
export function resolveTradeCommand(
  input: Readonly<{
    state: ActiveRun;
    command: TradeCommand;
    content: CompiledContentPack;
  ...
  if (command.type === 'trade-service') {
    ...
    const trade = state.activeTrade;
    if (trade === null)
      throw new Error('internal invariant: trade-service requires an active trade');
  ...
  if (plan === null || !plan.ok)
    throw new Error(`internal invariant: ${command.type} was not validated`);
  const trade = state.activeTrade;
  if (trade === null)
    throw new Error(`internal invariant: ${command.type} requires an active trade`);
  const population = state.populations.find(
    (candidate): candidate is MerchantPopulation =>
      candidate.model === 'merchant' && candidate.populationId === trade.merchantPopulationId,
  )!;
  const next: ActiveRun = {
    ...
    populations: replaceMerchantPopulation(state, {
      ...population,
      stockItemIds: plan.plan.stockItemIds,
    }),
    activeTrade: { ...trade, completedCommerce: true },
  };
  const commerce = {
