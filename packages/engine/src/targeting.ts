...
export type TargetInvalidReason = 'target.not_visible' | 'target.out_of_range' | 'target.blocked' | 'target.invalid';
...
function validatePoint(input: TargetValidationInput, point: Point, targetActorId?: OpaqueId): TargetValidation {
  const index = tileIndex(input.floor, point.x, point.y);
  if (index === undefined || !isVisible(input.visibilityWords, index)
    || (input.illumination.intensity[index] ?? 0) <= 0) {
    return { ok: false, reason: 'target.not_visible' };
  }
  const distance = Math.max(Math.abs(point.x - input.sourceActor.x), Math.abs(point.y - input.sourceActor.y));
  if (distance > input.range) return { ok: false, reason: 'target.out_of_range' };
  const cells = line(input.sourceActor, point);
  if (cells.slice(0, -1).some((cell) => {
...
export function validateTarget(input: TargetValidationInput): TargetValidation {
  if (!Number.isSafeInteger(input.range) || input.range < 0) throw new RangeError('target range must be a non-negative safe integer');
  if (input.sourceActor.floorId !== input.floor.floorId) return { ok: false, reason: 'target.invalid' };
  if (input.targetingId === 'target.self') {
    return { ok: true, cells: [{ x: input.sourceActor.x, y: input.sourceActor.y }], targetActorId: input.sourceActor.actorId };
  }
  if (input.targetingId === 'target.actor') {
    const target = input.targetActorId === null ? undefined
      : input.actors.find((actor) => actor.actorId === input.targetActorId && actor.floorId === input.floor.floorId);
