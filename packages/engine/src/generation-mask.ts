...
export function classicMask(width: number, height: number): readonly number[] {
  if (
    ...
    throw new GenerationError(
      ...
      'mask dimensions are outside the supported range',
    );
  }
...
function farthest(
  ...
  height: number,
  start: number,
): { index: number; distance: number; count: number } {
  ...
  for (let cursor = 0; cursor < queue.length; cursor += 1) {
    ...
    if (
      currentDistance > distance[best]! ||
      (currentDistance === distance[best]! && current < best)
    )
      best = current;
...
export function validateThemeMask(
  ...
  if (
    Array.from({ length: maskWords.length }, (_, index) => index).some(
      (index) =>
        !(index in maskWords) ||
        !Number.isInteger(maskWords[index]) ||
        maskWords[index]! < 0 ||
        maskWords[index]! > 0xffff_ffff,
    )
  ) {
    throw new GenerationError(
      'generation.invalid-theme',
      'theme mask words must be unsigned 32-bit integers',
    );
  }
  ...
  for (let x = 0; x < width; x += 1) {
    if (maskHas(maskWords, width, x, 0) || maskHas(maskWords, width, x, height - 1)) {
      throw new GenerationError(
        'generation.invalid-theme',
        'theme mask must exclude the outer border',
      );
    }
  }
  ...
  let start = -1;
  let cellCount = 0;
  for (let index = 0; index < usedBits; index += 1)
  ...
  if (first.count !== cellCount)
    throw new GenerationError('generation.invalid-theme', 'theme mask must be four-way connected');
  const second = farthest(maskWords, width, height, first.index);
  if (second.distance < minimumStairDistance) {
    throw new GenerationError(
      'generation.invalid-theme',
      'theme mask cannot satisfy the fallback stair distance',
    );
  }
...
export function createClassicTheme(
  ...
  if (
    ...
    throw new GenerationError(
      'generation.invalid-theme',
      'classic theme dimensions are outside the supported range',
    );
  }
  if (typeof settings !== 'object' || settings === null) {
    throw new GenerationError(
      'generation.invalid-theme',
      'classic theme settings must be an object',
    );
  }
  ...
  if (
    ...
    !Array.isArray(settings.ambient.color)
  ) {
  ...
  if (
    !Number.isSafeInteger(minimumRooms) ||
    minimumRooms < 1 ||
    !Number.isSafeInteger(minimumStairDistance) ||
