...
import {
  replacePopulation,
  sortedPopulations,
  synchronizeDeath as sharedSynchronizeDeath,
...
function encounter(content: CompiledContentPack, encounterId: OpaqueId): BossEncounterContentEntry {
  ...
  if (entry.maximumInstancesPerRun !== 1) {
    throw new Error(
      `internal invariant: boss encounter ${encounterId} must allow exactly one instance per run`,
    );
  }
  return entry;
}

function currentPhaseMaximum(
  population: BossPopulation,
  definition: BossEncounterContentEntry['definition'],
  maxHealth: number,
): number {
  if (population.currentPhaseId === null) return maxHealth;
  const phase = definition.phases.find(
    (candidate) => candidate.phaseId === population.currentPhaseId,
  );
  if (!phase)
    throw new Error(`internal invariant: boss phase ${population.currentPhaseId} does not exist`);
...
function mutatedFeature(feature: DungeonFeature, state: string): DungeonFeature | null {
  ...
  if (feature.type === 'secret' && state === 'secret.hidden')
    return { ...feature, state: 'hidden' };
  if (feature.type === 'secret' && state === 'secret.revealed')
    return { ...feature, state: 'revealed' };
  return null;
}

function bossEffectOperations(
  input: Readonly<{
  ...
  const arenaPlacements = (floors: ActiveRun['floors']) => {
    ...
    if (!floor)
      throw new Error(`internal invariant: boss floor ${input.population.floorId} does not exist`);
    return floor.vaults.filter((placement) => {
      const vault = entryById(input.content, placement.vaultId);
      return (
        vault?.kind === 'vault' &&
        input.definition.vaultTags.every((tag) => vault.tags.includes(tag))
      );
    });
  };
  ...
  return {
    'effect.feature.mutate': (operation) => {
      ...
      const features = operation.features.map((feature): DungeonFeature => {
        if (
          feature.floorId !== input.population.floorId ||
          !inArena(feature.x, feature.y, operation.floors)
        )
          return feature;
        ...
        if (
          feature.type === 'door' &&
          updated.type === 'door' &&
          feature.state !== updated.state &&
          (updated.state === 'open' || updated.state === 'closed')
        ) {
          events.push({
            type: updated.state === 'open' ? 'door.opened' : 'door.closed',
            eventId: operation.eventId,
            actorId: operation.sourceActorId,
      ...
      if (changed === 0)
        throw new Error(
          `internal invariant: boss arena feature target for ${state} does not exist`,
        );
    ...
    'effect.light.toggle': (operation) => {
      ...
      const items = operation.items.map((item) => {
        ...
        events.push({
          ...
          eventId: operation.eventId,
          actorId: operation.targetActorId,
          itemId: item.itemId,
          enabled,
        });
        return { ...item, enabled };
      ...
      const placementIds =
        placements === null ? null : new Set(placements.map((placement) => placement.placementId));
      const floors = operation.floors.map((floor) =>
        floor.floorId !== input.population.floorId
          ? floor
          : {
              ...floor,
              lights: floor.lights.map((light) => {
                if (
                  light.location.type !== 'fixed' &&
                  light.location.actorId !== operation.targetActorId
                )
                  return light;
                if (
                  placementIds !== null &&
                  (light.vaultPlacementId === null || !placementIds.has(light.vaultPlacementId))
                )
                  return light;
                changed += 1;
      ...
      if (changed === 0)
        throw new Error('internal invariant: boss arena light target does not exist');
    ...
    'effect.reveal': (operation) => {
      ...
      const features = operation.features.map((feature): DungeonFeature => {
        ...
        events.push({
          ...
          eventId: operation.eventId,
          actorId: input.heroId,
          featureId: feature.featureId,
        });
        const discoveredByActorIds = [...feature.discovery.discoveredByActorIds, input.heroId].sort(
          compareCodeUnits,
        );
    ...
    'effect.fuel.transfer': (operation) => {
      ...
      const owned = (item: (typeof operation.items)[number]) =>
        (item.location.type === 'backpack' || item.location.type === 'equipped') &&
        item.location.actorId === operation.targetActorId;
      const lights = operation.items
        .filter((item) => owned(item) && requireItem(input.content, item.contentId).light !== null)
      ...
      for (const lightItem of lights) {
        ...
        const fuel = operation.items
          .filter(
            (item) =>
              item.location.type === 'backpack' &&
              item.location.actorId === operation.targetActorId &&
              requireItem(input.content, item.contentId).tags.some((tag) =>
                light.fuelTags.includes(tag),
              ),
          )
        ...
        if (!consumed.ok)
          throw new Error(
            `internal invariant: boss arena fuel transfer failed with ${consumed.reason}`,
          );
        const items = consumed.items.map((item) =>
          item.itemId === lightItem.itemId
            ? { ...item, fuel: (lightItem.fuel ?? 0) + quantity }
            : item,
        );
        return {
          ...
          events: [
            {
              ...
              eventId: operation.eventId,
              actorId: operation.targetActorId,
              itemId: lightItem.itemId,
              fuelItemId: fuel.itemId,
              quantity,
              fuel: (lightItem.fuel ?? 0) + quantity,
            },
          ],
        };
      }
...
function applyNewPhases(
  ...
  const phases = input.definition.phases.filter(
    (phase) =>
      !crossed.has(phase.phaseId) &&
  ...
  const effectResult = resolveEffectSequence({
    effects: phases.flatMap((phase) => phase.effects),
    actors: input.state.actors,
    items: input.state.items,
    features: input.state.features,
    floors: input.state.floors,
    content: input.content,
    sourceActorId: input.boss.actorId,
    targetActorId: input.boss.actorId,
    effectsState: input.state.rng.effects,
    worldTime: input.state.worldTime,
    eventId: input.eventId,
    forceMoveDirection: { x: 1, y: 0 },
    operations: bossEffectOperations({
      content: input.content,
  ...
  const actors = effectResult.actors.map((actor) =>
    actor.actorId === input.boss.actorId ? { ...actor, behaviorId: finalPhase.behaviorId } : actor,
  );
  const phaseIds = [...input.population.crossedPhaseIds, ...phases.map((phase) => phase.phaseId)];
  const population: BossPopulation = {
    ...input.population,
    crossedPhaseIds: phaseIds,
    currentPhaseId: finalPhase.phaseId,
  };
  const phaseEvents: DomainEvent[] = phases.map((phase) => ({
  ...
  return {
    population,
    state: {
      ...applyEffectResult(input.state, effectResult),
      actors,
      features: effectResult.features,
      floors: effectResult.floors,
    },
    events: [...phaseEvents, ...effectResult.events],
  };
}

function recoverOnReentry(
  input: Readonly<{
    state: ActiveRun;
    population: BossPopulation;
    boss: ActorState;
  ...
  if (input.population.lastFloorExitAt === null)
  ...
  const rateAmount = Math.floor(
    elapsed * input.definition.recoveryPerWorldTime * input.boss.maxHealth,
  );
  const recoveryCap = Math.floor(
    (input.boss.maxHealth * input.definition.recoveryCapPercent) / 100,
  );
  const phaseMaximum = currentPhaseMaximum(
    input.population,
    input.definition,
    input.boss.maxHealth,
  );
  ...
  const recovered: BossPopulation = {
    ...population,
    recoveryHistory: [
      ...population.recoveryHistory,
      { at: input.state.activeFloorEnteredAt, amount },
    ],
  };
  return {
    state: withActor(input.state, boss),
...
function createRewards(
  ...
  const { state, createdItems, unique, receipt } = createPopulationLoot({
    content: input.content,
    state: input.state,
    tableId: input.definition.enhancedLootTableId,
...
export function bossCombatModifiers(
  input: Readonly<{
    state: Pick<ActiveRun, 'actors' | 'populations'>;
    content: CompiledContentPack;
    actorId: OpaqueId;
  }>,
  ...
  if (
    ...
    population.currentPhaseId === null
  )
    return ZERO_MODIFIERS;
  const phase = encounter(input.content, population.encounterId).definition.phases.find(
    (candidate) => candidate.phaseId === population.currentPhaseId,
  );
  if (!phase)
    throw new Error(`internal invariant: boss phase ${population.currentPhaseId} does not exist`);
...
export function advanceBosses(
  ...
  const bossEncounterIds = input.state.populations
    .filter((population) => population.model === 'boss')
  ...
  for (const original of sortedPopulations(state.populations)) {
    ...
    if (population.floorId !== state.activeFloorId) {
      if (population.lastFloorExitAt === null)
        population = { ...population, lastFloorExitAt: state.worldTime };
      state = replacePopulation(state, population);
      continue;
    }
    const recovered = recoverOnReentry({
      state,
      population,
      boss,
      definition,
    ...
    if (currentBoss.health <= 0) {
      ...
      state = replacePopulation(reward.state, reward.population);
      events.push(...reward.events);
      continue;
    }
    const phased = applyNewPhases({
      state,
      content: input.content,
      population,
      boss: currentBoss,
    ...
    state = phased.state;
    population = synchronizeDeath(
      phased.population,
      state.actors.find((actor) => actor.actorId === population.actorId)!,
    );
    ...
    const reward = createRewards({
      ...
      content: input.content,
      population,
      boss: state.actors.find((actor) => actor.actorId === population.actorId)!,
      definition,
      eventId: input.eventId,
    });
    state = reward.state;
    population = reward.population;
    events.push(...reward.events);
    state = replacePopulation(state, population);
  }
  return { state, events };
