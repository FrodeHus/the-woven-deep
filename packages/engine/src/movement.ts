...
export function movementAction(input: MovementActionInput): MovementActionResult {
  ...
  const occupant = input.actors.find(
    (candidate) =>
      candidate.actorId !== input.actor.actorId &&
      candidate.floorId === input.floor.floorId &&
      candidate.health > 0 &&
      candidate.x === to.x &&
      candidate.y === to.y,
  ...
  if (disposition === 'hostile')
    return { status: 'bump-attack', targetActorId: occupant.actorId, cost: input.cost };
  if (disposition === 'neutral') {
    return {
      status: 'decision_required',
      decision: { type: 'confirm-aggression', targetActorId: occupant.actorId },
    };
  }
