...
export function materializeMerchant(input: Readonly<{
  ...
  const effectiveDepth = definition.permanent ? Math.max(1, input.run.metrics.deepestDepth) : floor.depth;
  ...
  const graph = projectLootGraph({
    ...
    preflightLabel: 'merchant stock preflight', depth: effectiveDepth,
  ...
  if (root.choices.length === 0) {
    throw new Error(`internal invariant: merchant stock table ${definition.stockLootTableId} has no eligible choice at depth ${effectiveDepth}`);
  }
  ...
  const population: MerchantPopulation = {
    ...
    departureAt: definition.permanent ? null : input.run.worldTime + rolledLifetime, emittedWarningThresholds: [],
 ...
 */
export function restockMerchant(run: ActiveRun, input: Readonly<{
  content: CompiledContentPack;
  populationId: OpaqueId;
  ...
  if (!population) throw new Error(`internal invariant: merchant population ${input.populationId} does not exist`);
  ...
  if (!definition.permanent) {
    throw new Error(`internal invariant: restockMerchant requires a permanent merchant encounter, got ${population.encounterId}`);
  }
  const effectiveDepth = Math.max(1, run.metrics.deepestDepth);
  const graph = projectLootGraph({
    content: input.content, rootTableId: definition.stockLootTableId,
    preflightLabel: 'merchant restock preflight', depth: effectiveDepth,
    itemEligible: (item) => effectiveDepth >= item.minDepth && effectiveDepth <= item.maxDepth,
  });
  const root = graph.get(definition.stockLootTableId)!;
  if (root.choices.length === 0) {
    throw new Error(`internal invariant: merchant restock table ${definition.stockLootTableId} has no eligible choice at depth ${effectiveDepth}`);
  }
  if (boundedProduct(definition.maximumStockRolls, root.worstCreatedUnits, MAX_LOOT_CREATED_UNITS)
    > MAX_LOOT_CREATED_UNITS) {
    throw new RangeError(`merchant restock preflight: worst-case created items exceed runtime-safe limit ${MAX_LOOT_CREATED_UNITS}`);
  }
  ...
  const items = [
    ...run.items.filter((item) => !(item.location.type === 'merchant-stock'
  ...
  const event: DomainEvent = {
    type: 'merchant.restocked', eventId: `event.${input.populationId}.restock-${run.worldTime}`,
    populationId: input.populationId, actorId: population.actorId, stockItemIds,
  };
