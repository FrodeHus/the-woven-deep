...
const TERRAIN_IDS = Object.fromEntries(
  TILE_DEFINITIONS.map((definition) => [definition.name, definition.id]),
) as Readonly<Record<(typeof TILE_DEFINITIONS)[number]['name'], TileId>>;
...
function overlaps(
  candidate: Pick<Candidate, 'x' | 'y' | 'transformed'>,
  placed: VaultPlacement,
): boolean {
  return (
    candidate.x < placed.x + placed.width &&
    placed.x < candidate.x + candidate.transformed.width &&
    candidate.y < placed.y + placed.height &&
    placed.y < candidate.y + candidate.transformed.height
  );
}
...
function pointOutside(candidate: Candidate, x: number, y: number): boolean {
  return (
    x < candidate.x ||
    x >= candidate.x + candidate.transformed.width ||
    y < candidate.y ||
    y >= candidate.y + candidate.transformed.height
  );
}
...
function hasExternalEntranceConnection(topology: TopologyDraft, candidate: Candidate): boolean {
  return candidate.transformed.entrances.every((entrance) => {
    ...
    if (
      entrance.x !== 0 &&
      entrance.y !== 0 &&
      entrance.x !== candidate.transformed.width - 1 &&
      entrance.y !== candidate.transformed.height - 1
    )
      return false;
    return [
      [x, y - 1],
      ...
      if (
        !pointOutside(candidate, nextX!, nextY!) ||
        nextX! < 0 ||
        nextY! < 0 ||
        nextX! >= topology.width ||
        nextY! >= topology.height
      )
        return false;
      return tileDefinition(topology.tiles[nextY! * topology.width + nextX!]!)
...
function candidates(
  ...
  for (const room of rooms)
    for (const transformed of vaultTransforms(template)) {
      ...
      for (let y = minimumY; y <= maximumY; y += 1)
        for (let x = minimumX; x <= maximumX; x += 1) {
          ...
          for (const cell of transformed.cells) {
            const globalX = x + cell.x;
            const globalY = y + cell.y;
            const index = globalY * topology.width + globalX;
            if (
              topology.tiles[index] === 6 ||
              (globalX === topology.stairUp.x && globalY === topology.stairUp.y) ||
              (globalX === topology.stairDown.x && globalY === topology.stairDown.y)
            )
              eligible = false;
          }
          if (eligible && hasExternalEntranceConnection(topology, candidate))
...
function draftCandidate(
  ...
  for (const cell of candidate.transformed.cells) {
    tiles[(candidate.y + cell.y) * topology.width + candidate.x + cell.x] =
      TERRAIN_IDS[cell.terrain];
  }
  const vault: VaultPlacement = {
    placementId,
    vaultId: candidate.template.id,
    x: candidate.x,
    y: candidate.y,
    width: candidate.transformed.width,
    height: candidate.transformed.height,
    rotation: candidate.transformed.rotation,
    reflected: candidate.transformed.reflected,
    entrances: candidate.transformed.entrances.map(({ x, y }) => ({
      x: candidate.x + x,
      y: candidate.y + y,
    })),
  };
  const lights = candidate.transformed.fixtures.map(({ x, y, fixture }): LightSource => {
    ...
    return {
      lightId,
      location: { type: 'fixed', x: candidate.x + x, y: candidate.y + y },
      color: [...fixture.color] as [number, number, number],
      radius: fixture.radius,
      strength: fixture.strength,
      enabled: fixture.enabled,
      falloff: 'linear',
      vaultPlacementId: placementId,
  ...
  if (
    !vault.entrances.every(reachable) ||
    !placementSlots.filter((slot) => slot.required).every(reachable)
  )
    return null;
...
function chooseWeighted(
  templates: readonly VaultContentEntry[],
  state: Uint32State,
): readonly [VaultContentEntry, Uint32State] {
...
function chooseCandidate(
  ...
  state: Uint32State,
): readonly [Candidate, Uint32State] {
  const step = nextUint32(state);
...
export function placeVaults(
  ...
  const eligible = [...vaults]
    .filter(
      (vault) =>
        topology.depth >= vault.minDepth &&
        topology.depth <= vault.maxDepth &&
        (options.vaultTags ?? []).every((tag) => vault.tags.includes(tag)),
    )
    .sort((left, right) => compareText(left.id, right.id));
  const required =
    options.requiredVaultId === undefined
      ? undefined
      : eligible.find((vault) => vault.id === options.requiredVaultId);
  if (options.requiredVaultId !== undefined && !required)
    return { ok: false, code: 'vault.required-unavailable' };
  ...
  const attemptTemplate = (template: VaultContentEntry): boolean => {
    while ((counts.get(template.id) ?? 0) < template.maxPerFloor) {
      ...
      const selected = chooseCandidate(available, state);
      state = selected[1];
      ...
      tiles = draft.tiles;
      placed.push(...draft.vaults);
      lights.push(...draft.lights);
      placementSlots.push(...draft.placementSlots);
  ...
  if (required && !attemptTemplate(required))
    return { ok: false, code: 'vault.no-valid-placement' };
  while (true) {
    const availableTemplates = eligible.filter(
      (template) =>
        (counts.get(template.id) ?? 0) < template.maxPerFloor &&
        candidates({ ...topology, tiles }, template, placed, rejected).length > 0,
    );
    if (availableTemplates.length === 0) break;
    const selected = chooseWeighted(availableTemplates, state);
    state = selected[1];
