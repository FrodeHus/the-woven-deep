...
import { attributes, heroName, identifier, nullableIdentifier, populationBase, positiveQuantity, safeInteger, safeNonNegative, uint32, uint32State } from './primitives.js';
...
export const heroV6 = z.strictObject({ actorId: identifier, name: heroName, sightRadius: safeNonNegative,
...
export const roleMembership = z.strictObject({ actorId: identifier, roleId: z.string().min(1).max(80) });
...
export const populationV7 = z.discriminatedUnion('model', [...legacyPopulation.options, merchantPopulationV7]);
export const population = z.discriminatedUnion('model', [...legacyPopulation.options, merchantPopulation]);
...
import type { EncounterRunDecision, FallenHeroRunDecision, FallenHeroStandingSnapshot, PopulationInstance } from '../population-model.js';
...
type _EncounterDecisionDrift = Expect<SchemaMatches<z.infer<typeof encounterDecision>, EncounterRunDecision>>;
type _FallenStandingDrift = Expect<SchemaMatches<z.infer<typeof fallenStanding>, FallenHeroStandingSnapshot>>;
type _FallenDecisionDrift = Expect<SchemaMatches<z.infer<typeof fallenDecision>, FallenHeroRunDecision>>;
type _SurvivalDrift = Expect<SchemaMatches<z.infer<typeof survival>, SurvivalState>>;
type _IdentificationDrift = Expect<SchemaMatches<z.infer<typeof identification>, IdentificationState>>;
