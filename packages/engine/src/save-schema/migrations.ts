...
import { authoritativeEvent, eventOptions, hiddenPublicEventTypes, merchantDepartedEvent, merchantDepartureWarningEvent, merchantDiedEvent, merchantProvokedEvent, merchantStockDroppedEvent, populationCreatedEvent, processedResult, publicEvent, publicOnlyEventTypes, reputationChangedEvent, tradeBoughtEvent, tradeClosedEvent, tradeOpenedEvent, tradeServicePurchasedEvent, tradeSoldEvent } from './events.js';
import { floor } from './floor.js';
import { actor } from './actor.js';
import { feature, itemFields, itemLocationV7, legacyItemLocation } from './item.js';
import { encounterDecision, fallenDecision, fallenStanding, hero, heroV6, identification, legacyPopulation, populationV7, relationship, survival } from './population.js';
...
export const legacyPopulationCreatedEvent = z.strictObject({ type: z.literal('population.created'), eventId: identifier,
...
export const legacyEvent = z.discriminatedUnion('type', [...eventOptions, legacyPopulationCreatedEvent]);
export const legacyAuthoritativeEvent = legacyEvent.refine((value) => !publicOnlyEventTypes.has(value.type),
  'public projection event cannot be stored as authoritative');
export const legacyPublicEvent = legacyEvent.refine((value) => !hiddenPublicEventTypes.has(value.type),
...
export const legacyHero = z.strictObject({ actorId: identifier, name: heroName, sightRadius: safeNonNegative,
...
export const legacyRngEntries = Object.fromEntries(LEGACY_RNG_STREAM_NAMES.map((name) => [name, uint32State]));
export const legacyV5RngEntries = Object.fromEntries(LEGACY_V5_RNG_STREAM_NAMES.map((name) => [name, uint32State]));
export const legacyV5Event = z.discriminatedUnion('type', [...eventOptions, populationCreatedEvent, reputationChangedEvent,
...
export const legacyV5AuthoritativeEvent = legacyV5Event.refine((value) => !publicOnlyEventTypes.has(value.type),
  'public projection event cannot be stored as authoritative');
export const legacyV5PublicEvent = legacyV5Event.refine((value) => !hiddenPublicEventTypes.has(value.type),
  'authoritative or roll-bearing event must be projected before public storage');
