...
export const heroName = z.string().refine((name) => [...name].length >= 1 && [...name].length <= 40 && name.normalize('NFC') === name && !/[\p{Cc}\p{Cf}]/u.test(name));
...
export const uint32State = uint32Tuple.refine((state) => state.some((word) => word !== 0), 'state must not be all zero');
export const point = z.strictObject({ x: safeNonNegative, y: safeNonNegative });
export const direction = z.enum(['north', 'northeast', 'east', 'southeast', 'south', 'southwest', 'west', 'northwest']);
export const equipmentSlot = z.enum(['main-hand', 'off-hand', 'body', 'head', 'hands', 'feet', 'neck', 'left-ring', 'right-ring']);
export const positiveQuantity = z.number().int().safe().positive();
export const merchantServiceId = z.enum(['merchant-service.identify', 'merchant-service.strongbox']);
