...
export const entity = z.strictObject({ entityId: identifier, x: safeNonNegative, y: safeNonNegative });
export const color = z.tuple([uint8, uint8, uint8]);
export const ambient = z.strictObject({ color, strength: uint8 });
export const knowledge = z.strictObject({ exploredWords: z.array(uint32).readonly(), rememberedTerrainWords: z.array(uint32).readonly() });
...
export const fixedLocation = z.strictObject({ type: z.literal('fixed'), x: safeNonNegative, y: safeNonNegative });
...
export const tile = z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3), z.literal(4), z.literal(5), z.literal(6)]);
...
import type { FloorEntityPosition, FloorPlacementSlot, FloorSnapshot, VaultPlacement } from '../model.js';
