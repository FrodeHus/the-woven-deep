import { z } from 'zod';
import { direction, equipmentSlot, identifier, merchantServiceId, point, positiveQuantity, safeNonNegative } from './primitives.js';

export const moveCommand = z.strictObject({ type: z.literal('move'), commandId: identifier, expectedRevision: safeNonNegative, direction });
export const waitCommand = z.strictObject({ type: z.literal('wait'), commandId: identifier, expectedRevision: safeNonNegative });
...
export const commandBaseOptions = [
  ...
  z.strictObject({ ...commandBase, type: z.literal('trade-buy'), merchantPopulationId: identifier,
...
export const tradeServiceCommandV7 = z.strictObject({ ...commandBase, type: z.literal('trade-service'),
  merchantPopulationId: identifier, serviceId: z.literal('merchant-service.identify'), targetItemId: identifier });
export const tradeServiceCommand = z.strictObject({ ...commandBase, type: z.literal('trade-service'),
  merchantPopulationId: identifier, serviceId: merchantServiceId, targetItemId: identifier.nullable() });
export const houseDepositCommand = z.strictObject({ ...commandBase, type: z.literal('house-deposit'),
  itemId: identifier, quantity: positiveQuantity });
export const houseWithdrawCommand = z.strictObject({ ...commandBase, type: z.literal('house-withdraw'),
  itemId: identifier, quantity: positiveQuantity });
export const commandV7 = z.discriminatedUnion('type', [...commandBaseOptions, tradeServiceCommandV7]);
