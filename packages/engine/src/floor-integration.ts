...
function assertState(value: Uint32State, label: string): void {
  if (
    ...
    value.some(
      (word, index) =>
        !(index in value) || !Number.isInteger(word) || word < 0 || word > 0xffff_ffff,
    )
  ) {
...
export function integrateGeneratedFloor(
  ...
  if (
    !sameState(allocation.floorSeed, expectedAllocation.floorSeed) ||
  ...
  const transitioningToInsertedFloor =
    run.activeFloorId === generated.floor.floorId &&
  ...
  if (
    !isBootstrappingFirstFloor &&
    (previousFloor === undefined || generated.floor.floorId <= previousFloor.floorId)
  ) {
    throw new RangeError(
      'generated floor identifier must be a unique strict append in increasing order',
    );
  }
  ...
  if (generated.populationPlacement !== undefined) {
    ...
    if (placement.status === 'placed') {
      ...
      for (const diagnostic of placement.diagnostics)
        placementEvents.push({ ...diagnostic, eventId, floorId: generated.floor.floorId });
    }
    ...
    const result = placeFloorPopulations({
      ...
      content: population.content,
      ...(population.environmentTags === undefined
        ? {}
        : { environmentTags: population.environmentTags }),
      ...(population.forcedEncounterId === undefined
        ? {}
        : { forcedEncounterId: population.forcedEncounterId }),
    });
    const rejected = result.placements.find((candidate) => candidate.status === 'rejected');
    if (rejected)
      throw new RangeError(
        `required population placement rejected generated floor: ${rejected.reason}`,
      );
  ...
  const actorsAfterPlacement =
    fallen === null
      ? populatedRun.actors
      : [...populatedRun.actors, ...fallen.actors].sort((left, right) =>
          left.actorId < right.actorId ? -1 : left.actorId > right.actorId ? 1 : 0,
        );
  if (transitioningToInsertedFloor) {
    ...
    floor = {
      ...floor,
      knowledge: refreshKnowledge({ floor, hero: heroPerception(run.hero, actor), actors })
        .knowledge,
    };
  }
