...
export function relationshipBetween(
  ...
  const override = run.relationships.find(
    (candidate) =>
...
export function setRelationship(
  ...
  const pair = normalizedPair(leftActorId, rightActorId);
  const relationships = [
    ...run.relationships.filter(
      (candidate) =>
        candidate.leftActorId !== pair.leftActorId || candidate.rightActorId !== pair.rightActorId,
    ),
    { ...pair, relationship },
  ].sort((left, right) =>
    left.leftActorId < right.leftActorId
      ? -1
      : left.leftActorId > right.leftActorId
        ? 1
        : left.rightActorId < right.rightActorId
          ? -1
          : left.rightActorId > right.rightActorId
            ? 1
            : 0,
...
export function eligibleOpportunityAttackers(
  input: Readonly<{
    run: ActiveRun;
  ...
  if (actorHasConditionTrait(mover, 'condition-trait.avoids-opportunity-attacks', input.content))
    return [];
  return input.run.actors
    .filter(
      (candidate) =>
...
export function resolveOpportunityAttacks(
  ...
  for (const attackerId of attackerIds) {
    ...
    actors = actors.map((actor) =>
      actor.actorId === attackerId ? { ...actor, reactionReady: false } : actor,
    );
    events.push({
      type: 'reaction.triggered',
      eventId: input.eventId,
      actorId: attackerId,
      targetActorId: input.moverActorId,
    });
