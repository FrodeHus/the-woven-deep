...
function demoFloor(
  ...
  const base: FloorSnapshot = {
    ...
    tiles,
    entities: [],
    stairUp: withArena ? { x: 1, y: 6 } : null,
    stairDown: withArena ? { x: 10, y: 6 } : null,
    vaults: withArena
      ? [
          {
            placementId: 'vault.population-demo',
            vaultId: 'vault.lampwright-cache',
            x: 12,
            y: 1,
            width: 6,
            height: 11,
    ...
    placementSlots: withArena
      ? [
          {
            slotId: 'slot.champion',
            vaultPlacementId: 'vault.population-demo',
            kind: 'monster',
            required: false,
...
export function createPopulationDemoRun(pack: CompiledContentPack, scenarioSeed = 0): ActiveRun {
  ...
  let run: ActiveRun = {
    ...
    fallenHeroDecisions: selected.decisions.map((decision) => ({
      ...decision,
      retained: true,
  ...
  for (const definition of [group, swarm]) {
    const placement = placePopulation({
      run,
      floor: run.floors.find((floor) => floor.floorId === populationFloor.floorId)!,
      content: pack,
      forcedEncounterId: definition.id,
    });
    if (placement.status !== 'placed')
      throw new Error(`population fixture could not place ${definition.id}: ${placement.reason}`);
    run = publishPlacement(run, placement);
  }
  const bossPlacement = placePopulation({
    run,
    floor: run.floors.find((floor) => floor.floorId === bossFloor.floorId)!,
    content: pack,
    forcedEncounterId: boss.id,
  });
  if (bossPlacement.status !== 'placed')
    throw new Error(`population fixture could not place ${boss.id}: ${bossPlacement.reason}`);
  run = publishPlacement(run, bossPlacement);
  const fallen = placeFallenHeroEncounters({
    run,
    floor: run.floors.find((floor) => floor.floorId === bossFloor.floorId)!,
    content: pack,
  });
  run = {
    ...run,
    floors: run.floors.map((floor) =>
      floor.floorId === fallen.floor.floorId ? fallen.floor : floor,
    ),
    actors: [...run.actors, ...fallen.actors].sort((a, b) =>
      compareCodeUnits(a.actorId, b.actorId),
    ),
    populations: fallen.populations,
    fallenHeroDecisions: fallen.decisions,
  };
  ...
  const relationships = run.actors
    .flatMap((left, leftIndex) =>
      run.actors.slice(leftIndex + 1).map((right) => ({
    ...
    .sort(
      (left, right) =>
        compareCodeUnits(left.leftActorId, right.leftActorId) ||
  ...
  run = recordFloorEntered(
    {
      ...
      rng: {
        ...
        encounters: [
          seedWord,
          (seedWord ^ 0x9e3779b9) >>> 0,
          Math.imul(seedWord, 0x85ebca6b) >>> 0,
          Math.imul(seedWord ^ 0xc2b2ae35, 0x27d4eb2f) >>> 0,
        ],
      },
    },
    populationFloor.depth,
  );
...
export function validatePopulationInvariants(run: ActiveRun, pack: CompiledContentPack): void {
  ...
  for (const floor of run.floors) {
    ...
    const blockedPoints = run.actors
      .filter(
        (actor) =>
          actor.floorId === floor.floorId && actor.health > 0 && actor.populationId !== null,
      )
    ...
    if (
      ...
      throw new Error('population invariant: required route is disconnected');
  }
  ...
  for (const population of run.populations) {
    if (population.createdAt > run.worldTime)
      throw new Error('population invariant: creation is in the future');
    for (const actorId of population.livingMemberIds) {
      ...
      if (memberships.has(actorId))
        throw new Error(`population invariant: ${actorId} has duplicate membership`);
      memberships.add(actorId);
      const actor = run.actors.find((candidate) => candidate.actorId === actorId)!;
      if (actor.floorId !== population.floorId)
        throw new Error('population invariant: member is on the wrong floor');
    }
    if (
      population.model === 'group' &&
      population.sharedKnowledge.some((memory) => memory.source === 'group')
    ) {
      throw new Error(
        'population invariant: shared knowledge lost its legitimate observation source',
      );
    }
    if (
      population.model === 'boss' &&
      new Set(population.crossedPhaseIds).size !== population.crossedPhaseIds.length
    ) {
    ...
    if (
      ...
      definition.model === 'swarm'
    ) {
      const livingChildren = population.livingMemberIds.filter(
        (actorId) => actorId !== population.sourceActorId,
      ).length;
      const livingFloorActors = run.actors.filter(
        (actor) => actor.floorId === population.floorId && actor.health > 0,
      ).length;
      if (
        livingChildren > definition.definition.maximumLivingChildren ||
  ...
  const echoRecords = run.populations
    .filter((population) => population.model === 'echo')
  ...
  for (const population of run.populations.filter(
    ...
    if (
      run.items.filter((item) => item.itemId === `item.reward.${population.populationId}.unique`)
        .length !== 1
    ) {
...
export function populationDemoScenario(seed: number): PopulationDemoScenario {
  if (!Number.isSafeInteger(seed) || seed < 0)
    throw new RangeError('population scenario seed must be non-negative');
  const mixed = Math.imul((seed ^ (seed >>> 16)) >>> 0, 0x45d9f3b) >>> 0;
  return {
    seed,
    reloadMask: mixed & 0x1ff,
    relayMemberIndex: (mixed >>> 9) % 3,
    bossHealthPercent: 1 + ((mixed >>> 11) % 20),
    recoveryElapsed: 20 + ((mixed >>> 16) % 81),
  };
}

export function shrinkPopulationScenario(seed: number): readonly number[] {
  const values: number[] = [];
  for (let current = Math.floor(seed / 2); current > 0; current = Math.floor(current / 2))
    values.push(current);
  if (seed !== 0) values.push(0);
  return [...new Set(values)];
}

export function populationDemoCommands(
  initial: ActiveRun,
  scenario: PopulationDemoScenario,
): readonly PopulationDemoInput[] {
  const targetFor = (boundary: (typeof POPULATION_REPLAY_BOUNDARIES)[number]) => {
    if (boundary === 'before-leader-death') {
      return initial.populations.find(
        (population): population is GroupPopulation => population.model === 'group',
      )!.leaderActorId!;
    }
    ...
    if (boundary === 'before-echo-defeat')
      return initial.populations.find((population) => population.model === 'echo')!.actorId;
    if (boundary === 'before-reward-creation')
      return initial.populations.find((population) => population.model === 'boss')!.actorId;
    return null;
  };
  return POPULATION_REPLAY_BOUNDARIES.map((boundary, index) => {
    const common = {
      commandId: `command.population-demo-${String(index + 1).padStart(2, '0')}`,
...
function prepareCommandAttack(state: ActiveRun, targetActorId: string): ActiveRun {
  ...
  if (target.floorId !== hero.floorId)
    throw new Error(`population fixture attack target ${targetActorId} is off the hero floor`);
  ...
  for (const [dx, dy] of [
    ...
    if (
      ...
      x >= floor.width ||
      y >= floor.height ||
      movementBlockReason(floor.tiles[y * floor.width + x]!) !== undefined ||
      occupied.has(`${x}:${y}`)
    )
      continue;
    destination = { x, y };
  ...
  if (destination === null)
    throw new Error(`population fixture cannot place attack target ${targetActorId}`);
...
function preparePopulationDemoBoundary(
  state: ActiveRun,
  ...
  if (boundary === 'before-group-relay') {
    ...
    const observers = group.livingMemberIds
      .map((actorId) => state.actors.find((actor) => actor.actorId === actorId)!)
    ...
    return {
      ...state,
      actors: state.actors.map((actor) =>
        actor.actorId === observer.actorId
          ? {
              ...actor,
              behaviorState: {
                ...actor.behaviorState,
  ...
  if (boundary === 'before-source-spawn') {
    ...
    const initialSwarm = current.populations.find(
      (population): population is SwarmPopulation => population.model === 'swarm',
    )!;
    current = {
      ...current,
      actors: current.actors.map((actor) =>
        actor.actorId === initialSwarm.sourceActorId
          ? {
              ...actor,
              x: 17,
              y: 11,
      ...
      floors: current.floors.map((floor) =>
        floor.floorId === initialSwarm.floorId
          ? {
              ...floor,
              entities: floor.entities.map((entity) =>
                entity.entityId === initialSwarm.sourceActorId
                  ? { ...entity, x: 17, y: 11 }
                  : entity,
              ),
            }
    ...
    for (let attempt = 0; attempt < 16; attempt += 1) {
      const swarm = current.populations.find(
        (population): population is SwarmPopulation => population.model === 'swarm',
      )!;
      const livingChildren = swarm.livingMemberIds.filter(
        (actorId) => actorId !== swarm.sourceActorId,
      ).length;
      if (livingChildren >= definition.definition.maximumLivingChildren) break;
      const readyAt = Math.max(
        current.worldTime,
        swarm.nextSpawnAt,
        current.activeFloorEnteredAt + 1,
      );
      current = {
        ...current,
        worldTime: readyAt,
        actors: current.actors.map((actor) =>
          actor.actorId === swarm.sourceActorId ? { ...actor, energy: 100 } : actor,
        ),
        populations: current.populations.map((population) =>
          population.populationId === swarm.populationId
            ? { ...swarm, nextSpawnAt: readyAt }
            : population,
        ),
      };
      current = resolveSwarmSpawnAction({
        state: current,
        content: pack,
        sourceActorId: swarm.sourceActorId,
        eventId: `${command.commandId}.fixture`,
      }).state;
    }
    const swarm = current.populations.find(
      (population): population is SwarmPopulation => population.model === 'swarm',
    ...
    const removedChildId = [...swarm.livingMemberIds]
      .filter((actorId) => actorId !== swarm.sourceActorId)
      .sort()
      .at(-1)!;
    current = {
      ...current,
      ...
      floors: current.floors.map((floor) =>
        floor.floorId === swarm.floorId
          ? {
              ...floor,
              entities: floor.entities.filter((entity) => entity.entityId !== removedChildId),
            }
          : floor,
      ),
      populations: current.populations.map((population) =>
        population.populationId === swarm.populationId
          ? {
              ...swarm,
              spawnedCount: swarm.spawnedCount - 1,
              livingMemberIds: swarm.livingMemberIds.filter(
                (actorId) => actorId !== removedChildId,
              ),
            }
          : population,
      ),
    };
    const preparedSwarm = current.populations.find(
      (population): population is SwarmPopulation => population.model === 'swarm',
    )!;
    ...
    return {
      ...
      worldTime: readyAt,
      actors: current.actors.map((actor) =>
        actor.actorId === preparedSwarm.sourceActorId ? { ...actor, energy: 1_000 } : actor,
      ),
      populations: current.populations.map((population) =>
        population.populationId === preparedSwarm.populationId
          ? { ...preparedSwarm, nextSpawnAt: readyAt, emittedCapLevels: [] }
          : population,
      ),
    };
  }
  if (boundary === 'before-leader-death') {
    ...
    const ready = {
      ...
      populations: state.populations.map((population) =>
        population.populationId === swarm.populationId
          ? { ...swarm, nextSpawnAt: readyAt, emittedCapLevels: [] }
          : population,
      ),
    };
    ...
    return recordFloorEntered(
      {
        ...state,
        worldTime: enteredAt,
        activeFloorId: boss.floorId,
        activeFloorEnteredAt: enteredAt,
        actors: state.actors.map((actor) =>
          actor.actorId === state.hero.actorId
            ...
            : actor.actorId === boss.actorId
              ? {
                  ...
                  health: Math.max(
                    1,
                    Math.floor((actor.maxHealth * scenario.bossHealthPercent) / 100),
                  ),
                }
              : actor,
        ),
        populations: state.populations.map((population) =>
          population.populationId === boss.populationId
            ? { ...boss, lastFloorExitAt: state.worldTime }
            : population,
        ),
      },
      depth,
...
export function resolvePopulationDemoCommand(
  state: ActiveRun,
  input: PopulationDemoInput,
  pack: CompiledContentPack,
): Readonly<{
  ...
  if (replay.state.worldTime < state.worldTime)
    throw new Error(`population fixture time reversed at ${input.boundary}`);
...
export function runPopulationDemo(
  ...
  for (const [index, input] of populationDemoCommands(initial, scenario).entries()) {
    ...
    if (resolved.result.status !== 'applied')
      throw new Error(`population demo command ${input.command.commandId} was rejected`);
    records.push({
...
export function populationDemoEquivalent(
  left: PopulationDemoResult,
  right: PopulationDemoResult,
): boolean {
  return (
