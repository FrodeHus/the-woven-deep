...
export type EquipmentPlan = Readonly<{
  ok: true;
...
export function equipmentPlan(input: Readonly<{
  run: ActiveRun;
  ...
  if ((source.location.type !== 'backpack' && source.location.type !== 'equipped')
    || source.location.actorId !== actor.actorId) return { ok: false, reason: 'item.unavailable' };
  const definition = itemDefinition(input.content, source.contentId);
  if (!definition.equipment?.slots.includes(input.slot)) return { ok: false, reason: 'item.unavailable' };
  ...
  for (const item of input.run.items) {
    if (item.itemId === source.itemId || item.location.type !== 'equipped'
      || item.location.actorId !== actor.actorId) continue;
    const existing = itemDefinition(input.content, item.contentId).equipment;
    const existingOccupied = new Set<EquipmentSlot>([item.location.slot, ...(existing?.reservedSlots ?? [])]);
...
export type EquipmentTransition = Readonly<{ ok: true; run: ActiveRun }>
  | Readonly<{ ok: false; reason: 'item.missing' | 'item.unavailable' | 'inventory.full' }>;

export function equipItem(input: Readonly<{
  run: ActiveRun;
  content: CompiledContentPack;
  actorId: OpaqueId;
  itemId: OpaqueId;
  ...
  for (const item of input.run.items) {
    if (displaced.has(item.itemId) && item.location.type === 'equipped') equipment[item.location.slot] = null;
  }
  equipment[input.slot] = source.itemId;
  const items = input.run.items.map((item): ItemInstance => {
    if (item.itemId === source.itemId) return { ...item, location: { type: 'equipped', actorId: actor.actorId, slot: input.slot } };
    if (displaced.has(item.itemId)) return { ...item, location: { type: 'backpack', actorId: actor.actorId } };
    return item;
  });
  return { ok: true, run: {
    ...input.run, items,
    actors: input.run.actors.map((candidate) => candidate.actorId === actor.actorId ? { ...actor, equipment } : candidate),
  } };
}

export function unequipItem(input: Readonly<{
  run: ActiveRun;
  actorId: OpaqueId;
  slot: EquipmentSlot;
}>): EquipmentTransition {
  ...
  return { ok: true, run: {
    ...input.run,
    actors: input.run.actors.map((candidate) => candidate.actorId === actor.actorId ? { ...actor, equipment } : candidate),
    items: input.run.items.map((item) => item.itemId === itemId
      ? { ...item, location: { type: 'backpack' as const, actorId: actor.actorId } } : item),
  } };
}
...
export function equipmentModifiers(input: Readonly<{
  ...
  for (const slot of SLOT_ORDER) {
    ...
    if (definition.combat?.accuracy) {
      base[definition.combat.ammunitionTag ? 'rangedAccuracy' : 'meleeAccuracy'] = definition.combat.accuracy;
    }
...
export function itemLightSources(input: Readonly<{
  run: Pick<ActiveRun, 'actors' | 'items'>;
  content: CompiledContentPack;
  floorId: OpaqueId;
  ...
  for (const item of input.run.items) {
    ...
    lights.push({
      lightId: item.itemId, location, color: definition.light.color, radius: definition.light.radius,
      strength: definition.light.strength, enabled: true, falloff: 'linear',
      vaultPlacementId: null, presentation: null,
    });
  }
  return lights.sort((left, right) => left.lightId < right.lightId ? -1 : left.lightId > right.lightId ? 1 : 0);
}

function ownedBy(item: ItemInstance, actorId: OpaqueId): boolean {
  return (item.location.type === 'backpack' || item.location.type === 'equipped')
    && item.location.actorId === actorId;
}

export function toggleItemLight(input: Readonly<{
  run: ActiveRun; content: CompiledContentPack; actorId: OpaqueId; itemId: OpaqueId; enabled: boolean;
}>): EquipmentTransition {
...
export function refuelItem(input: Readonly<{
  run: ActiveRun; content: CompiledContentPack; actorId: OpaqueId;
  itemId: OpaqueId; fuelItemId: OpaqueId; quantity: number;
}>): RefuelTransition {
  ...
  if (!light || !ownedBy(target, input.actorId) || fuel.location.type !== 'backpack'
    || fuel.location.actorId !== input.actorId
    || !fuelDefinition.tags.some((tag) => light.fuelTags.includes(tag))) {
    return { ok: false, reason: 'item.unavailable' };
  }
  if (!Number.isSafeInteger(input.quantity) || input.quantity <= 0 || input.quantity > fuel.quantity) {
    return { ok: false, reason: 'item.unavailable' };
  }
  ...
  if (!consumed.ok) return { ok: false, reason: consumed.reason === 'item.missing' ? 'item.missing' : 'item.unavailable' };
  return { ok: true, quantity, run: { ...consumed.run,
    items: consumed.run.items.map((item) => item.itemId === target.itemId
      ? { ...item, fuel: (target.fuel ?? 0) + quantity } : item) } };
}
