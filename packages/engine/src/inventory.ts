 ...
 */
export function projectLootGraph(
  input: Readonly<{
  ...
  const visit = (tableId: OpaqueId, trail: readonly OpaqueId[]): ProjectedLootTable => {
    ...
    if (
      !Number.isSafeInteger(table.rolls) ||
      table.rolls <= 0 ||
      table.rolls > MAX_LOOT_TABLE_ROLLS
    ) {
      throw new RangeError(
        `${label}: roll count exceeds runtime-safe limit ${MAX_LOOT_TABLE_ROLLS}`,
      );
    }
    if (
      table.choices.some((choice) => !Number.isSafeInteger(choice.weight) || choice.weight <= 0) ||
      !checkedTotalWithin(
        table.choices.map((choice) => choice.weight),
        MAX_LOOT_WEIGHT_TOTAL,
      )
    ) {
    ...
    for (const choice of table.choices) {
      if (
        !Number.isSafeInteger(choice.maximumQuantity) ||
        choice.maximumQuantity <= 0 ||
        choice.maximumQuantity > MAX_LOOT_CHOICE_QUANTITY
      ) {
        throw new RangeError(
          `${label}: choice quantity exceeds runtime-safe limit ${MAX_LOOT_CHOICE_QUANTITY}`,
        );
      }
      if (
        !Number.isSafeInteger(choice.minimumQuantity) ||
        choice.minimumQuantity <= 0 ||
      ...
      if ((choice.contentId === null) === (choice.lootTableId === null)) {
        throw new Error(
          `internal invariant: loot table ${tableId} choice must have exactly one target`,
        );
      }
      const directItem =
        choice.contentId === null ? null : itemDefinition(content, choice.contentId);
      if (directItem && choice.maximumQuantity > directItem.stackLimit) {
        throw new RangeError(
          `${label}: choice quantity exceeds item stack limit ${directItem.stackLimit}`,
        );
      }
      if (choice.contentId !== null && bossUniqueIds.has(choice.contentId)) {
        throw new Error(
          `${label}: guaranteed boss-unique item ${choice.contentId} cannot appear in ordinary loot`,
        );
      }
      if (directItem !== null) {
    ...
    if (worst > MAX_LOOT_CREATED_UNITS) {
      throw new RangeError(
        `${label}: worst-case created units exceed runtime-safe limit ${MAX_LOOT_CREATED_UNITS}`,
      );
    }
 ...
 */
export function rollLootFromProjection(
  input: Readonly<{
    content: CompiledContentPack;
  ...
  const resolve = (tableId: OpaqueId): void => {
    ...
    if (!projected)
      throw new Error(
        `internal invariant: loot table ${tableId} is not part of the projected graph`,
      );
    ...
    for (let roll = 0; roll < projected.table.rolls; roll += 1) {
      const selected = rollDie(state, totalWeight);
      ...
      items.push({
        itemId: `${input.itemIdPrefix}.${String(items.length + 1).padStart(6, '0')}`,
  ...
  for (let iteration = 0; iteration < (input.rootIterations ?? 1); iteration += 1)
    resolve(input.tableId);
  return { items, state };
}

export function validateEchoLootGraph(
  input: Readonly<{
    content: CompiledContentPack;
    tableId: OpaqueId;
    ...
    for (const choice of table.choices) {
      if (choice.contentId !== null) {
        if (choice.contentId === input.recordedHeirloomContentId) {
          throw new Error(
            `Echo loot graph ${input.tableId} reaches recorded heirloom ${choice.contentId}; Echo rewards must be ordinary`,
          );
        }
        if (bossUniqueIds.has(choice.contentId)) {
          throw new Error(
            `Echo loot graph ${input.tableId} reaches guaranteed boss-unique item ${choice.contentId}; Echo rewards must be ordinary`,
          );
        }
...
export function createFloorLootFromTable(
  input: Readonly<{
    content: CompiledContentPack;
    tableId: OpaqueId;
    state: Uint32State;
 ...
 */
export function createPopulationLoot(
  input: Readonly<{
    content: CompiledContentPack;
    state: ActiveRun;
    tableId: OpaqueId;
  ...
  if ((input.uniqueContentId != null) !== (input.uniqueItemId != null)) {
    throw new Error(
      'internal invariant: createPopulationLoot requires both uniqueContentId and ' +
        'uniqueItemId together, or neither',
    );
  }
  const unique =
    input.uniqueContentId != null && input.uniqueItemId != null
      ? createFloorItem({
          content: input.content,
          contentId: input.uniqueContentId,
          itemId: input.uniqueItemId,
...
export function createRecordedHeirloom(
  input: Readonly<{
    content: CompiledContentPack;
  ...
  const item: ItemInstance = {
    ...
    heirloom: {
      ...
      originatingRank: 1,
      sourceItemId: input.snapshot.sourceItemId,
    },
  };
  return {
    item,
...
export function recordedHeirloomContentId(
  input: Readonly<{
    content: CompiledContentPack;
    snapshot: RecordedHeirloomSnapshot;
    equippedItemContentIds: readonly OpaqueId[];
...
export function canStack(left: ItemInstance, right: ItemInstance): boolean {
  return (
    left.heirloom === undefined &&
    right.heirloom === undefined &&
...
export function inventorySlotCount(
  input: Readonly<{
  ...
  const used = input.run.items.filter(
 ...
 */
export function depositIntoBackpack(
  input: Readonly<{
    run: ActiveRun;
    content: CompiledContentPack;
    actorId: OpaqueId;
  ...
  const backpack = input.run.items
    .filter(
      (entry) =>
        entry.location.type === 'backpack' &&
        entry.location.actorId === actor.actorId &&
        entry.itemId !== source.itemId &&
        canStack(entry, source),
    )
  ...
  if (remaining > 0) {
    ...
    if (
      ...
      input.run.items.some((entry) => entry.itemId === carriedId)
    ) {
...
export function pickupItem(
  ...
  if (!actor || !source) return failure('item.missing');
  if (
    source.location.type !== 'floor' ||
    source.location.floorId !== actor.floorId ||
    source.location.x !== actor.x ||
    source.location.y !== actor.y
  )
  ...
  if (remaining > 0) {
    ...
    if (carriedId !== source.itemId && input.run.items.some((item) => item.itemId === carriedId))
      return failure('item.id-conflict');
    carried = {
      ...source,
      itemId: carriedId,
...
export function dropItem(
  ...
  const source = input.run.items.find((item) => item.itemId === input.itemId);
  if (!actor || !source) return failure('item.missing');
  if (source.location.type !== 'backpack' || source.location.actorId !== actor.actorId)
    return failure('item.unavailable');
  if (input.quantity > source.quantity) return failure('item.quantity');
  ...
  if (partial && input.run.items.some((item) => item.itemId === droppedId))
    return failure('item.id-conflict');
  ...
  const items = input.run.items.map((item) =>
    item.itemId === source.itemId
      ? partial
        ? { ...source, quantity: source.quantity - input.quantity }
        : dropped
      : item,
  );
...
export function splitStack(
  ...
  if (source.location.type !== 'backpack' || source.location.actorId !== input.actorId)
    return failure('item.unavailable');
  if (
    input.quantity >= source.quantity ||
    input.quantity > itemDefinition(input.content, source.contentId).stackLimit
  ) {
    return failure('item.quantity');
  }
  if (input.run.items.some((item) => item.itemId === input.newItemId))
    return failure('item.id-conflict');
...
export function mergeStacks(
  ...
  if (
    left.location.type !== 'backpack' ||
    right.location.type !== 'backpack' ||
    left.location.actorId !== input.actorId ||
    right.location.actorId !== input.actorId
  )
    return failure('item.unavailable');
  ...
  const items = input.run.items.flatMap((item) => {
    if (item.itemId === receiver.itemId)
      return [{ ...receiver, quantity: receiver.quantity + transferred }];
    if (item.itemId !== donor.itemId) return [item];
    return donor.quantity === transferred
      ? []
      : [{ ...donor, quantity: donor.quantity - transferred }];
  });
  return success(input.run, items);
}
...
export function consumeItemQuantityFromItems(
  input: Readonly<{
