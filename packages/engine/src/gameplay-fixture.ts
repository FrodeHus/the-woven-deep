...
function selectActorCells(floor: FloorSnapshot, blocked: ReadonlySet<string>): Readonly<{
  hero: Point;
  rat: Point;
  beetle: Point;
}> {
...
function selectSecretCell(floor: FloorSnapshot): Point {
  ...
  const secret = wallCells.find(({ x, y }) =>
...
export function createGameplayDemoRun(pack: CompiledContentPack): GameplayDemoRun {
  ...
  const secret = selectSecretCell(generated.floor);
  ...
  const positions = selectActorCells(generated.floor, featureCells);
