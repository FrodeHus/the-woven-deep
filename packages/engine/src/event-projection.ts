  ...
  for (const event of input.events) {
    switch (event.type) {
      ...
      case 'actor.healed':
        ...
        break;
      case 'loot.dropped':
        if (actorVisible(event.actorId) && event.itemIds.every((itemId) => itemVisible(itemId))) output.push(event);
        break;
      ...
      case 'run.concluded': case 'run.finalized': case 'achievement.granted': output.push(event); break;
