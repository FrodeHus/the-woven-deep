import { z } from 'zod';
import { tileIndex, type Direction } from './model.js';
...
const activeRunV1Schema = z.strictObject({
...
function validateSemantics(run: ActiveRunV1): ActiveRunV1 {
...
export function validateActiveRunV1(input: unknown): ActiveRunV1 {
  const parsed = activeRunV1Schema.safeParse(input);
  if (!parsed.success) {
