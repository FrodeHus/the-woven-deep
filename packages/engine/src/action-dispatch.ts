...
type ActionDispatchRegistry = { [T in GameAction['type']]: ActionResolver<Extract<GameAction, { type: T }>> };

const resolveBumpAttack: ActionResolver<Extract<GameAction, { type: 'bump-attack' }>> = (
  { state, action, actor, content, eventId, events },
) => {
  let next = state;
  ...
  if (actor.playerControlled) {
    const merchant = next.populations.find((candidate): candidate is MerchantPopulation =>
    ...
    if (merchant) {
      const provoked = provokeMerchant({
        state: next, content, merchantPopulationId: merchant.populationId,
        sourceActorId: actor.actorId, eventId,
      });
      next = provoked.state;
  ...
  if (relationshipBetween(next, actor.actorId, action.targetActorId) !== 'hostile') {
    ...
    events.push({
      type: 'relationship.changed', eventId, actorId: actor.actorId,
      targetActorId: action.targetActorId, relationship: 'hostile',
    });
  }
  const resolved = combat({
    actors: next.actors, combatState: next.rng.combat, attackerId: actor.actorId,
    targetActorId: action.targetActorId, eventId, content,
    items: next.items, survival: next.survival, populations: next.populations,
    fallenHeroStandings: next.fallenHeroStandings, worldTime: next.worldTime, hero: next.hero,
  });
...
const resolveDoor: ActionResolver<Extract<GameAction, { type: 'open-door' | 'close-door' }>> = (
  { state, action, actor, eventId, events },
) => {
  ...
  events.push({ type: action.type === 'open-door' ? 'door.opened' : 'door.closed',
    eventId, actorId: actor.actorId, featureId: action.featureId });
...
const ACTION_DISPATCH: ActionDispatchRegistry = {
  ...
  'swarm-spawn': ({ state, actor, content, eventId, events }) => {
    const result = resolveSwarmSpawnAction({ state, content, sourceActorId: actor.actorId, eventId });
  ...
  search: ({ state, actor, content, eventId, events }) => {
    ...
    const result = searchFeatures({ run: state, actorId: actor.actorId,
      illumination: perception.illumination.intensity[index]!, eventId });
    events.push(...result.events);
    return { state: result.run, chargeEnergy: true };
  },
  disarm: ({ state, action, actor, content, eventId, events }) => {
    const result = disarmTrap({ run: state, content, actorId: actor.actorId,
      featureId: action.featureId, eventId });
    events.push(...result.events);
    return { state: result.run, chargeEnergy: true };
  },
  ...
  'toggle-light': ({ state, action, actor, content, eventId, events }) => {
    const transition = toggleItemLight({ run: state, content,
    ...
    events.push({ type: 'item.light-toggled', eventId, actorId: actor.actorId,
      itemId: action.itemId, enabled: action.enabled });
    return { state: transition.run, chargeEnergy: true };
  },
  refuel: ({ state, action, actor, content, eventId, events }) => {
    const transition = refuelItem({ run: state, content, actorId: actor.actorId,
    ...
    events.push({ type: 'item.refueled', eventId, actorId: actor.actorId,
  ...
  equip: ({ state, action, actor, content, eventId, events }) => {
    const transition = equipItem({ run: state, content, actorId: actor.actorId,
    ...
    events.push({ type: 'item.equipped', eventId, actorId: actor.actorId,
      itemId: action.itemId, slot: action.slot });
    return { state: transition.run, chargeEnergy: true };
  },
  unequip: ({ state, action, actor, eventId, events }) => {
    ...
    events.push({ type: 'item.unequipped', eventId, actorId: actor.actorId,
      itemId: action.itemId, slot: action.slot });
    return { state: transition.run, chargeEnergy: true };
  },
  'use-item': ({ state, action, actor, content, eventId, events }) => {
    ...
    const resolved = resolveEffectSequence({
      effects: definition.effects, actors: state.actors, items: state.items, content,
      sourceActorId: actor.actorId, sourceItemId: source.itemId, targetActorId: target.actorId,
      effectsState: state.rng.effects, worldTime: state.worldTime, eventId,
  ...
  fire: ({ state, action, actor, content, eventId, events }) => {
    ...
    const defense = profile(target, content, next.items, next.actors, next.survival,
      next.populations, next.fallenHeroStandings, next.worldTime, next.hero);
    const shot = resolveAttack({
      eventId, attackerId: actor.actorId, targetActorId: target.actorId,
      actors: next.actors, combatState: next.rng.combat,
  ...
  'throw-item': ({ state, action, actor, content, eventId, events }) => {
    ...
    if (definition.effects.some((effect) => effect.effectId === 'effect.item.consume')) {
      ...
      events.push({ type: 'item.thrown', eventId, actorId: actor.actorId,
  ...
  pickup: ({ state, action, actor, content, eventId, events }) => {
    const transition = pickupItem({
      run: state, content, actorId: actor.actorId, itemId: action.itemId,
      quantity: action.quantity, newItemId: action.newItemId,
    });
    if (!transition.ok) throw new Error(`internal invariant: validated pickup failed with ${transition.reason}`);
    events.push({ type: 'item.picked-up', eventId, actorId: actor.actorId,
  ...
  drop: ({ state, action, actor, eventId, events }) => {
    ...
    events.push({ type: 'item.dropped', eventId, actorId: actor.actorId,
      itemId: action.itemId, quantity: action.quantity });
    return { state: transition.run, chargeEnergy: true };
  },
  'split-stack': ({ state, action, actor, content, eventId, events }) => {
    ...
    events.push({ type: 'item.stack-split', eventId, actorId: actor.actorId,
  ...
  move: ({ state, action, actor, content, eventId, events }) => {
    ...
    const reactions = resolveOpportunityAttacks({
      run: next, content, moverActorId: actor.actorId,
      from: { x: actor.x, y: actor.y }, to: action.to, eventId,
      resolveAttack: (attack) => combat({ ...attack, content, items: next.items,
        survival: next.survival, populations: next.populations,
        fallenHeroStandings: next.fallenHeroStandings, worldTime: next.worldTime, hero: next.hero }),
    });
    ...
    if (reactions.movementAllowed) {
      ...
      const floor = current && next.floors.find((candidate) => candidate.floorId === current.floorId);
      const direction = current ? movementDirection(current, action.to) : null;
      const validated = current && floor && direction ? movementAction({
        actor: current, floor, actors: next.actors, features: next.features,
        relationships: next.relationships, direction, cost: action.cost,
      }) : null;
      if (validated?.status === 'move' && validated.to.x === action.to.x && validated.to.y === action.to.y) {
        ...
        events.push(actor.playerControlled
          ? { type: 'hero.moved', eventId, heroId: actor.actorId,
            from: { x: actor.x, y: actor.y }, to: validated.to }
          : { type: 'actor.moved', eventId, actorId: actor.actorId,
            from: { x: actor.x, y: actor.y }, to: validated.to });
        const trap = next.features.find((feature) => feature.type === 'trap' && feature.state === 'armed'
          && feature.floorId === actor.floorId && feature.x === validated.to.x && feature.y === validated.to.y);
        if (trap) {
          const triggered = triggerTrap({ run: next, content, actorId: actor.actorId,
            featureId: trap.featureId, eventId });
          next = triggered.run; events.push(...triggered.events);
        }
      }
    }
    return { state: next, chargeEnergy: true };
  ...
  wait: ({ state, actor, eventId, events }) => {
    if (actor.playerControlled) events.push({
      type: 'hero.waited', eventId, heroId: actor.actorId, x: actor.x, y: actor.y,
    });
...
export function applyAction(input: Readonly<{
  state: ActiveRun;
  action: GameAction;
  ...
  const resolver = (ACTION_DISPATCH[action.type] ?? resolveBumpAttack) as ActionResolver<GameAction>;
  const resolved = resolver({ state: input.state, action, actor, content: input.content, eventId: input.eventId, events });
