...
export function replacePopulationList(
  ...
  return populations.map((candidate) =>
    candidate.populationId === replacement.populationId ? replacement : candidate,
  );
}
...
export function synchronizeDeath<
  P extends Readonly<{
    livingMemberIds: readonly OpaqueId[];
    formerMemberIds: readonly OpaqueId[];
  }>,
