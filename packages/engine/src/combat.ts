...
export function resolveDamage(input: DamageResolutionInput): number {
  ...
  if (!Number.isSafeInteger(effective))
    throw new RangeError('effective damage must be a safe integer');
...
export function composePopulationCombatModifiers(
  ...
  return modifierSets.reduce<PopulationCombatModifiers>(
    (total, modifiers) => ({
...
export function applyPopulationCombatModifiers<
  T extends Readonly<{
    accuracy: number;
    defense: number;
    damage: DiceDefinition;
  }>,
...
export function resolveAttack(input: AttackResolutionInput): CombatResolution {
  ...
  if (
    !Number.isSafeInteger(input.damage.count) ||
    input.damage.count <= 0 ||
    !Number.isSafeInteger(input.damage.sides) ||
    input.damage.sides <= 0
  ) {
  ...
  rolledDamage = Math.max(
    0,
    safeInteger('rolled damage with bonus', rolledDamage + input.damage.bonus),
  );
