...
function validateStandings(standings: readonly FallenHeroStandingSnapshot[]): void {
  if (standings.length > 10)
    throw new RangeError('fallen hero standings may contain at most ten records');
  const ids = new Set<string>();
  standings.forEach((standing, index) => {
    if (standing.rank !== index + 1)
      throw new RangeError('fallen hero standings must be ordered contiguously by rank');
    if (ids.has(standing.hallRecordId))
      throw new RangeError(`duplicate fallen hero record ${standing.hallRecordId}`);
    ids.add(standing.hallRecordId);
  });
}

export function retainEchoCandidates(
  input: Readonly<{
  ...
  if (input.candidates.length !== input.rolls.length)
    throw new RangeError('every Echo candidate requires one roll');
  if (!Number.isFinite(input.chance) || input.chance < 0 || input.chance > 1)
    throw new RangeError('Echo chance is invalid');
  if (!Number.isSafeInteger(input.maximum) || input.maximum <= 0)
    throw new RangeError('Echo maximum must be positive');
  return input.candidates
    .map((candidate, index) => ({ candidate, roll: input.rolls[index]! }))
    .filter(
      ({ roll }) =>
    ...
    .map(({ candidate }) => candidate.hallRecordId);
}

export function createFallenHeroRunDecisions(
  input: Readonly<{
  ...
  const rolls = echoes.map(() => {
    const step = nextUint32(state);
    state = step.state;
    return step.value;
  });
  const retainedEchoes = new Set(
    retainEchoCandidates({
      candidates: echoes,
      rolls,
      chance: input.template.echoAppearanceChance,
      maximum: input.template.maximumEchoesPerRun,
    }),
  );
  const conquered = new Set(input.conqueredChampionRecordIds);
  return {
    state,
    decisions: input.standings.map((standing, index): FallenHeroRunDecision =>
      index === 0
        ? {
...
function monster(
  ...
  if (!entry || entry.kind !== 'monster')
    throw new Error(`Champion fallback monster ${template.fallbackMonsterId} does not exist`);
  return entry;
}
...
export function normalizeFallenHero(
  input: Readonly<{
  ...
  const championHealth = clamp(
    fallback.health,
    input.template.minimumHealth,
    input.template.maximumHealth,
  );
  ...
  for (const id of input.standing.equippedItemContentIds) {
    ...
    if (
      fallback.accuracy + nextAccuracy <= 0 ||
      fallback.defense + nextDefense <= 0 ||
      rawDamage + nextDamage <= 0
    )
      continue;
  ...
  const championDamage = Math.min(input.template.damageMaximum, rawDamage + equipmentDamage);
  const championDefense = Math.min(
    input.template.attributeMaximum,
    fallback.defense + equipmentDefense,
  );
  const championAccuracy = Math.min(
    input.template.attributeMaximum,
    fallback.accuracy + equipmentAccuracy,
  );
  const echo = input.role === 'echo';
  const health = echo
    ? Math.min(
        championHealth - 1,
        Math.floor((championHealth * input.template.echoHealthPercent) / 100),
      )
    : championHealth;
  const damageMaximum = echo
    ? Math.min(
        championDamage - 1,
        Math.floor((championDamage * input.template.echoDamagePercent) / 100),
      )
    : championDamage;
  const defenseMaximum = echo
    ? Math.min(
        championDefense - 1,
        Math.floor((championDefense * input.template.echoDefensePercent) / 100),
      )
    : championDefense;
  const accuracyMaximum = echo
    ? Math.min(
        championAccuracy - 1,
  ...
  return {
    displayName: echo
      ? boundedPrefixedDisplay('Echo of ', input.standing.heroName)
      : boundedSuffixedDisplay(input.standing.heroName, ", the Deep's Champion"),
    glyph: input.standing.portraitGlyph || fallback.glyph,
    color: fallback.color,
    monsterId: fallback.id,
    attributes,
    health: Math.max(1, health),
    damageMaximum: Math.max(0, damageMaximum),
    defenseMaximum: Math.max(0, defenseMaximum),
    accuracyMaximum: Math.max(0, accuracyMaximum),
    equipmentContentIds,
    abilityIds,
  };
}

export function fallenHeroCombatModifiers(
  input: Readonly<{
  ...
  if (!definition || !standing)
    throw new Error(`fallen hero combat state for ${actor.actorId} is incomplete`);
  ...
  return {
    accuracy: normalized.accuracyMaximum - fallback.accuracy,
...
export function assertEchoTemplateBoundaries(
  ...
  if (content === undefined) {
    if (
      definition.maximumHealth <= 1 ||
      definition.damageMaximum <= 0 ||
      definition.attributeMaximum <= 0
    ) {
  ...
  if (health <= 1 || damage <= 0 || defense <= 0 || accuracy <= 0) {
    throw new Error(
      'Echo health, damage, defense, and accuracy boundaries cannot be strictly weaker',
    );
  }
}
...
function requiredPoints(floor: FloorSnapshot): readonly Readonly<{ x: number; y: number }>[] {
  return [
    ...
    ...floor.placementSlots
      .filter((slot) => slot.kind === 'objective')
      .map(({ x, y }) => ({ x, y })),
...
export function placeFallenHeroEncounters(
  input: Readonly<{
    run: ActiveRun;
    floor: FloorSnapshot;
    content: CompiledContentPack;
  ...
  if (!definition || input.run.fallenHeroDecisions.length === 0) {
    return {
      floor: input.floor,
      actors: [],
      populations: input.run.populations,
  ...
  slots = slots.filter(
    (slot) => !selectedCells.some((cell) => cell.x === slot.x && cell.y === slot.y),
  );
  const decisions = input.run.fallenHeroDecisions.map((decision): FallenHeroRunDecision => {
    const standing = input.run.fallenHeroStandings.find(
    ...
    try {
      normalized = normalizeFallenHero({
        standing,
        template: definition,
        content: input.content,
        role: decision.role,
      ...
      if (
        ...
        /ability.*strictly weaker/i.test(error.message)
      ) {
        return decision;
      }
      throw error;
    ...
    const actor: ActorState = {
      ...
      behaviorId: fallback.behaviorId,
...
export function advanceFallenHeroEncounters(
  input: Readonly<{
    state: ActiveRun;
    content: CompiledContentPack;
    eventId: OpaqueId;
  ...
  for (const original of sortedPopulations(state.populations)) {
    ...
    if (!actor || !standing)
      throw new Error(`fallen hero population ${original.populationId} is incomplete`);
    ...
    if (population.model === 'champion') {
      ...
      events.push(
        ...
        },
        {
          ...
          contentId: reward.item.contentId,
    ...
    state = {
      ...state,
      populations: replacePopulationList(state.populations, population),
      fallenHeroDecisions: state.fallenHeroDecisions.map((decision) =>
        decision.hallRecordId === standing.hallRecordId
          ? { ...decision, encountered: true, defeated: true }
          : decision,
      ),
    };
  }
