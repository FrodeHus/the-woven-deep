...
function preflight(definition: SwarmEncounterContentEntry['definition']): void {
  ...
  if (definition.maximumSpawnQuantity > MAX_SWARM_SPAWN_QUANTITY) {
    throw new RangeError(
      `swarm preflight: spawn quantity exceeds runtime-safe limit ${MAX_SWARM_SPAWN_QUANTITY}`,
    );
  }
  if (
...
function capEvent(
  ...
  return population.emittedCapLevels.includes(level)
    ? null
    : {
        type: 'swarm.cap-reached',
        eventId,
        populationId: population.populationId,
        sourceActorId: population.sourceActorId,
        level,
      };
}
...
function spawnedActor(
  source: ActorState,
  definition: Extract<CompiledContentPack['entries'][number], { kind: 'monster' }>,
  population: SwarmPopulation,
  roleId: string,
  actorId: OpaqueId,
  x: number,
...
function legalCells(
  ...
  for (const feature of state.features)
    if (feature.floorId === floor.floorId) occupied.add(`${feature.x}:${feature.y}`);
  ...
  for (let y = 0; y < floor.height; y += 1)
    for (let x = 0; x < floor.width; x += 1) {
      if (
        Math.max(Math.abs(x - source.x), Math.abs(y - source.y)) > radius ||
...
function retreatGoal(state: ActiveRun, actor: ActorState) {
  const memory = [...actor.behaviorState.lastKnownTargets].sort(
    (a, b) => b.observedAt - a.observedAt || compareCodeUnits(a.observerActorId, b.observerActorId),
  )[0];
  ...
  const distance = (x: number, y: number) =>
    Math.max(Math.abs(x - memory.x), Math.abs(y - memory.y));
  const candidates = Array.from({ length: floor.width * floor.height }, (_, index) => ({
    ...
    .sort((a, b) => distance(b.x, b.y) - distance(a.x, a.y) || a.y - b.y || a.x - b.x);
...
export function swarmCombatModifiers(
  input: Readonly<{
  ...
  if (
    ...
    population.shutdownState !== 'frenzy' ||
    population.shutdownExpiresAt === null ||
    population.shutdownExpiresAt <= input.state.worldTime
  )
    return ZERO_MODIFIERS;
  return parseSwarmResponseParameters(
    encounter(input.content, population.encounterId).definition.responseParameters,
    'frenzy',
  ).modifiers;
}

export function advanceSwarms(
  input: Readonly<{
    state: ActiveRun;
    content: CompiledContentPack;
    eventId: OpaqueId;
    sourceActionActorId?: OpaqueId;
  }>,
  ...
  for (const original of sortedPopulations(state.populations)) {
    ...
    const floorSwarmCount = state.actors.filter(
      (actor) =>
        actor.floorId === population.floorId &&
        actor.health > 0 &&
        state.populations.some(
          (candidate) =>
            candidate.model === 'swarm' && candidate.populationId === actor.populationId,
        ),
    ).length;
    const retainedCapLevels = population.emittedCapLevels.filter((level) =>
      level === 'source'
        ? childCount >= def.maximumLivingChildren
        : level === 'encounter'
          ? population.livingMemberIds.length >= def.maximumLivingMembers
          : floorSwarmCount >= def.maximumFloorActors,
    );
    if (retainedCapLevels.length !== population.emittedCapLevels.length)
      population = { ...population, emittedCapLevels: retainedCapLevels };
    if (!sourceAlive && population.shutdownState === null) {
      const response = def.sourceDestructionResponse;
      population = {
        ...population,
        shutdownState: response,
        nextSpawnAt:
          response === 'decay'
            ? deadline(
                state.worldTime,
                parseSwarmResponseParameters(def.responseParameters, 'decay').interval,
              )
        ...
        shutdownExpiresAt:
          response === 'frenzy'
            ? deadline(
                state.worldTime,
                parseSwarmResponseParameters(def.responseParameters, 'frenzy').duration,
              )
            : null,
      };
      if (response === 'decay') {
        for (const actorId of population.livingMemberIds) {
          ...
          state = { ...state, actors: applied.actors };
    ...
    if (population.shutdownState === 'decay' && population.nextSpawnAt <= state.worldTime) {
          ...
          effects: [
            {
              ...
              parameters: {
                damageType: 'physical',
                dice: { count: 1, sides: 1, bonus: damage - 1 },
              },
      ...
      population = syncDeaths(
        { ...population, nextSpawnAt: deadline(state.worldTime, interval) },
        state.actors,
      );
    } else if (
      population.shutdownState === 'frenzy' &&
      ...
      population.shutdownState === null &&
      population.nextSpawnAt <= state.worldTime &&
      source &&
      ...
      if (levels.length > 0) {
        for (const level of levels) {
          const event = capEvent(population, level, input.eventId);
        ...
        const quantityRoll = rollDie(rng, def.maximumSpawnQuantity - def.minimumSpawnQuantity + 1);
        rng = quantityRoll.state;
        const requested = def.minimumSpawnQuantity + quantityRoll.value - 1;
        const capacity = Math.min(
          def.maximumLivingChildren - childCount,
          def.maximumLivingMembers - population.livingMemberIds.length,
          def.maximumFloorActors - floorSwarmCount,
        );
        ...
        if (requested > def.maximumLivingMembers - population.livingMemberIds.length)
          limitingLevels.push('encounter');
        ...
        for (let index = 0; index < quantity; index += 1) {
          const choice = rollDie(rng, totalWeight);
          rng = choice.state;
          let cursor = choice.value;
          const role = def.spawnRoles.find((candidate) => (cursor -= candidate.weight) <= 0)!;
          const monster = input.content.entries.find(
            (entry) => entry.kind === 'monster' && entry.id === role.monsterId,
          );
          if (!monster || monster.kind !== 'monster')
            throw new Error(`internal invariant: swarm monster ${role.monsterId} does not exist`);
          ...
          if (state.actors.some((actor) => actor.actorId === actorId))
            throw new Error(`internal invariant: duplicate swarm actor ${actorId}`);
          created.push(
            spawnedActor(
              source,
              monster,
        ...
        population = {
          ...
          peakLivingSize: Math.max(
            population.peakLivingSize,
            population.livingMemberIds.length + created.length,
          ),
        ...
        state = {
          ...state,
          rng: { ...state.rng, encounters: rng },
          actors: [...state.actors, ...created].sort((a, b) =>
            compareCodeUnits(a.actorId, b.actorId),
          ),
        };
        if (created.length > 0)
          events.push({
            type: 'swarm.members-created',
            eventId: input.eventId,
            populationId: population.populationId,
            sourceActorId: population.sourceActorId,
            actorIds,
        ...
        for (const level of limitingLevels) {
          if (!original.emittedCapLevels.includes(level))
            events.push({
              type: 'swarm.cap-reached',
              eventId: input.eventId,
              populationId: population.populationId,
...
export function swarmSpawnAction(
  input: Readonly<{ state: ActiveRun; content: CompiledContentPack; actorId: OpaqueId }>,
) {
  const actor = input.state.actors.find((candidate) => candidate.actorId === input.actorId);
  const population = input.state.populations.find(
    (candidate) => candidate.model === 'swarm' && candidate.sourceActorId === input.actorId,
  );
  if (
    !actor ||
    actor.health <= 0 ||
    actor.energy < balanceEntry(input.content).readinessThreshold ||
    population?.model !== 'swarm' ||
    !population.livingMemberIds.includes(actor.actorId) ||
    population.floorId !== input.state.activeFloorId ||
    population.shutdownState !== null ||
    population.nextSpawnAt > input.state.worldTime ||
    population.nextSpawnAt <= input.state.activeFloorEnteredAt
  )
    return null;
  return {
    type: 'swarm-spawn' as const,
    actorId: actor.actorId,
    cost: actionCostFor(balanceEntry(input.content), 'action.spawn'),
  };
}

export function resolveSwarmSpawnAction(
  input: Readonly<{
    state: ActiveRun;
    content: CompiledContentPack;
  ...
  const action = swarmSpawnAction({
    state: input.state,
    content: input.content,
    actorId: input.sourceActorId,
  });
  if (!action)
    throw new Error(`internal invariant: swarm source ${input.sourceActorId} cannot spawn`);
  ...
  return {
    ...
    state: {
      ...
      actors: result.state.actors.map((actor) =>
        actor.actorId === input.sourceActorId ? chargeActionEnergy(actor, action.cost) : actor,
      ),
    },
  };
}
