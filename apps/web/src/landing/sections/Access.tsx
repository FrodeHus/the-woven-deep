  ...
  return (
    <section id="access" className="wd-section wd-access" aria-labelledby="access-heading">
      <div data-reveal className="wd-section-intro">
        ...
        <h2 id="access-heading" className="wd-h2">{ACCESS.heading}</h2>
        ...
        <div data-reveal className="wd-access-card wd-access-guest">
          ...
          <ul className="wd-benefit-list">
            {ACCESS.guest.positives.map((text) => (
              <li key={text}><span aria-hidden="true" className="wd-mark wd-mark-positive">›</span>{text}</li>
            ))}
            {ACCESS.guest.limits.map((text) => (
              <li key={text} className="wd-benefit-limit"><span aria-hidden="true" className="wd-mark wd-mark-limit">✕</span>{text}</li>
            ))}
          </ul>
          <a href={PLAY_ROUTE} className="wd-cta-outline wd-cta-block">{ACCESS.guest.cta}</a>
        </div>
        <div data-reveal className="wd-access-card wd-access-member">
          ...
          <ul className="wd-benefit-list">
            {ACCESS.member.benefits.map((text) => (
              <li key={text}><span aria-hidden="true" className="wd-mark wd-mark-star">✦</span>{text}</li>
            ))}
          </ul>
          ...
          <a href={REGISTRATION_COMING_SOON_HREF} className="wd-cta-primary wd-cta-block">{ACCESS.member.cta}</a>
        </div>
      </div>
    </section>
  );
