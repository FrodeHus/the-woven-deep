...
describe('routeKey', () => {
  ...
  it('maps Shift+T to trade-open -- bare "t" is unbound so it never collides with vi movement', () => {
    expect(routeKey({ event: keyEvent('T', { shiftKey: true }), overlayOpen: false })).toEqual({ type: 'trade-open' });
    expect(routeKey({ event: keyEvent('T', { shiftKey: false }), overlayOpen: false })).toBeNull();
    expect(routeKey({ event: keyEvent('t'), overlayOpen: false })).toBeNull();
  });
