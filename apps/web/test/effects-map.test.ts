...
describe('effectsForEvents', () => {
  ...
  it('maps hero.damaged to a hit-flash at the hero cell', () => {
    const events: PublicEvent[] = [{ type: 'hero.damaged', amount: 3, damageType: 'physical' }];
    expect(effectsForEvents(events, HERO, positions)).toEqual([
  ...
  it('caps at MAX_TRANSIENT_EFFECTS, dropping the oldest first', () => {
    const events: PublicEvent[] = Array.from(
      { length: MAX_TRANSIENT_EFFECTS + 5 },
      (_unused, index) => ({
    ...
    expect(mapped.at(-1)).toEqual({
      key: `event.${MAX_TRANSIENT_EFFECTS + 4}`,
      kind: 'hit-flash',
      x: 8,
      y: 5,
    });
