...
import {
  requestLogin, fetchSession, logout, fetchProfileSettings, putProfileSettings,
} from '../../src/api.js';
...
describe('requestLogin', () => {
  it('posts the email to /api/auth/login and resolves on 200', async () => {
    ...
    await expect(requestLogin('player@example.com', fetcher as unknown as typeof fetch)).resolves.toBeUndefined();
...
describe('fetchProfileSettings', () => {
  it('maps {settings, settingsVersion} to {settingsJson, settingsVersion}', async () => {
    const fetcher = vi.fn().mockResolvedValueOnce(new Response(
      JSON.stringify({ settings: '{"a":1}', settingsVersion: 3 }),
      { status: 200 },
    ));
...
describe('putProfileSettings', () => {
  ...
  it('maps a non-200 response to ok:false', async () => {
    const fetcher = vi.fn().mockResolvedValueOnce(new Response(JSON.stringify({ error: 'stale' }), { status: 400 }));
