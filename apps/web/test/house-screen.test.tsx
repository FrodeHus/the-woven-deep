...
function snapshotOf(projection: GameplayProjection): SessionSnapshot {
  return {
    ...
    houseOpen: true, conclusion: null, sightings: { monsterIds: [], itemIds: [], landmarks: [] }, heroClassTags: [], onboarding: { counts: {}, dismissed: [] },
  };
}

function withHouseItems(projection: GameplayProjection, items: readonly { itemId: string; name: string }[], capacity = 6): GameplayProjection {
...
describe('HouseScreen', () => {
  it('shows the capacity readout for an empty house', () => {
    render(<HouseScreen snapshot={snapshotOf(baseProjection)} onDispatch={vi.fn()} onClose={vi.fn()} />);
  ...
  it('deposits the selected backpack item on Enter (keyboard-only)', async () => {
    ...
    render(<HouseScreen snapshot={snapshotOf(baseProjection)} onDispatch={onDispatch} onClose={vi.fn()} />);
    ...
    expect(onDispatch).toHaveBeenCalledWith({
      type: 'house-transfer', action: 'deposit', itemId: first.itemId, quantity: 1,
    });
  });
  ...
  it('switches focus to the house column on Tab and withdraws the selected house item on Enter', async () => {
    const user = userEvent.setup();
    const onDispatch = vi.fn();
    const projection = withHouseItems(baseProjection, [{ itemId: 'item.house-test.sword', name: 'Old sword' }]);
    render(<HouseScreen snapshot={snapshotOf(projection)} onDispatch={onDispatch} onClose={vi.fn()} />);

    await user.keyboard('{Tab}');
    await user.keyboard('{Enter}');

    expect(onDispatch).toHaveBeenCalledWith({
      type: 'house-transfer', action: 'withdraw', itemId: 'item.house-test.sword', quantity: 1,
    });
  });
  ...
  it('moves the selection within the focused list with ArrowUp/ArrowDown', async () => {
    ...
    expect(onDispatch).toHaveBeenCalledWith({
      type: 'house-transfer', action: 'withdraw', itemId: 'item.house-test.b', quantity: 1,
    });
  });

  it('renders correctly and still allows dispatching a deposit attempt when the house is at capacity', () => {
    const onDispatch = vi.fn();
    const fullItems = Array.from({ length: 6 }, (_, index) => ({ itemId: `item.house-test.${index}`, name: `Stack ${index}` }));
  ...
  it('closes on Escape', async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    render(<HouseScreen snapshot={snapshotOf(baseProjection)} onDispatch={vi.fn()} onClose={onClose} />);
