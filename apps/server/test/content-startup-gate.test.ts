...
function runtime(overrides: Partial<Parameters<typeof verifyContentStartupGate>[0]> = {}) {
  ...
  const implementation = {
    async startValid() {
      calls.push('start-valid');
    },
    async assertReadOnly() {
      calls.push('assert-read-only');
    },
    async smokeValid() {
...
describe('content startup integration gate orchestration', () => {
  it('publishes a repeatable root gate and a read-only compose content mount', async () => {
    ...
    expect(packageJson.scripts['content:startup-gate']).toBe(
      'node scripts/content-startup-gate.mjs',
    );
  ...
  it('fails when the invalid schema starts', async () => {
    ...
    await expect(verifyContentStartupGate(fake.implementation)).rejects.toThrow(
      'invalid content pack unexpectedly started',
    );
  });
  ...
  it('fails when rejected content changes immutable publication', async () => {
    ...
    await expect(verifyContentStartupGate(fake.implementation)).rejects.toThrow(
      'rejected content changed published packs',
    );
  });
});
